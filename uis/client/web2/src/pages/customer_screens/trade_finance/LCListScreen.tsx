...
import { FiFileText, FiPlus, FiChevronRight, FiRefreshCw, FiAlertCircle } from 'react-icons/fi';
...
const LCListScreen: React.FC = () => {
  ...
  const fetchLCs = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await tradeFinanceService.getLCs();
      setLcs(data);
    } catch {
      setError('Failed to load letters of credit');
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-800 shadow-sm">
        <div className="px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-gray-900 dark:text-white">Letters of Credit</h1>
          </div>
          <button
            onClick={() => navigate('/trade-finance/lc/apply')}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold"
            style={{ backgroundColor: '#05966914', color: '#059669', border: '1px solid #05966933' }}
          >
            <FiPlus size={16} />
            Apply for LC
          </button>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 px-12 text-center">
          <div className="p-7 rounded-full" style={{ backgroundColor: '#05966914' }}>
            <FiFileText size={72} color="#059669" />
          </div>
          <p className="text-xl font-bold text-gray-900 dark:text-white">No Letters of Credit</p>
        ...
        <div className="p-5 space-y-4">
          {lcs.map((lc) => {
            ...
            return (
              <button
                key={lc.id}
                onClick={() => navigate('/trade-finance/lc/details', { state: { lc } })}
                className="w-full text-left bg-white dark:bg-gray-800 rounded-2xl p-[18px] transition-shadow hover:shadow-md"
                style={{ border: `1.5px solid ${color}26`, boxShadow: `0 8px 20px ${color}12, 0 3px 10px rgba(0,0,0,0.05)` }}
              >
                {/* Header row */}
                <div className="flex items-start gap-3.5">
                  <div className="p-3 rounded-2xl flex-shrink-0" style={{ background: `linear-gradient(135deg, #05966933, #05966914)` }}>
                    <FiFileText size={24} color="#059669" />
                  </div>
                ...
                >
                  <InfoCell label="Amount" value={formatAmount(lc.amount, lc.currency)} />
                  <InfoCell label="Type" value={lc.type} />
                  {lc.expiryDate && <InfoCell label="Expiry" value={lc.expiryDate.substring(0, 10)} />}
                </div>

                {/* SWIFT ref */}
                {lc.swiftRef && (
                  <p className="mt-2.5 text-[12px] text-gray-400 dark:text-gray-500 font-mono">
                    SWIFT: {lc.swiftRef}
                  </p>
                )}
              </button>
