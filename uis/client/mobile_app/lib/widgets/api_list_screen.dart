...
class _ApiListScreenState extends State<ApiListScreen> {
  ...
  String get _endpoint => widget.apiEndpoint ?? widget.apiPath ?? '';

  List<String> get _keys {
    if (widget.columnKeys != null && widget.columnKeys!.isNotEmpty) {
      return widget.columnKeys!;
    }
    return widget.columnLabels.map((l) => l.toLowerCase().replaceAll(' ', '_')).toList();
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      if (connectivity.isOnline) {
        ...
        final items = _parseResponse(responseData is Map<String, dynamic> ? responseData : {'items': responseData});
        if (items.isNotEmpty) {
          setState(() {
            _items = items;
            _loading = false;
          ...
          // Cache the response
          await cache.cacheResponse(_endpoint, responseData);
          return;
        }
      }
    } catch (_) {
    ...
    try {
      final cached = connectivity.isOnline
          ? await cache.getCachedResponse(_endpoint)
          : await cache.getCachedResponseForce(_endpoint);
      if (cached != null) {
  ...
  Future<void> _createItem(Map<String, String> data) async {
    ...
    if (connectivity.isOnline) {
      try {
        await api.post(_endpoint, data: data.map((k, v) => MapEntry(k, v)));
        // Refresh from API
        _loadData();
      } catch (_) {
        // Queue for later sync
        await CacheService.instance.queueMutation('POST', _endpoint, data);
        _updatePendingCount();
        if (mounted) {
  ...
  Future<void> _deleteItem(Map<String, String> item, int index) async {
    ...
    if (connectivity.isOnline) {
      try {
        ...
        await CacheService.instance.queueMutation('DELETE', '$_endpoint/$id', null);
        _updatePendingCount();
      }
    } else {
      await CacheService.instance.queueMutation('DELETE', '$_endpoint/$id', null);
  ...
  Future<void> _syncPending() async {
    ...
    for (final m in mutations) {
      try {
        ...
        switch (method) {
          case 'POST':
            if (body != null) await api.post(endpoint, data: body);
            break;
          case 'PUT':
            if (body != null) await api.put(endpoint, data: body);
            break;
          case 'DELETE':
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ..._keys.asMap().entries.map((e) => Padding(
  ...
  void _showDetail(Map<String, String> item) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(item[_keys[0]] ?? widget.title),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: List.generate(_keys.length, (i) => Padding(
