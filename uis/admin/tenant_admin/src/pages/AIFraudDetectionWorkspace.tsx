...
export default function AIFraudDetectionWorkspace() {
  const [items, setItems] = useState<any[]>([]);
  const [search, setSearch] = useState("");
  const [stats, setStats] = useState<any>(null);

  useEffect(() => {
    fetch("/aml-case-manager/v1/alerts")
      .then(r => r.json())
  ...
  const columns: Column[] = [{key:"id",label:"ID"},{key:"transactionId",label:"Transaction"},{key:"riskScore",label:"Risk Score"},{key:"ruleTriggered",label:"Rule"},{key:"amount",label:"Amount"},{key:"action",label:"Action"},{key:"createdAt",label:"Time"}];
  ...
  return (
    <div style={{padding:"1.5rem"}}>
      <h1 style={{fontSize:"1.5rem",fontWeight:700,marginBottom:"1rem"}}>AI Fraud Detection</h1>
      {stats && (
