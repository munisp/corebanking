...
export default function ESGBankingWorkspace() {
  const [items, setItems] = useState<any[]>([]);
  const [search, setSearch] = useState("");
  const [stats, setStats] = useState<any>(null);

  useEffect(() => {
    fetch("/agriculture/v1/esg-impact")
      .then(r => r.json())
  ...
  const columns: Column[] = [{key:"id",label:"ID"},{key:"entityName",label:"Entity"},{key:"environmentScore",label:"E Score"},{key:"socialScore",label:"S Score"},{key:"governanceScore",label:"G Score"},{key:"overallScore",label:"Overall"},{key:"rating",label:"Rating"}];
  ...
  return (
    <div style={{padding:"1.5rem"}}>
      <h1 style={{fontSize:"1.5rem",fontWeight:700,marginBottom:"1rem"}}>ESG / Carbon Credit Banking</h1>
      {stats && (
