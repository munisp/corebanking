...
const MyTemporalAccess: React.FC = () => {
  ...
  const tenantId = import.meta.env.VITE_TENANT_ID; // Get from auth context
  ...
  const checkPermission = async (
    ...
    try {
      ...
      if (result.allowed) {
        ...
        if (result.requires_mfa) message += " (MFA required)";
        if (result.requires_approval) message += " (Approval required)";
