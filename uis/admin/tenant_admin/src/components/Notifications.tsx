...
export default function Notifications() {
  ...
  const handleToggle = () => {
    if (!isOpen && buttonRef.current) {
      ...
      setPanelStyle({
        top: rect.bottom + 8,
        // align panel's right edge with button's right edge; clamp so it never goes off-screen
        left: Math.max(8, Math.min(rect.right - 384, window.innerWidth - 392)),
      });
    }
    setIsOpen(prev => !prev);
  };
  ...
  return (
    <div className="relative">
      ...
      {/* Portal: renders directly on document.body — bypasses all ancestor overflow/stacking-context issues */}
      {isOpen && createPortal(
        <>
          {/* Backdrop */}
          <div
          ...
          {/* Notification Panel */}
          <div
            className="fixed w-96 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-gray-200 dark:border-slate-700 z-[9999]"
