...
SERVICE_PREFIX = "/circuit-breaker"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "circuit-breaker-rs"
...
def test_circuit_breaker_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"circuit-breaker-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_circuit_breaker_rs_has_runtime_entrypoint():
