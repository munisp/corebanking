...
SERVICE_PREFIX = "/tenant-billing"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tenant-billing-go"
...
def test_tenant_billing_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tenant-billing-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tenant_billing_go_has_runtime_entrypoint():
