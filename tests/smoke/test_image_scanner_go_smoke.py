...
SERVICE_PREFIX = "/image-scanner"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "image-scanner-go"
...
def test_image_scanner_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"image-scanner-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_image_scanner_go_has_runtime_entrypoint():
