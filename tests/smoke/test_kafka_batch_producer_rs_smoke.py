...
SERVICE_PREFIX = "/kafka-batch-producer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kafka-batch-producer-rs"
...
def test_kafka_batch_producer_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kafka-batch-producer-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kafka_batch_producer_rs_has_runtime_entrypoint():
