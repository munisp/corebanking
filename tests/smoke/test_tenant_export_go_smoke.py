...
SERVICE_PREFIX = "/tenant-export"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tenant-export-go"
...
def test_tenant_export_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tenant-export-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tenant_export_go_has_runtime_entrypoint():
