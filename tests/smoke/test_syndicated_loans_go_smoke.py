...
SERVICE_PREFIX = "/syndicated-loans"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "syndicated-loans-go"
...
def test_syndicated_loans_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"syndicated-loans-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_syndicated_loans_go_has_runtime_entrypoint():
