...
SERVICE_PREFIX = "/db-migrations"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "db-migrations"
...
def test_db_migrations_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"db-migrations not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_db_migrations_has_runtime_entrypoint():
