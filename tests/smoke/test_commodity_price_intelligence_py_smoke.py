...
SERVICE_PREFIX = "/commodity-price-intelligence"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "commodity-price-intelligence-py"
...
def test_commodity_price_intelligence_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"commodity-price-intelligence-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_commodity_price_intelligence_py_has_runtime_entrypoint():
