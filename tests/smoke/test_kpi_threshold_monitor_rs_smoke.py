...
SERVICE_PREFIX = "/kpi-threshold-monitor"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kpi-threshold-monitor-rs"
...
def test_kpi_threshold_monitor_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kpi-threshold-monitor-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kpi_threshold_monitor_rs_has_runtime_entrypoint():
