...
SERVICE_PREFIX = "/opensearch-analytics"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "opensearch-analytics-py"
...
def test_opensearch_analytics_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"opensearch-analytics-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_opensearch_analytics_py_has_runtime_entrypoint():
