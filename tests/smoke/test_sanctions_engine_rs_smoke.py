...
SERVICE_PREFIX = "/sanctions-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "sanctions-engine-rs"
...
def test_sanctions_engine_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"sanctions-engine-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_sanctions_engine_rs_has_runtime_entrypoint():
