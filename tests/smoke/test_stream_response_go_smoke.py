...
SERVICE_PREFIX = "/stream-response"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "stream-response-go"
...
def test_stream_response_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"stream-response-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_stream_response_go_has_runtime_entrypoint():
