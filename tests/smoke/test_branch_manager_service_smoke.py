...
SERVICE_PREFIX = "/branch-manager"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "branch-manager-service"
...
def test_branch_manager_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"branch-manager-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_branch_manager_service_has_runtime_entrypoint():
