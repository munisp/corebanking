...
SERVICE_PREFIX = "/voice-biometric-auth"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "voice-biometric-auth-rs"
...
def test_voice_biometric_auth_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"voice-biometric-auth-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_voice_biometric_auth_rs_has_runtime_entrypoint():
