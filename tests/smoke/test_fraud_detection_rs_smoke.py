...
SERVICE_PREFIX = "/fraud-detection"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "fraud-detection-rs"
...
def test_fraud_detection_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"fraud-detection-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_fraud_detection_rs_has_runtime_entrypoint():
