...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "treasury-liquidity-rs"
...
def test_treasury_liquidity_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"treasury-liquidity-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_treasury_liquidity_rs_has_runtime_entrypoint():
