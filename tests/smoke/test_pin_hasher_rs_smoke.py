...
SERVICE_PREFIX = "/pin-hasher"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "pin-hasher-rs"
...
def test_pin_hasher_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"pin-hasher-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_pin_hasher_rs_has_runtime_entrypoint():
