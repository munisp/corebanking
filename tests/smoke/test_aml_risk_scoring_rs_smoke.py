...
SERVICE_PREFIX = "/aml-risk-scoring"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "aml-risk-scoring-rs"
...
def test_aml_risk_scoring_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"aml-risk-scoring-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_aml_risk_scoring_rs_has_runtime_entrypoint():
