...
SERVICE_PREFIX = "/i18n"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "i18n-service-go"
...
def test_i18n_service_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"i18n-service-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_i18n_service_go_has_runtime_entrypoint():
