...
SERVICE_PREFIX = "/fluvio-wasm-transform"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "fluvio-wasm-transform-rs"
...
def test_fluvio_wasm_transform_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"fluvio-wasm-transform-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_fluvio_wasm_transform_rs_has_runtime_entrypoint():
