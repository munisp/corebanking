...
SERVICE_PREFIX = "/materialized-view-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "materialized-view-engine-go"
...
def test_materialized_view_engine_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"materialized-view-engine-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_materialized_view_engine_go_has_runtime_entrypoint():
