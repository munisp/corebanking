...
SERVICE_PREFIX = "/query-cache-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "query-cache-engine-rs"
...
def test_query_cache_engine_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"query-cache-engine-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_query_cache_engine_rs_has_runtime_entrypoint():
