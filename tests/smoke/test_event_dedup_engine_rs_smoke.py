...
SERVICE_PREFIX = "/event-dedup-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "event-dedup-engine-rs"
...
def test_event_dedup_engine_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"event-dedup-engine-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_event_dedup_engine_rs_has_runtime_entrypoint():
