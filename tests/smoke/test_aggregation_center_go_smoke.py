...
SERVICE_PREFIX = "/aggregation-center"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "aggregation-center-go"
...
def test_aggregation_center_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"aggregation-center-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_aggregation_center_go_has_runtime_entrypoint():
