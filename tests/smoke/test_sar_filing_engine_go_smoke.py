...
SERVICE_PREFIX = "/sar-filing-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "sar-filing-engine-go"
...
def test_sar_filing_engine_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"sar-filing-engine-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_sar_filing_engine_go_has_runtime_entrypoint():
