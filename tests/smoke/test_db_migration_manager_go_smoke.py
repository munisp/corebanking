...
SERVICE_PREFIX = "/db-migration-manager"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "db-migration-manager-go"
...
def test_db_migration_manager_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"db-migration-manager-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_db_migration_manager_go_has_runtime_entrypoint():
