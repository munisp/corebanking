...
SERVICE_PREFIX = "/notification-router"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "notification-router-go"
...
def test_notification_router_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"notification-router-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_notification_router_go_has_runtime_entrypoint():
