...
SERVICE_PREFIX = "/kyc-data-quality"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kyc-data-quality-py"
...
def test_kyc_data_quality_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kyc-data-quality-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kyc_data_quality_py_has_runtime_entrypoint():
