...
SERVICE_PREFIX = "/docker-hardener"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "docker-hardener-py"
...
def test_docker_hardener_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"docker-hardener-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_docker_hardener_py_has_runtime_entrypoint():
