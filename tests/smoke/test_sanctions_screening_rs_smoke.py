...
SERVICE_PREFIX = "/sanctions-screening"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "sanctions-screening-rs"
...
def test_sanctions_screening_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"sanctions-screening-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_sanctions_screening_rs_has_runtime_entrypoint():
