...
SERVICE_PREFIX = "/grpc-gateway"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "grpc-gateway-rs"
...
def test_grpc_gateway_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"grpc-gateway-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_grpc_gateway_rs_has_runtime_entrypoint():
