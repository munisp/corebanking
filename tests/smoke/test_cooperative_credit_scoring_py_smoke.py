...
SERVICE_PREFIX = "/cooperative-credit-scoring"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cooperative-credit-scoring-py"
...
def test_cooperative_credit_scoring_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cooperative-credit-scoring-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cooperative_credit_scoring_py_has_runtime_entrypoint():
