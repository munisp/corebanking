...
SERVICE_PREFIX = "/ml-security-service"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ml-security-service"
...
def test_ml_security_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ml-security-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ml_security_service_has_runtime_entrypoint():
