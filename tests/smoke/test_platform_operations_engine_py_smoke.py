...
SERVICE_PREFIX = "/platform-operations-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "platform-operations-engine-py"
...
def test_platform_operations_engine_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"platform-operations-engine-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_platform_operations_engine_py_has_runtime_entrypoint():
