...
SERVICE_PREFIX = "/agent-account-opening"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-account-opening-py"
...
def test_agent_account_opening_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-account-opening-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_account_opening_py_has_runtime_entrypoint():
