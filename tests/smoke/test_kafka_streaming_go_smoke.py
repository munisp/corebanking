...
SERVICE_PREFIX = "/kafka-streaming"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kafka-streaming-go"
...
def test_kafka_streaming_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kafka-streaming-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kafka_streaming_go_has_runtime_entrypoint():
