...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "neo4j-coa-graph-rs"
...
def test_neo4j_coa_graph_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"neo4j-coa-graph-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_neo4j_coa_graph_rs_has_runtime_entrypoint():
