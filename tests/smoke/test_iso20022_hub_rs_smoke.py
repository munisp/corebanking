...
SERVICE_PREFIX = "/iso20022"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "iso20022-hub-rs"
...
def test_iso20022_hub_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"iso20022-hub-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_iso20022_hub_rs_has_runtime_entrypoint():
