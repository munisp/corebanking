...
SERVICE_PREFIX = "/multi-entity"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "multi-entity-go"
...
def test_multi_entity_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"multi-entity-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_multi_entity_go_has_runtime_entrypoint():
