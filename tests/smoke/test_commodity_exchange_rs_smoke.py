...
SERVICE_PREFIX = "/commodity-exchange"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "commodity-exchange-rs"
...
def test_commodity_exchange_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"commodity-exchange-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_commodity_exchange_rs_has_runtime_entrypoint():
