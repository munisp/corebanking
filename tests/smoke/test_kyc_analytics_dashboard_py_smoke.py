...
SERVICE_PREFIX = "/kyc-analytics-dashboard"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kyc-analytics-dashboard-py"
...
def test_kyc_analytics_dashboard_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kyc-analytics-dashboard-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kyc_analytics_dashboard_py_has_runtime_entrypoint():
