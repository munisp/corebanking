...
SERVICE_PREFIX = "/tenant-isolation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tenant-isolation-go"
...
def test_tenant_isolation_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tenant-isolation-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tenant_isolation_go_has_runtime_entrypoint():
