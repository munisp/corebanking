...
SERVICE_PREFIX = "/falkordb-graph"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "falkordb-graph-rs"
...
def test_falkordb_graph_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"falkordb-graph-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_falkordb_graph_rs_has_runtime_entrypoint():
