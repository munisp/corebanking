...
SERVICE_PREFIX = "/face-match"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "face-match-rs"
...
def test_face_match_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"face-match-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_face_match_rs_has_runtime_entrypoint():
