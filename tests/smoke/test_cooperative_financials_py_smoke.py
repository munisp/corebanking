...
SERVICE_PREFIX = "/cooperative-financials"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cooperative-financials-py"
...
def test_cooperative_financials_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cooperative-financials-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cooperative_financials_py_has_runtime_entrypoint():
