...
SERVICE_PREFIX = "/internal-auditor"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "internal-auditor-service"
...
def test_internal_auditor_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"internal-auditor-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_internal_auditor_service_has_runtime_entrypoint():
