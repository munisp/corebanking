...
SERVICE_PREFIX = "/ddos-shield"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ddos-shield-go"
...
def test_ddos_shield_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ddos-shield-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ddos_shield_go_has_runtime_entrypoint():
