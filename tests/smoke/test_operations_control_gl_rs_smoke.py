...
SERVICE_PREFIX = "/operations-control-gl"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "operations-control-gl-rs"
...
def test_operations_control_gl_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"operations-control-gl-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_operations_control_gl_rs_has_runtime_entrypoint():
