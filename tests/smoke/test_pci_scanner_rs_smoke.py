...
SERVICE_PREFIX = "/pci-scanner"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "pci-scanner-rs"
...
def test_pci_scanner_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"pci-scanner-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_pci_scanner_rs_has_runtime_entrypoint():
