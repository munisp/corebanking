...
SERVICE_PREFIX = "/corporate-monitoring"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "corporate-monitoring-go"
...
def test_corporate_monitoring_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"corporate-monitoring-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_corporate_monitoring_go_has_runtime_entrypoint():
