...
SERVICE_PREFIX = "/whatsapp-banking-flows"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "whatsapp-banking-flows-rs"
...
def test_whatsapp_banking_flows_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"whatsapp-banking-flows-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_whatsapp_banking_flows_rs_has_runtime_entrypoint():
