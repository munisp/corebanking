...
SERVICE_PREFIX = "/agri-reinsurance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agri-reinsurance-go"
...
def test_agri_reinsurance_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agri-reinsurance-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agri_reinsurance_go_has_runtime_entrypoint():
