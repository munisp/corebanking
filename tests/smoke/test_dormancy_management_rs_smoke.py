...
SERVICE_PREFIX = "/dormancy"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "dormancy-management-rs"
...
def test_dormancy_management_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"dormancy-management-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_dormancy_management_rs_has_runtime_entrypoint():
