...
SERVICE_PREFIX = "/network-policy-manager"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "network-policy-manager-py"
...
def test_network_policy_manager_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"network-policy-manager-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_network_policy_manager_py_has_runtime_entrypoint():
