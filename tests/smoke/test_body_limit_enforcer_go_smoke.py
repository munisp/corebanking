...
SERVICE_PREFIX = "/body-limit-enforcer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "body-limit-enforcer-go"
...
def test_body_limit_enforcer_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"body-limit-enforcer-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_body_limit_enforcer_go_has_runtime_entrypoint():
