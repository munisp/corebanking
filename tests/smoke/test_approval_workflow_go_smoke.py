...
SERVICE_PREFIX = "/approval-workflow"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "approval-workflow-go"
...
def test_approval_workflow_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"approval-workflow-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_approval_workflow_go_has_runtime_entrypoint():
