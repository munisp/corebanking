...
SERVICE_PREFIX = "/rm"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "relationship-manager-service"
...
def test_relationship_manager_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"relationship-manager-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_relationship_manager_service_has_runtime_entrypoint():
