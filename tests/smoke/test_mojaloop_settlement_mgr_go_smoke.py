...
SERVICE_PREFIX = "/settlement"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "mojaloop-settlement-mgr-go"
...
def test_mojaloop_settlement_mgr_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"mojaloop-settlement-mgr-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_mojaloop_settlement_mgr_go_has_runtime_entrypoint():
