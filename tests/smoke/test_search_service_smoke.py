...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "search-service"
...
def test_search_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"search-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_search_service_has_runtime_entrypoint():
