...
SERVICE_PREFIX = "/agent-reconciliation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-reconciliation-py"
...
def test_agent_reconciliation_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-reconciliation-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_reconciliation_py_has_runtime_entrypoint():
