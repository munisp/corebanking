...
SERVICE_PREFIX = "/address-verification"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "address-verification-py"
...
def test_address_verification_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"address-verification-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_address_verification_py_has_runtime_entrypoint():
