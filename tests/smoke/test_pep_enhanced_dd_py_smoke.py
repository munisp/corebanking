...
SERVICE_PREFIX = "/pep-enhanced-dd"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "pep-enhanced-dd-py"
...
def test_pep_enhanced_dd_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"pep-enhanced-dd-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_pep_enhanced_dd_py_has_runtime_entrypoint():
