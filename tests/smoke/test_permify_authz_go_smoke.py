...
SERVICE_PREFIX = "/permify-authz"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "permify-authz-go"
...
def test_permify_authz_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"permify-authz-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_permify_authz_go_has_runtime_entrypoint():
