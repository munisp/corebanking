...
SERVICE_PREFIX = "/wire-transfer-monitor/v1/wire-transfers/stats"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "wire-transfer-monitor-rs"
...
def test_wire_transfer_monitor_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"wire-transfer-monitor-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_wire_transfer_monitor_rs_has_runtime_entrypoint():
