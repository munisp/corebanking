...
SERVICE_PREFIX = "/cbn-anchor-borrowers"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cbn-anchor-borrowers-go"
...
def test_cbn_anchor_borrowers_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cbn-anchor-borrowers-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cbn_anchor_borrowers_go_has_runtime_entrypoint():
