...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "security-service"
...
def test_security_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"security-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_security_service_has_runtime_entrypoint():
