...
SERVICE_PREFIX = "/agent-farmer-onboarding"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-farmer-onboarding-go"
...
def test_agent_farmer_onboarding_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-farmer-onboarding-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_farmer_onboarding_go_has_runtime_entrypoint():
