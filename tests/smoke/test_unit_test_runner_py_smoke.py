...
SERVICE_PREFIX = "/unit-test-runner"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "unit-test-runner-py"
...
def test_unit_test_runner_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"unit-test-runner-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_unit_test_runner_py_has_runtime_entrypoint():
