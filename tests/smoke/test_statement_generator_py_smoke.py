...
SERVICE_PREFIX = "/statement-generator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "statement-generator-py"
...
def test_statement_generator_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"statement-generator-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_statement_generator_py_has_runtime_entrypoint():
