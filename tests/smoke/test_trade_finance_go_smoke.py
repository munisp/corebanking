...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "trade-finance-go"
...
def test_trade_finance_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"trade-finance-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_trade_finance_go_has_runtime_entrypoint():
