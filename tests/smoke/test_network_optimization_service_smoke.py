...
SERVICE_PREFIX = "/network-optimization"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "network-optimization-service"
...
def test_network_optimization_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"network-optimization-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_network_optimization_service_has_runtime_entrypoint():
