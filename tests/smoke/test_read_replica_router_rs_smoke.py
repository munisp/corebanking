...
SERVICE_PREFIX = "/read-replica-router"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "read-replica-router-rs"
...
def test_read_replica_router_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"read-replica-router-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_read_replica_router_rs_has_runtime_entrypoint():
