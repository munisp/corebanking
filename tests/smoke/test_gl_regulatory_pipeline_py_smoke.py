...
SERVICE_PREFIX = "/gl-regulatory-pipeline"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "gl-regulatory-pipeline-py"
...
def test_gl_regulatory_pipeline_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"gl-regulatory-pipeline-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_gl_regulatory_pipeline_py_has_runtime_entrypoint():
