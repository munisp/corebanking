...
SERVICE_PREFIX = "/audit"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "audit-service"
...
def test_audit_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"audit-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_audit_service_has_runtime_entrypoint():
