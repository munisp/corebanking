...
SERVICE_PREFIX = "/beneficiaries"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "beneficiary-management-go"
...
def test_beneficiary_management_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"beneficiary-management-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_beneficiary_management_go_has_runtime_entrypoint():
