...
SERVICE_PREFIX = "/qdrant-financial-search"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "qdrant-financial-search-go"
...
def test_qdrant_financial_search_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"qdrant-financial-search-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_qdrant_financial_search_go_has_runtime_entrypoint():
