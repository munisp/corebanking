...
SERVICE_PREFIX = "/accessibility-auditor"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "accessibility-auditor-py"
...
def test_accessibility_auditor_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"accessibility-auditor-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_accessibility_auditor_py_has_runtime_entrypoint():
