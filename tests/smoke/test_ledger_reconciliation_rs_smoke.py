...
SERVICE_PREFIX = "/ledger-reconciliation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ledger-reconciliation-rs"
...
def test_ledger_reconciliation_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ledger-reconciliation-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ledger_reconciliation_rs_has_runtime_entrypoint():
