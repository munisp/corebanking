...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "atm-management"
...
def test_atm_management_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"atm-management not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_atm_management_has_runtime_entrypoint():
