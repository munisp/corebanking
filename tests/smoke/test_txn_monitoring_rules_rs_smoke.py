...
SERVICE_PREFIX = "/txn-monitoring-rules"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "txn-monitoring-rules-rs"
...
def test_txn_monitoring_rules_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"txn-monitoring-rules-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_txn_monitoring_rules_rs_has_runtime_entrypoint():
