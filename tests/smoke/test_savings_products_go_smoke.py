...
SERVICE_PREFIX = "/savings-products"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "savings-products-go"
...
def test_savings_products_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"savings-products-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_savings_products_go_has_runtime_entrypoint():
