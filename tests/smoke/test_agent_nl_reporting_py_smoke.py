...
SERVICE_PREFIX = "/agent-nl-reporting"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-nl-reporting-py"
...
def test_agent_nl_reporting_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-nl-reporting-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_nl_reporting_py_has_runtime_entrypoint():
