...
SERVICE_PREFIX = "/error-telemetry"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "error-telemetry-py"
...
def test_error_telemetry_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"error-telemetry-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_error_telemetry_py_has_runtime_entrypoint():
