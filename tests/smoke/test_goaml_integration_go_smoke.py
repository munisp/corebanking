...
SERVICE_PREFIX = "/goaml-integration"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "goaml-integration-go"
...
def test_goaml_integration_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"goaml-integration-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_goaml_integration_go_has_runtime_entrypoint():
