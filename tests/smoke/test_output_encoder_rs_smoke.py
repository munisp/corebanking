...
SERVICE_PREFIX = "/output-encoder"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "output-encoder-rs"
...
def test_output_encoder_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"output-encoder-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_output_encoder_rs_has_runtime_entrypoint():
