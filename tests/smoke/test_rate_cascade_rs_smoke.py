...
SERVICE_PREFIX = "/rate-cascade"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "rate-cascade-rs"
...
def test_rate_cascade_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"rate-cascade-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_rate_cascade_rs_has_runtime_entrypoint():
