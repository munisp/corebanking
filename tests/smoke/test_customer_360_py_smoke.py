...
SERVICE_PREFIX = "/customer-360"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "customer-360-py"
...
def test_customer_360_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"customer-360-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_customer_360_py_has_runtime_entrypoint():
