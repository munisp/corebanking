...
SERVICE_PREFIX = "/cooperative-meetings"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cooperative-meetings-go"
...
def test_cooperative_meetings_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cooperative-meetings-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cooperative_meetings_go_has_runtime_entrypoint():
