...
SERVICE_PREFIX = "/secrets-rotation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "secrets-rotation-rs"
...
def test_secrets_rotation_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"secrets-rotation-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_secrets_rotation_rs_has_runtime_entrypoint():
