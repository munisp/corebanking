...
SERVICE_PREFIX = "/locker"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "locker-go"
...
def test_locker_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"locker-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_locker_go_has_runtime_entrypoint():
