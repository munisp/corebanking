...
SERVICE_PREFIX = "/interest-computation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "interest-computation-rs"
...
def test_interest_computation_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"interest-computation-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_interest_computation_rs_has_runtime_entrypoint():
