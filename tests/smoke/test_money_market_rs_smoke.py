...
SERVICE_PREFIX = "/money-market"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "money-market-rs"
...
def test_money_market_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"money-market-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_money_market_rs_has_runtime_entrypoint():
