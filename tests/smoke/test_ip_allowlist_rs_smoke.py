...
SERVICE_PREFIX = "/ip-allowlist"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ip-allowlist-rs"
...
def test_ip_allowlist_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ip-allowlist-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ip_allowlist_rs_has_runtime_entrypoint():
