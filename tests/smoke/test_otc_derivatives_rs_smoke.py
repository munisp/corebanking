...
SERVICE_PREFIX = "/otc"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "otc-derivatives-rs"
...
def test_otc_derivatives_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"otc-derivatives-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_otc_derivatives_rs_has_runtime_entrypoint():
