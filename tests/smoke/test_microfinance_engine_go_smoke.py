...
SERVICE_PREFIX = "/microfinance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "microfinance-engine-go"
...
def test_microfinance_engine_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"microfinance-engine-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_microfinance_engine_go_has_runtime_entrypoint():
