...
SERVICE_PREFIX = "/video-kyc"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "video-kyc-py"
...
def test_video_kyc_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"video-kyc-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_video_kyc_py_has_runtime_entrypoint():
