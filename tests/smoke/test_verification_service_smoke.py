...
SERVICE_PREFIX = "/verification"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "verification-service"
...
def test_verification_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"verification-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_verification_service_has_runtime_entrypoint():
