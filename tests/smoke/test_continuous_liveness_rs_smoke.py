...
SERVICE_PREFIX = "/continuous-liveness"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "continuous-liveness-rs"
...
def test_continuous_liveness_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"continuous-liveness-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_continuous_liveness_rs_has_runtime_entrypoint():
