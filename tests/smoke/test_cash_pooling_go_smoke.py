...
SERVICE_PREFIX = "/cash-pooling"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cash-pooling-go"
...
def test_cash_pooling_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cash-pooling-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cash_pooling_go_has_runtime_entrypoint():
