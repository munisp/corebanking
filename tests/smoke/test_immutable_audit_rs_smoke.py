...
SERVICE_PREFIX = "/immutable-audit"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "immutable-audit-rs"
...
def test_immutable_audit_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"immutable-audit-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_immutable_audit_rs_has_runtime_entrypoint():
