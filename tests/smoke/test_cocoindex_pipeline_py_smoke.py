...
SERVICE_PREFIX = "/cocoindex-pipeline"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cocoindex-pipeline-py"
...
def test_cocoindex_pipeline_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cocoindex-pipeline-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cocoindex_pipeline_py_has_runtime_entrypoint():
