...
SERVICE_PREFIX = "/supply-chain"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "supply-chain-service"
...
def test_supply_chain_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"supply-chain-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_supply_chain_service_has_runtime_entrypoint():
