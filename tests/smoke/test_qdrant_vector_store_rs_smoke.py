...
SERVICE_PREFIX = "/qdrant-vector-store"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "qdrant-vector-store-rs"
...
def test_qdrant_vector_store_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"qdrant-vector-store-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_qdrant_vector_store_rs_has_runtime_entrypoint():
