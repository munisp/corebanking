...
SERVICE_PREFIX = "/satellite-crop-monitor"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "satellite-crop-monitor-rs"
...
def test_satellite_crop_monitor_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"satellite-crop-monitor-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_satellite_crop_monitor_rs_has_runtime_entrypoint():
