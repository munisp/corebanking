...
SERVICE_PREFIX = "/trade-finance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "trade-finance-service"
...
def test_trade_finance_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"trade-finance-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_trade_finance_service_has_runtime_entrypoint():
