...
SERVICE_PREFIX = "/risk-based-approach"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "risk-based-approach-py"
...
def test_risk_based_approach_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"risk-based-approach-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_risk_based_approach_py_has_runtime_entrypoint():
