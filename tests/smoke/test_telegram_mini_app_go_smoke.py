...
SERVICE_PREFIX = "/telegram-mini-app"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "telegram-mini-app-go"
...
def test_telegram_mini_app_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"telegram-mini-app-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_telegram_mini_app_go_has_runtime_entrypoint():
