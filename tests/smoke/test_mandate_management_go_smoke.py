...
SERVICE_PREFIX = "/mandates"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "mandate-management-go"
...
def test_mandate_management_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"mandate-management-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_mandate_management_go_has_runtime_entrypoint():
