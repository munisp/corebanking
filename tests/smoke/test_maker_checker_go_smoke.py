...
SERVICE_PREFIX = "/approvals"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "maker-checker-go"
...
def test_maker_checker_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"maker-checker-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_maker_checker_go_has_runtime_entrypoint():
