...
SERVICE_PREFIX = "/customer-onboarding"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "customer-onboarding"
...
def test_customer_onboarding_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"customer-onboarding not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_customer_onboarding_has_runtime_entrypoint():
