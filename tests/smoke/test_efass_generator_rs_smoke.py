...
SERVICE_PREFIX = "/efass-generator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "efass-generator-rs"
...
def test_efass_generator_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"efass-generator-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_efass_generator_rs_has_runtime_entrypoint():
