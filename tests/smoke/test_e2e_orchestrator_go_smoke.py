...
SERVICE_PREFIX = "/e2e-orchestrator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "e2e-orchestrator-go"
...
def test_e2e_orchestrator_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"e2e-orchestrator-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_e2e_orchestrator_go_has_runtime_entrypoint():
