...
SERVICE_PREFIX = "/adverse-media-scanner"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "adverse-media-scanner-py"
...
def test_adverse_media_scanner_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"adverse-media-scanner-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_adverse_media_scanner_py_has_runtime_entrypoint():
