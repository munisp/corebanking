...
SERVICE_PREFIX = "/kyc-workflow-orchestration"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kyc-workflow-orchestration-py"
...
def test_kyc_workflow_orchestration_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kyc-workflow-orchestration-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kyc_workflow_orchestration_py_has_runtime_entrypoint():
