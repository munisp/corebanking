...
SERVICE_PREFIX = "/distroless-builder"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "distroless-builder-py"
...
def test_distroless_builder_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"distroless-builder-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_distroless_builder_py_has_runtime_entrypoint():
