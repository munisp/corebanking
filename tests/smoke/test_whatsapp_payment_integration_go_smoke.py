...
SERVICE_PREFIX = "/whatsapp-payment-integration"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "whatsapp-payment-integration-go"
...
def test_whatsapp_payment_integration_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"whatsapp-payment-integration-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_whatsapp_payment_integration_go_has_runtime_entrypoint():
