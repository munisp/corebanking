...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "epr-kgqa-rs"
...
def test_epr_kgqa_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"epr-kgqa-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_epr_kgqa_rs_has_runtime_entrypoint():
