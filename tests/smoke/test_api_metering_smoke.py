...
SERVICE_PREFIX = "/api-metering"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "api-metering"
...
def test_api_metering_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"api-metering not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_api_metering_has_runtime_entrypoint():
