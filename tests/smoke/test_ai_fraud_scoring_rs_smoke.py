...
SERVICE_PREFIX = "/ai-fraud-scoring"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ai-fraud-scoring-rs"
...
def test_ai_fraud_scoring_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ai-fraud-scoring-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ai_fraud_scoring_rs_has_runtime_entrypoint():
