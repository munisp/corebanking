...
SERVICE_PREFIX = "/efass-kyc-returns"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "efass-kyc-returns-py"
...
def test_efass_kyc_returns_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"efass-kyc-returns-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_efass_kyc_returns_py_has_runtime_entrypoint():
