...
SERVICE_PREFIX = "/signature-verification"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "signature-verification-rs"
...
def test_signature_verification_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"signature-verification-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_signature_verification_rs_has_runtime_entrypoint():
