...
SERVICE_PREFIX = "/tenant-management"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tenant-management"
...
def test_tenant_management_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tenant-management not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tenant_management_has_runtime_entrypoint():
