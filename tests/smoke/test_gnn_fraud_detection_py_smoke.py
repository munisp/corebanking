...
SERVICE_PREFIX = "/gnn-fraud-detection"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "gnn-fraud-detection-py"
...
def test_gnn_fraud_detection_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"gnn-fraud-detection-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_gnn_fraud_detection_py_has_runtime_entrypoint():
