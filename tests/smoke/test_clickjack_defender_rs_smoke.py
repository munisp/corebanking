...
SERVICE_PREFIX = "/clickjack-defender"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "clickjack-defender-rs"
...
def test_clickjack_defender_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"clickjack-defender-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_clickjack_defender_rs_has_runtime_entrypoint():
