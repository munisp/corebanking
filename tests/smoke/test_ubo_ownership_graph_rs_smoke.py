...
SERVICE_PREFIX = "/ubo-ownership-graph"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ubo-ownership-graph-rs"
...
def test_ubo_ownership_graph_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ubo-ownership-graph-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ubo_ownership_graph_rs_has_runtime_entrypoint():
