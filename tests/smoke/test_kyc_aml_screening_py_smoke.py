...
SERVICE_PREFIX = "/kyc-aml-screening"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kyc-aml-screening-py"
...
def test_kyc_aml_screening_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kyc-aml-screening-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kyc_aml_screening_py_has_runtime_entrypoint():
