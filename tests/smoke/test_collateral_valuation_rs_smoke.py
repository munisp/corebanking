...
SERVICE_PREFIX = "/collateral"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "collateral-valuation-rs"
...
def test_collateral_valuation_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"collateral-valuation-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_collateral_valuation_rs_has_runtime_entrypoint():
