...
SERVICE_PREFIX = "/parametric-insurance-iot"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "parametric-insurance-iot-rs"
...
def test_parametric_insurance_iot_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"parametric-insurance-iot-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_parametric_insurance_iot_rs_has_runtime_entrypoint():
