...
SERVICE_PREFIX = "/multicurrency"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "multicurrency-revaluation-rs"
...
def test_multicurrency_revaluation_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"multicurrency-revaluation-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_multicurrency_revaluation_rs_has_runtime_entrypoint():
