...
SERVICE_PREFIX = "/prepared-stmt-cache"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "prepared-stmt-cache-go"
...
def test_prepared_stmt_cache_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"prepared-stmt-cache-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_prepared_stmt_cache_go_has_runtime_entrypoint():
