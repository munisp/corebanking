...
SERVICE_PREFIX = "/txn-pattern-analyzer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "txn-pattern-analyzer-py"
...
def test_txn_pattern_analyzer_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"txn-pattern-analyzer-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_txn_pattern_analyzer_py_has_runtime_entrypoint():
