...
SERVICE_PREFIX = "/custody"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "custody-service-go"
...
def test_custody_service_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"custody-service-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_custody_service_go_has_runtime_entrypoint():
