...
SERVICE_PREFIX = "/mcmc-bayesian-risk"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "mcmc-bayesian-risk-py"
...
def test_mcmc_bayesian_risk_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"mcmc-bayesian-risk-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_mcmc_bayesian_risk_py_has_runtime_entrypoint():
