...
SERVICE_PREFIX = "/portfolio"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "portfolio-mgmt-rs"
...
def test_portfolio_mgmt_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"portfolio-mgmt-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_portfolio_mgmt_rs_has_runtime_entrypoint():
