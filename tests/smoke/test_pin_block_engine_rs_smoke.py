...
SERVICE_PREFIX = "/pin-block-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "pin-block-engine-rs"
...
def test_pin_block_engine_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"pin-block-engine-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_pin_block_engine_rs_has_runtime_entrypoint():
