...
SERVICE_PREFIX = "/etd"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "etd-trading-rs"
...
def test_etd_trading_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"etd-trading-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_etd_trading_rs_has_runtime_entrypoint():
