...
SERVICE_PREFIX = "/idempotency"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "idempotency-go"
...
def test_idempotency_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"idempotency-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_idempotency_go_has_runtime_entrypoint():
