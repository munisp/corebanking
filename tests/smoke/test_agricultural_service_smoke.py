...
SERVICE_PREFIX = "/agricultural"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agricultural-service"
...
def test_agricultural_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agricultural-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agricultural_service_has_runtime_entrypoint():
