...
SERVICE_PREFIX = "/siem-exporter"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "siem-exporter-py"
...
def test_siem_exporter_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"siem-exporter-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_siem_exporter_py_has_runtime_entrypoint():
