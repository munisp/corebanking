...
SERVICE_PREFIX = "/cloud-kms-bridge"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cloud-kms-bridge-rs"
...
def test_cloud_kms_bridge_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cloud-kms-bridge-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cloud_kms_bridge_rs_has_runtime_entrypoint():
