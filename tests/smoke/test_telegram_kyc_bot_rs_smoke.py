...
SERVICE_PREFIX = "/telegram-kyc-bot"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "telegram-kyc-bot-rs"
...
def test_telegram_kyc_bot_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"telegram-kyc-bot-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_telegram_kyc_bot_rs_has_runtime_entrypoint():
