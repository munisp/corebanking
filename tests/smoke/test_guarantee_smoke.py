...
SERVICE_PREFIX = "/guarantees"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "guarantee"
...
def test_guarantee_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"guarantee not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_guarantee_has_runtime_entrypoint():
