...
SERVICE_PREFIX = "/hot-data-cache"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "hot-data-cache-rs"
...
def test_hot_data_cache_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"hot-data-cache-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_hot_data_cache_rs_has_runtime_entrypoint():
