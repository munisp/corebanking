...
SERVICE_PREFIX = "/agriculture-banking"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agriculture-banking-rs"
...
def test_agriculture_banking_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agriculture-banking-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agriculture_banking_rs_has_runtime_entrypoint():
