...
SERVICE_PREFIX = "/temporal-access"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "temporal-access-service"
...
def test_temporal_access_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"temporal-access-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_temporal_access_service_has_runtime_entrypoint():
