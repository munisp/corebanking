...
SERVICE_PREFIX = "/card"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "card-service"
...
def test_card_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"card-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_card_service_has_runtime_entrypoint():
