...
SERVICE_PREFIX = "/adaptive-rate-limiter"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "adaptive-rate-limiter-rs"
...
def test_adaptive_rate_limiter_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"adaptive-rate-limiter-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_adaptive_rate_limiter_rs_has_runtime_entrypoint():
