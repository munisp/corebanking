...
SERVICE_PREFIX = "/voice-ivr-menu"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "voice-ivr-menu-go"
...
def test_voice_ivr_menu_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"voice-ivr-menu-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_voice_ivr_menu_go_has_runtime_entrypoint():
