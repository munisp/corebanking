...
SERVICE_PREFIX = "/standing-orders"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "standing-orders-go"
...
def test_standing_orders_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"standing-orders-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_standing_orders_go_has_runtime_entrypoint():
