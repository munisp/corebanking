...
SERVICE_PREFIX = "/agri-input-marketplace"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agri-input-marketplace-go"
...
def test_agri_input_marketplace_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agri-input-marketplace-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agri_input_marketplace_go_has_runtime_entrypoint():
