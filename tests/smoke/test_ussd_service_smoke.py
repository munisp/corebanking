...
SERVICE_PREFIX = "/ussd-service"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ussd-service"
...
def test_ussd_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ussd-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ussd_service_has_runtime_entrypoint():
