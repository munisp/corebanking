...
SERVICE_PREFIX = "/erpnext-bridge"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "erpnext-bridge-go"
...
def test_erpnext_bridge_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"erpnext-bridge-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_erpnext_bridge_go_has_runtime_entrypoint():
