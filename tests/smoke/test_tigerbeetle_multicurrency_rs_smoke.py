...
SERVICE_PREFIX = "/tigerbeetle-multicurrency"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tigerbeetle-multicurrency-rs"
...
def test_tigerbeetle_multicurrency_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tigerbeetle-multicurrency-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tigerbeetle_multicurrency_rs_has_runtime_entrypoint():
