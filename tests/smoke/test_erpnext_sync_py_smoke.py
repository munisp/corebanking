...
SERVICE_PREFIX = "/erpnext-sync"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "erpnext-sync-py"
...
def test_erpnext_sync_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"erpnext-sync-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_erpnext_sync_py_has_runtime_entrypoint():
