...
SERVICE_PREFIX = "/cheque-clearing"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cheque-clearing-go"
...
def test_cheque_clearing_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cheque-clearing-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cheque_clearing_go_has_runtime_entrypoint():
