...
SERVICE_PREFIX = "/ussd-multilingual"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ussd-multilingual-py"
...
def test_ussd_multilingual_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ussd-multilingual-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ussd_multilingual_py_has_runtime_entrypoint():
