...
SERVICE_PREFIX = "/data-intelligence"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "data-intelligence"
...
def test_data_intelligence_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"data-intelligence not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_data_intelligence_has_runtime_entrypoint():
