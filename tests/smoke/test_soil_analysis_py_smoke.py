...
SERVICE_PREFIX = "/soil-analysis"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "soil-analysis-py"
...
def test_soil_analysis_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"soil-analysis-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_soil_analysis_py_has_runtime_entrypoint():
