...
SERVICE_PREFIX = "/feature-flag-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "feature-flag-engine-rs"
...
def test_feature_flag_engine_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"feature-flag-engine-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_feature_flag_engine_rs_has_runtime_entrypoint():
