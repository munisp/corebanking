...
SERVICE_PREFIX = "/virtual-account"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "virtual-account-service"
...
def test_virtual_account_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"virtual-account-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_virtual_account_service_has_runtime_entrypoint():
