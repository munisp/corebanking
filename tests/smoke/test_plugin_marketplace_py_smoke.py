...
SERVICE_PREFIX = "/plugin-marketplace"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "plugin-marketplace-py"
...
def test_plugin_marketplace_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"plugin-marketplace-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_plugin_marketplace_py_has_runtime_entrypoint():
