...
SERVICE_PREFIX = "/ml"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ml-service"
...
def test_ml_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ml-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ml_service_has_runtime_entrypoint():
