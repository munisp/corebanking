...
SERVICE_PREFIX = "/connectivity"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "connectivity-service"
...
def test_connectivity_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"connectivity-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_connectivity_service_has_runtime_entrypoint():
