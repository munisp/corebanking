...
SERVICE_PREFIX = "/realtime-notification-service"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "realtime-notification-service"
...
def test_realtime_notification_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"realtime-notification-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_realtime_notification_service_has_runtime_entrypoint():
