...
SERVICE_PREFIX = "/ollama-inference"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ollama-inference-go"
...
def test_ollama_inference_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ollama-inference-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ollama_inference_go_has_runtime_entrypoint():
