...
SERVICE_PREFIX = "/platform-hardening"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "platform-hardening-rs"
...
def test_platform_hardening_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"platform-hardening-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_platform_hardening_rs_has_runtime_entrypoint():
