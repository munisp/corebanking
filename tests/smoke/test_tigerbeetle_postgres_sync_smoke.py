...
SERVICE_PREFIX = "/tigerbeetle-postgres-sync"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tigerbeetle-postgres-sync"
...
def test_tigerbeetle_postgres_sync_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tigerbeetle-postgres-sync not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tigerbeetle_postgres_sync_has_runtime_entrypoint():
