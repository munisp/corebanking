...
SERVICE_PREFIX = "/billing-enforcement"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "billing-enforcement-rs"
...
def test_billing_enforcement_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"billing-enforcement-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_billing_enforcement_rs_has_runtime_entrypoint():
