...
SERVICE_PREFIX = "/sw-api-cache"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "sw-api-cache-go"
...
def test_sw_api_cache_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"sw-api-cache-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_sw_api_cache_go_has_runtime_entrypoint():
