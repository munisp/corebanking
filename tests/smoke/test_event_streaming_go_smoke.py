...
SERVICE_PREFIX = "/event-streaming"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "event-streaming-go"
...
def test_event_streaming_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"event-streaming-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_event_streaming_go_has_runtime_entrypoint():
