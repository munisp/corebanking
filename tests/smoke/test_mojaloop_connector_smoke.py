...
SERVICE_PREFIX = "/mojaloop-connector"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "mojaloop-connector"
...
def test_mojaloop_connector_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"mojaloop-connector not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_mojaloop_connector_has_runtime_entrypoint():
