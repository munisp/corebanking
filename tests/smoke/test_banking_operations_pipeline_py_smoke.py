...
SERVICE_PREFIX = "/banking-operations-pipeline"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "banking-operations-pipeline-py"
...
def test_banking_operations_pipeline_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"banking-operations-pipeline-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_banking_operations_pipeline_py_has_runtime_entrypoint():
