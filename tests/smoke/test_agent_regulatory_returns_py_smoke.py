...
SERVICE_PREFIX = "/agent-regulatory-returns"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-regulatory-returns-py"
...
def test_agent_regulatory_returns_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-regulatory-returns-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_regulatory_returns_py_has_runtime_entrypoint():
