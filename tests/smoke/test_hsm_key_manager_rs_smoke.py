...
SERVICE_PREFIX = "/hsm-key-manager"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "hsm-key-manager-rs"
...
def test_hsm_key_manager_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"hsm-key-manager-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_hsm_key_manager_rs_has_runtime_entrypoint():
