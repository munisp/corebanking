...
SERVICE_PREFIX = "/batch-processing"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "batch-processing-py"
...
def test_batch_processing_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"batch-processing-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_batch_processing_py_has_runtime_entrypoint():
