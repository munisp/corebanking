...
SERVICE_PREFIX = "/contract-test"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "contract-test-rs"
...
def test_contract_test_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"contract-test-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_contract_test_rs_has_runtime_entrypoint():
