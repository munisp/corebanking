...
SERVICE_PREFIX = "/batch-aggregator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "batch-aggregator-go"
...
def test_batch_aggregator_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"batch-aggregator-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_batch_aggregator_go_has_runtime_entrypoint():
