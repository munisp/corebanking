...
SERVICE_PREFIX = "/livestock-insurance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "livestock-insurance-rs"
...
def test_livestock_insurance_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"livestock-insurance-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_livestock_insurance_rs_has_runtime_entrypoint():
