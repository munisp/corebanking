...
SERVICE_PREFIX = "/payment-rails-connectors"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "payment-rails-connectors"
...
def test_payment_rails_connectors_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"payment-rails-connectors not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_payment_rails_connectors_has_runtime_entrypoint():
