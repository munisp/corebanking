...
SERVICE_PREFIX = "/agent-loan-origination"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-loan-origination-py"
...
def test_agent_loan_origination_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-loan-origination-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_loan_origination_py_has_runtime_entrypoint():
