...
SERVICE_PREFIX = "/typology-detector"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "typology-detector-rs"
...
def test_typology_detector_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"typology-detector-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_typology_detector_rs_has_runtime_entrypoint():
