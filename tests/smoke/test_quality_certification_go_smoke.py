...
SERVICE_PREFIX = "/quality-certification"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "quality-certification-go"
...
def test_quality_certification_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"quality-certification-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_quality_certification_go_has_runtime_entrypoint():
