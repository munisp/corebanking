...
SERVICE_PREFIX = "/connection-pooler"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "connection-pooler-rs"
...
def test_connection_pooler_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"connection-pooler-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_connection_pooler_rs_has_runtime_entrypoint():
