...
SERVICE_PREFIX = "/security-hardening"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "security-hardening-go"
...
def test_security_hardening_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"security-hardening-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_security_hardening_go_has_runtime_entrypoint():
