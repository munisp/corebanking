...
SERVICE_PREFIX = "/reconciliation-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "reconciliation-engine-rs"
...
def test_reconciliation_engine_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"reconciliation-engine-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_reconciliation_engine_rs_has_runtime_entrypoint():
