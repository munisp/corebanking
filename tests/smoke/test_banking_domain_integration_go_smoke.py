...
SERVICE_PREFIX = "/banking-domain-integration"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "banking-domain-integration-go"
...
def test_banking_domain_integration_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"banking-domain-integration-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_banking_domain_integration_go_has_runtime_entrypoint():
