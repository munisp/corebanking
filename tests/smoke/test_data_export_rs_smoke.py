...
SERVICE_PREFIX = "/data-export"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "data-export-rs"
...
def test_data_export_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"data-export-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_data_export_rs_has_runtime_entrypoint():
