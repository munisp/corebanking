...
SERVICE_PREFIX = "/salary"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "salary-processing-go"
...
def test_salary_processing_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"salary-processing-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_salary_processing_go_has_runtime_entrypoint():
