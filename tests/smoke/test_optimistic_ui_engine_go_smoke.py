...
SERVICE_PREFIX = "/optimistic-ui-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "optimistic-ui-engine-go"
...
def test_optimistic_ui_engine_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"optimistic-ui-engine-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_optimistic_ui_engine_go_has_runtime_entrypoint():
