...
SERVICE_PREFIX = "/temporal-orchestrator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "temporal-orchestrator-py"
...
def test_temporal_orchestrator_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"temporal-orchestrator-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_temporal_orchestrator_py_has_runtime_entrypoint():
