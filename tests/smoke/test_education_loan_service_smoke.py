...
SERVICE_PREFIX = "/education-loan"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "education-loan-service"
...
def test_education_loan_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"education-loan-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_education_loan_service_has_runtime_entrypoint():
