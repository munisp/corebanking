...
SERVICE_PREFIX = "/safe-deposit"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "safe-deposit-go"
...
def test_safe_deposit_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"safe-deposit-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_safe_deposit_go_has_runtime_entrypoint():
