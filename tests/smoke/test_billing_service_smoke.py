...
SERVICE_PREFIX = "/billing"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "billing-service"
...
def test_billing_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"billing-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_billing_service_has_runtime_entrypoint():
