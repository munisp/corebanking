...
SERVICE_PREFIX = "/ocr"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ocr-service"
...
def test_ocr_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ocr-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ocr_service_has_runtime_entrypoint():
