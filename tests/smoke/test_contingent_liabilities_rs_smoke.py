...
SERVICE_PREFIX = "/contingent-liabilities"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "contingent-liabilities-rs"
...
def test_contingent_liabilities_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"contingent-liabilities-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_contingent_liabilities_rs_has_runtime_entrypoint():
