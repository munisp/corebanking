...
SERVICE_PREFIX = "/billing-rating"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "billing-rating-rs"
...
def test_billing_rating_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"billing-rating-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_billing_rating_rs_has_runtime_entrypoint():
