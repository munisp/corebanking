...
SERVICE_PREFIX = "/accounting-rules"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "accounting-rules-rs"
...
def test_accounting_rules_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"accounting-rules-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_accounting_rules_rs_has_runtime_entrypoint():
