...
SERVICE_PREFIX = "/fixed-assets"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "fixed-assets-go"
...
def test_fixed_assets_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"fixed-assets-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_fixed_assets_go_has_runtime_entrypoint():
