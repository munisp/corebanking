...
SERVICE_PREFIX = "/developer-portal"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "developer-portal-go"
...
def test_developer_portal_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"developer-portal-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_developer_portal_go_has_runtime_entrypoint():
