...
SERVICE_PREFIX = "/whatsapp-document"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "whatsapp-document-service-rs"
...
def test_whatsapp_document_service_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"whatsapp-document-service-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_whatsapp_document_service_rs_has_runtime_entrypoint():
