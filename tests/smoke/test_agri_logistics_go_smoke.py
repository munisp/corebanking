...
SERVICE_PREFIX = "/agri-logistics"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agri-logistics-go"
...
def test_agri_logistics_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agri-logistics-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agri_logistics_go_has_runtime_entrypoint():
