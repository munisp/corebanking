...
SERVICE_PREFIX = "/event-sourcing"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "event-sourcing-go"
...
def test_event_sourcing_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"event-sourcing-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_event_sourcing_go_has_runtime_entrypoint():
