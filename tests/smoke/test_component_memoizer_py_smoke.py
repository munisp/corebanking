...
SERVICE_PREFIX = "/component-memoizer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "component-memoizer-py"
...
def test_component_memoizer_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"component-memoizer-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_component_memoizer_py_has_runtime_entrypoint():
