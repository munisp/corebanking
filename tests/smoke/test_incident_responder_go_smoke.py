...
SERVICE_PREFIX = "/incident-responder"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "incident-responder-go"
...
def test_incident_responder_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"incident-responder-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_incident_responder_go_has_runtime_entrypoint():
