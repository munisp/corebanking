...
SERVICE_PREFIX = "/equipment-leasing"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "equipment-leasing-go"
...
def test_equipment_leasing_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"equipment-leasing-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_equipment_leasing_go_has_runtime_entrypoint():
