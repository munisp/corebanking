...
SERVICE_PREFIX = "/route-schema-enforcer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "route-schema-enforcer-go"
...
def test_route_schema_enforcer_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"route-schema-enforcer-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_route_schema_enforcer_go_has_runtime_entrypoint():
