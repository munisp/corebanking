...
SERVICE_PREFIX = "/response-compressor"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "response-compressor-rs"
...
def test_response_compressor_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"response-compressor-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_response_compressor_rs_has_runtime_entrypoint():
