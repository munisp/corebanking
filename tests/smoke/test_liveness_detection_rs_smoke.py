...
SERVICE_PREFIX = "/liveness-detection"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "liveness-detection-rs"
...
def test_liveness_detection_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"liveness-detection-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_liveness_detection_rs_has_runtime_entrypoint():
