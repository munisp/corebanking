...
SERVICE_PREFIX = "/credit-facility"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "credit-facility-go"
...
def test_credit_facility_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"credit-facility-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_credit_facility_go_has_runtime_entrypoint():
