...
SERVICE_PREFIX = "/table-partitioner"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "table-partitioner-rs"
...
def test_table_partitioner_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"table-partitioner-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_table_partitioner_rs_has_runtime_entrypoint():
