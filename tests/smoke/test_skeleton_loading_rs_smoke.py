...
SERVICE_PREFIX = "/skeleton-loading"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "skeleton-loading-rs"
...
def test_skeleton_loading_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"skeleton-loading-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_skeleton_loading_rs_has_runtime_entrypoint():
