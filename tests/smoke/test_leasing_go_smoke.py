...
SERVICE_PREFIX = "/leasing"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "leasing-go"
...
def test_leasing_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"leasing-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_leasing_go_has_runtime_entrypoint():
