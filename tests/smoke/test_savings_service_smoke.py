...
SERVICE_PREFIX = "/savings"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "savings-service"
...
def test_savings_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"savings-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_savings_service_has_runtime_entrypoint():
