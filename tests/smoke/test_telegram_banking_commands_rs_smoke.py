...
SERVICE_PREFIX = "/telegram-banking-commands"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "telegram-banking-commands-rs"
...
def test_telegram_banking_commands_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"telegram-banking-commands-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_telegram_banking_commands_rs_has_runtime_entrypoint():
