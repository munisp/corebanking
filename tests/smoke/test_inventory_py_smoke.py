...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "inventory-py"
...
def test_inventory_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"inventory-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_inventory_py_has_runtime_entrypoint():
