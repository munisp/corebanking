...
SERVICE_PREFIX = "/insurance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "insurance-service"
...
def test_insurance_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"insurance-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_insurance_service_has_runtime_entrypoint():
