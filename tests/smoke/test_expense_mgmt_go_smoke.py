...
SERVICE_PREFIX = "/expenses"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "expense-mgmt-go"
...
def test_expense_mgmt_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"expense-mgmt-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_expense_mgmt_go_has_runtime_entrypoint():
