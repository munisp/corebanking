...
SERVICE_PREFIX = "/nirsal-agro-geocoop"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "nirsal-agro-geocoop-go"
...
def test_nirsal_agro_geocoop_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"nirsal-agro-geocoop-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_nirsal_agro_geocoop_go_has_runtime_entrypoint():
