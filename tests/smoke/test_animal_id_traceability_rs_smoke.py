...
SERVICE_PREFIX = "/animal-id-traceability"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "animal-id-traceability-rs"
...
def test_animal_id_traceability_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"animal-id-traceability-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_animal_id_traceability_rs_has_runtime_entrypoint():
