...
SERVICE_PREFIX = "/islamic-banking"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "islamic-banking-service"
...
def test_islamic_banking_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"islamic-banking-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_islamic_banking_service_has_runtime_entrypoint():
