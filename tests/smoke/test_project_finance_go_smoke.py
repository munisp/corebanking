...
SERVICE_PREFIX = "/project-finance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "project-finance-go"
...
def test_project_finance_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"project-finance-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_project_finance_go_has_runtime_entrypoint():
