...
SERVICE_PREFIX = "/payment-hub"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "payment-hub"
...
def test_payment_hub_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"payment-hub not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_payment_hub_has_runtime_entrypoint():
