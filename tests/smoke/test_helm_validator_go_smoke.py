...
SERVICE_PREFIX = "/helm-validator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "helm-validator-go"
...
def test_helm_validator_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"helm-validator-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_helm_validator_go_has_runtime_entrypoint():
