...
SERVICE_PREFIX = "/ifrs9-ecl-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ifrs9-ecl-engine-rs"
...
def test_ifrs9_ecl_engine_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ifrs9-ecl-engine-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ifrs9_ecl_engine_rs_has_runtime_entrypoint():
