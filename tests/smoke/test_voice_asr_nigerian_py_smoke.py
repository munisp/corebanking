...
SERVICE_PREFIX = "/voice-asr-nigerian"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "voice-asr-nigerian-py"
...
def test_voice_asr_nigerian_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"voice-asr-nigerian-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_voice_asr_nigerian_py_has_runtime_entrypoint():
