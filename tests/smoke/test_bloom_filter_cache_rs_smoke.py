...
SERVICE_PREFIX = "/bloom-filter-cache"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "bloom-filter-cache-rs"
...
def test_bloom_filter_cache_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"bloom-filter-cache-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_bloom_filter_cache_rs_has_runtime_entrypoint():
