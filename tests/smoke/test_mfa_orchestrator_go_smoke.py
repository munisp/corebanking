...
SERVICE_PREFIX = "/mfa-orchestrator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "mfa-orchestrator-go"
...
def test_mfa_orchestrator_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"mfa-orchestrator-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_mfa_orchestrator_go_has_runtime_entrypoint():
