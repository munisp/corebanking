...
SERVICE_PREFIX = "/voice-tts-nigerian"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "voice-tts-nigerian-rs"
...
def test_voice_tts_nigerian_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"voice-tts-nigerian-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_voice_tts_nigerian_rs_has_runtime_entrypoint():
