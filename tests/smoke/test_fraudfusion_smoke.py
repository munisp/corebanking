...
SERVICE_PREFIX = "/fraudfusion"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "fraudfusion"
...
def test_fraudfusion_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"fraudfusion not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_fraudfusion_has_runtime_entrypoint():
