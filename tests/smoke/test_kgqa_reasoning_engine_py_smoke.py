...
SERVICE_PREFIX = "/kgqa-reasoning"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kgqa-reasoning-engine-py"
...
def test_kgqa_reasoning_engine_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kgqa-reasoning-engine-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kgqa_reasoning_engine_py_has_runtime_entrypoint():
