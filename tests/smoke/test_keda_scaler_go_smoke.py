...
SERVICE_PREFIX = "/keda-scaler"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "keda-scaler-go"
...
def test_keda_scaler_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"keda-scaler-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_keda_scaler_go_has_runtime_entrypoint():
