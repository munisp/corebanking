...
SERVICE_PREFIX = "/opensearch-indexer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "opensearch-indexer-py"
...
def test_opensearch_indexer_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"opensearch-indexer-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_opensearch_indexer_py_has_runtime_entrypoint():
