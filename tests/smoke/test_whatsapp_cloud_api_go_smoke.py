...
SERVICE_PREFIX = "/whatsapp-cloud-api"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "whatsapp-cloud-api-go"
...
def test_whatsapp_cloud_api_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"whatsapp-cloud-api-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_whatsapp_cloud_api_go_has_runtime_entrypoint():
