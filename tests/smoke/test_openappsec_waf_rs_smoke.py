...
SERVICE_PREFIX = "/openappsec-waf"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "openappsec-waf-rs"
...
def test_openappsec_waf_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"openappsec-waf-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_openappsec_waf_rs_has_runtime_entrypoint():
