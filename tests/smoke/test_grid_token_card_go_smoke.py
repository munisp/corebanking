...
SERVICE_PREFIX = "/grid-token-card"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "grid-token-card-go"
...
def test_grid_token_card_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"grid-token-card-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_grid_token_card_go_has_runtime_entrypoint():
