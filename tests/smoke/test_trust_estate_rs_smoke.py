...
SERVICE_PREFIX = "/trust-estate"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "trust-estate-rs"
...
def test_trust_estate_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"trust-estate-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_trust_estate_rs_has_runtime_entrypoint():
