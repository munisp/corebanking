...
SERVICE_PREFIX = "/biometric"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "biometric-service"
...
def test_biometric_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"biometric-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_biometric_service_has_runtime_entrypoint():
