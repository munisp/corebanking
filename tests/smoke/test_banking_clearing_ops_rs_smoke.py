...
SERVICE_PREFIX = "/banking-clearing-ops"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "banking-clearing-ops-rs"
...
def test_banking_clearing_ops_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"banking-clearing-ops-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_banking_clearing_ops_rs_has_runtime_entrypoint():
