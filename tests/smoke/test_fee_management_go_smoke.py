...
SERVICE_PREFIX = "/fee-management"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "fee-management-go"
...
def test_fee_management_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"fee-management-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_fee_management_go_has_runtime_entrypoint():
