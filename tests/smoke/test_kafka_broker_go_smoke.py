...
SERVICE_PREFIX = "/kafka-broker"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kafka-broker-go"
...
def test_kafka_broker_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kafka-broker-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kafka_broker_go_has_runtime_entrypoint():
