...
SERVICE_PREFIX = "/otel-collector"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "otel-collector-go"
...
def test_otel_collector_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"otel-collector-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_otel_collector_go_has_runtime_entrypoint():
