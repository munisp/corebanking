...
SERVICE_PREFIX = "/sql-parameterizer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "sql-parameterizer-rs"
...
def test_sql_parameterizer_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"sql-parameterizer-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_sql_parameterizer_rs_has_runtime_entrypoint():
