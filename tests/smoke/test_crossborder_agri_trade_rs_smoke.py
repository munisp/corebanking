...
SERVICE_PREFIX = "/crossborder-agri-trade"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "crossborder-agri-trade-rs"
...
def test_crossborder_agri_trade_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"crossborder-agri-trade-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_crossborder_agri_trade_rs_has_runtime_entrypoint():
