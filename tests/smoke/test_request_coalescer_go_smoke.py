...
SERVICE_PREFIX = "/request-coalescer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "request-coalescer-go"
...
def test_request_coalescer_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"request-coalescer-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_request_coalescer_go_has_runtime_entrypoint():
