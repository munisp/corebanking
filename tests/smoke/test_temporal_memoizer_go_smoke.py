...
SERVICE_PREFIX = "/temporal-memoizer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "temporal-memoizer-go"
...
def test_temporal_memoizer_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"temporal-memoizer-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_temporal_memoizer_go_has_runtime_entrypoint():
