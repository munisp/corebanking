...
SERVICE_PREFIX = "/scratch-card-pin"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "scratch-card-pin-go"
...
def test_scratch_card_pin_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"scratch-card-pin-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_scratch_card_pin_go_has_runtime_entrypoint():
