...
SERVICE_PREFIX = "/fraudfusion-ensemble"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "fraudfusion-ensemble-rs"
...
def test_fraudfusion_ensemble_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"fraudfusion-ensemble-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_fraudfusion_ensemble_rs_has_runtime_entrypoint():
