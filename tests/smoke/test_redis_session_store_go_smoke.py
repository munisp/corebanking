...
SERVICE_PREFIX = "/redis-session-store"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "redis-session-store-go"
...
def test_redis_session_store_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"redis-session-store-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_redis_session_store_go_has_runtime_entrypoint():
