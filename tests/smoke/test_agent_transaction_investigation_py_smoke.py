...
SERVICE_PREFIX = "/agent-transaction-investigation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-transaction-investigation-py"
...
def test_agent_transaction_investigation_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-transaction-investigation-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_transaction_investigation_py_has_runtime_entrypoint():
