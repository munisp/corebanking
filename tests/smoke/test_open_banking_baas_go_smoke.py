...
SERVICE_PREFIX = "/open-banking-baas"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "open-banking-baas-go"
...
def test_open_banking_baas_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"open-banking-baas-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_open_banking_baas_go_has_runtime_entrypoint():
