...
SERVICE_PREFIX = "/developers"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "developer-platform-service"
...
def test_developer_platform_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"developer-platform-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_developer_platform_service_has_runtime_entrypoint():
