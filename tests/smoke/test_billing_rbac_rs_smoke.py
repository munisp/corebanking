...
SERVICE_PREFIX = "/billing-rbac"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "billing-rbac-rs"
...
def test_billing_rbac_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"billing-rbac-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_billing_rbac_rs_has_runtime_entrypoint():
