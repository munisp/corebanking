...
SERVICE_PREFIX = "/esusu-groups"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "esusu-groups-go"
...
def test_esusu_groups_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"esusu-groups-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_esusu_groups_go_has_runtime_entrypoint():
