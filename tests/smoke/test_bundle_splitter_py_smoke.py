...
SERVICE_PREFIX = "/bundle-splitter"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "bundle-splitter-py"
...
def test_bundle_splitter_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"bundle-splitter-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_bundle_splitter_py_has_runtime_entrypoint():
