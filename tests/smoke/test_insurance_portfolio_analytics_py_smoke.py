...
SERVICE_PREFIX = "/insurance-portfolio-analytics"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "insurance-portfolio-analytics-py"
...
def test_insurance_portfolio_analytics_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"insurance-portfolio-analytics-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_insurance_portfolio_analytics_py_has_runtime_entrypoint():
