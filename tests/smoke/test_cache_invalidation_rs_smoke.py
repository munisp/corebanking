...
SERVICE_PREFIX = "/cache-invalidation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cache-invalidation-rs"
...
def test_cache_invalidation_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cache-invalidation-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cache_invalidation_rs_has_runtime_entrypoint():
