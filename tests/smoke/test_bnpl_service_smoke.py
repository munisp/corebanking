...
SERVICE_PREFIX = "/bnpl"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "bnpl-service"
...
def test_bnpl_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"bnpl-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_bnpl_service_has_runtime_entrypoint():
