...
SERVICE_PREFIX = "/kafka-lakehouse-connector"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kafka-lakehouse-connector"
...
def test_kafka_lakehouse_connector_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kafka-lakehouse-connector not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kafka_lakehouse_connector_has_runtime_entrypoint():
