...
SERVICE_PREFIX = "/bulk-payments"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "bulk-payments-rs"
...
def test_bulk_payments_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"bulk-payments-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_bulk_payments_rs_has_runtime_entrypoint():
