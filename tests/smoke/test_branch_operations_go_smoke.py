...
SERVICE_PREFIX = "/branch-operations"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "branch-operations-go"
...
def test_branch_operations_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"branch-operations-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_branch_operations_go_has_runtime_entrypoint():
