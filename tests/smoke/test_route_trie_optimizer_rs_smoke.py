...
SERVICE_PREFIX = "/route-trie-optimizer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "route-trie-optimizer-rs"
...
def test_route_trie_optimizer_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"route-trie-optimizer-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_route_trie_optimizer_rs_has_runtime_entrypoint():
