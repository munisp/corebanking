...
SERVICE_PREFIX = "/agri-iot-sensor"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agri-iot-sensor-rs"
...
def test_agri_iot_sensor_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agri-iot-sensor-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agri_iot_sensor_rs_has_runtime_entrypoint():
