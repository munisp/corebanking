...
SERVICE_PREFIX = "/agent-cash-management"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-cash-management-py"
...
def test_agent_cash_management_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-cash-management-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_cash_management_py_has_runtime_entrypoint():
