...
SERVICE_PREFIX = "/security-gateway"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "security-gateway-go"
...
def test_security_gateway_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"security-gateway-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_security_gateway_go_has_runtime_entrypoint():
