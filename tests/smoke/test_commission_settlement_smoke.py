...
SERVICE_PREFIX = "/commission"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "commission-settlement"
...
def test_commission_settlement_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"commission-settlement not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_commission_settlement_has_runtime_entrypoint():
