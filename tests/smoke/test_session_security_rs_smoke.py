...
SERVICE_PREFIX = "/session-security"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "session-security-rs"
...
def test_session_security_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"session-security-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_session_security_rs_has_runtime_entrypoint():
