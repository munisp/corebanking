...
SERVICE_PREFIX = "/billing-ingestor"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "billing-ingestor-go"
...
def test_billing_ingestor_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"billing-ingestor-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_billing_ingestor_go_has_runtime_entrypoint():
