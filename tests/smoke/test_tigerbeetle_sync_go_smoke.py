...
SERVICE_PREFIX = "/tigerbeetle-sync"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tigerbeetle-sync-go"
...
def test_tigerbeetle_sync_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tigerbeetle-sync-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tigerbeetle_sync_go_has_runtime_entrypoint():
