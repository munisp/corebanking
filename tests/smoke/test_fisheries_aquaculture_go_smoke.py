...
SERVICE_PREFIX = "/fisheries-aquaculture"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "fisheries-aquaculture-go"
...
def test_fisheries_aquaculture_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"fisheries-aquaculture-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_fisheries_aquaculture_go_has_runtime_entrypoint():
