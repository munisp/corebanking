...
SERVICE_PREFIX = "/platform-security-infra"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "platform-security-infra-go"
...
def test_platform_security_infra_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"platform-security-infra-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_platform_security_infra_go_has_runtime_entrypoint():
