...
SERVICE_PREFIX = "/regulatory-automation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "regulatory-automation-py"
...
def test_regulatory_automation_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"regulatory-automation-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_regulatory_automation_py_has_runtime_entrypoint():
