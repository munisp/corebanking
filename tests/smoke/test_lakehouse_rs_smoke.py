...
SERVICE_PREFIX = "/lakehouse"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "lakehouse-rs"
...
def test_lakehouse_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"lakehouse-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_lakehouse_rs_has_runtime_entrypoint():
