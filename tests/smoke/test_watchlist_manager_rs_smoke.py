...
SERVICE_PREFIX = "/watchlist-manager"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "watchlist-manager-rs"
...
def test_watchlist_manager_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"watchlist-manager-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_watchlist_manager_rs_has_runtime_entrypoint():
