...
SERVICE_PREFIX = "/field-level-encryption"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "field-level-encryption-rs"
...
def test_field_level_encryption_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"field-level-encryption-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_field_level_encryption_rs_has_runtime_entrypoint():
