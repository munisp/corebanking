...
SERVICE_PREFIX = "/investment"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "investment-service"
...
def test_investment_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"investment-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_investment_service_has_runtime_entrypoint():
