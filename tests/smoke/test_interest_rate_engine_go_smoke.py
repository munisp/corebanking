...
SERVICE_PREFIX = "/interest-rate-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "interest-rate-engine-go"
...
def test_interest_rate_engine_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"interest-rate-engine-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_interest_rate_engine_go_has_runtime_entrypoint():
