...
SERVICE_PREFIX = "/loan-calculator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "loan-calculator-go"
...
def test_loan_calculator_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"loan-calculator-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_loan_calculator_go_has_runtime_entrypoint():
