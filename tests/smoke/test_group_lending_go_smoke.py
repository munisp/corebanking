...
SERVICE_PREFIX = "/group-lending"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "group-lending-go"
...
def test_group_lending_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"group-lending-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_group_lending_go_has_runtime_entrypoint():
