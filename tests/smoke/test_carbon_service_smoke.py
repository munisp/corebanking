...
SERVICE_PREFIX = "/carbon"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "carbon-service"
...
def test_carbon_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"carbon-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_carbon_service_has_runtime_entrypoint():
