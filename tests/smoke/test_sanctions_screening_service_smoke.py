...
SERVICE_PREFIX = "/sanctions"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "sanctions-screening-service"
...
def test_sanctions_screening_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"sanctions-screening-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_sanctions_screening_service_has_runtime_entrypoint():
