...
SERVICE_PREFIX = "/loan-origination"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "loan-origination-go"
...
def test_loan_origination_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"loan-origination-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_loan_origination_go_has_runtime_entrypoint():
