...
SERVICE_PREFIX = "/tls-terminator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tls-terminator-go"
...
def test_tls_terminator_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tls-terminator-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tls_terminator_go_has_runtime_entrypoint():
