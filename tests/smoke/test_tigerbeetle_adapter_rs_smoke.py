...
SERVICE_PREFIX = "/tigerbeetle-adapter"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tigerbeetle-adapter-rs"
...
def test_tigerbeetle_adapter_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tigerbeetle-adapter-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tigerbeetle_adapter_rs_has_runtime_entrypoint():
