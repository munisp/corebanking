...
SERVICE_PREFIX = "/bvn-nin"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "bvn-nin-verification-go"
...
def test_bvn_nin_verification_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"bvn-nin-verification-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_bvn_nin_verification_go_has_runtime_entrypoint():
