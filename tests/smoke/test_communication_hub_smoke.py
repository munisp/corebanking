...
SERVICE_PREFIX = "/communication-hub"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "communication-hub"
...
def test_communication_hub_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"communication-hub not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_communication_hub_has_runtime_entrypoint():
