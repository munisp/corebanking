...
SERVICE_PREFIX = "/post-harvest-loss-tracker"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "post-harvest-loss-tracker-go"
...
def test_post_harvest_loss_tracker_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"post-harvest-loss-tracker-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_post_harvest_loss_tracker_go_has_runtime_entrypoint():
