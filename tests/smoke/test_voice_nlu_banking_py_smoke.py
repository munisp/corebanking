...
SERVICE_PREFIX = "/voice-nlu-banking"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "voice-nlu-banking-py"
...
def test_voice_nlu_banking_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"voice-nlu-banking-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_voice_nlu_banking_py_has_runtime_entrypoint():
