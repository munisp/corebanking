...
SERVICE_PREFIX = "/cbn-agsmeis"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cbn-agsmeis-go"
...
def test_cbn_agsmeis_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cbn-agsmeis-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cbn_agsmeis_go_has_runtime_entrypoint():
