...
SERVICE_PREFIX = "/kyc-event-consumer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kyc-event-consumer-py"
...
def test_kyc_event_consumer_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kyc-event-consumer-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kyc_event_consumer_py_has_runtime_entrypoint():
