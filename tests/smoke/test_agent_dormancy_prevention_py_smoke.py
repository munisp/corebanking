...
SERVICE_PREFIX = "/agent-dormancy-prevention"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-dormancy-prevention-py"
...
def test_agent_dormancy_prevention_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-dormancy-prevention-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_dormancy_prevention_py_has_runtime_entrypoint():
