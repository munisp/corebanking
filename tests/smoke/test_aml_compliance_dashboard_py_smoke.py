...
SERVICE_PREFIX = "/aml-compliance-dashboard"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "aml-compliance-dashboard-py"
...
def test_aml_compliance_dashboard_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"aml-compliance-dashboard-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_aml_compliance_dashboard_py_has_runtime_entrypoint():
