...
SERVICE_PREFIX = "/api-analytics"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "api-analytics-py"
...
def test_api_analytics_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"api-analytics-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_api_analytics_py_has_runtime_entrypoint():
