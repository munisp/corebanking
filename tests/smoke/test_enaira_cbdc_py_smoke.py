...
SERVICE_PREFIX = "/enaira-cbdc"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "enaira-cbdc-py"
...
def test_enaira_cbdc_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"enaira-cbdc-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_enaira_cbdc_py_has_runtime_entrypoint():
