...
SERVICE_PREFIX = "/cbn-tiered-kyc"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cbn-tiered-kyc-rs"
...
def test_cbn_tiered_kyc_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cbn-tiered-kyc-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cbn_tiered_kyc_rs_has_runtime_entrypoint():
