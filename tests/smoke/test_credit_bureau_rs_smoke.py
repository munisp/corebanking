...
SERVICE_PREFIX = "/credit-bureau"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "credit-bureau-rs"
...
def test_credit_bureau_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"credit-bureau-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_credit_bureau_rs_has_runtime_entrypoint():
