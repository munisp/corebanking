...
SERVICE_PREFIX = "/growth-features"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "growth-features-go"
...
def test_growth_features_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"growth-features-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_growth_features_go_has_runtime_entrypoint():
