...
SERVICE_PREFIX = "/event-correlator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "event-correlator-py"
...
def test_event_correlator_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"event-correlator-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_event_correlator_py_has_runtime_entrypoint():
