...
SERVICE_PREFIX = "/soc2-evidence-collector"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "soc2-evidence-collector-py"
...
def test_soc2_evidence_collector_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"soc2-evidence-collector-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_soc2_evidence_collector_py_has_runtime_entrypoint():
