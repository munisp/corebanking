...
SERVICE_PREFIX = "/swift-iso20022"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "swift-iso20022-rs"
...
def test_swift_iso20022_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"swift-iso20022-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_swift_iso20022_rs_has_runtime_entrypoint():
