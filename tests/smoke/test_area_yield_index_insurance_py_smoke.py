...
SERVICE_PREFIX = "/area-yield-index-insurance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "area-yield-index-insurance-py"
...
def test_area_yield_index_insurance_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"area-yield-index-insurance-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_area_yield_index_insurance_py_has_runtime_entrypoint():
