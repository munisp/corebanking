...
SERVICE_PREFIX = "/fraud"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "fraud-service"
...
def test_fraud_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"fraud-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_fraud_service_has_runtime_entrypoint():
