...
def test_voice_banking_seed_exists():
    seed_path = Path(__file__).parent.parent.parent / "services/voice-banking/seed_data.json"
    if not seed_path.exists():
        pytest.skip(f"{seed_path} not found — run in CI or seed first")
    payload = json.loads(seed_path.read_text())
    assert payload["service"] == "voice-banking"
    assert len(payload["records"]) >= 2


def test_voice_banking_service_has_runtime_entrypoint():
    service_dir = Path(__file__).parent.parent.parent / "services/voice-banking"
