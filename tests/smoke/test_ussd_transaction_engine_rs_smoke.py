...
SERVICE_PREFIX = "/ussd-transaction-engine"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ussd-transaction-engine-rs"
...
def test_ussd_transaction_engine_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ussd-transaction-engine-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ussd_transaction_engine_rs_has_runtime_entrypoint():
