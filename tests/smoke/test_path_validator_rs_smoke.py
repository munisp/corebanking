...
SERVICE_PREFIX = "/path-validator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "path-validator-rs"
...
def test_path_validator_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"path-validator-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_path_validator_rs_has_runtime_entrypoint():
