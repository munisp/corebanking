...
SERVICE_PREFIX = "/beneficial-ownership"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "beneficial-ownership-go"
...
def test_beneficial_ownership_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"beneficial-ownership-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_beneficial_ownership_go_has_runtime_entrypoint():
