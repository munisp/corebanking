...
SERVICE_PREFIX = "/eod"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "eod-processor-go"
...
def test_eod_processor_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"eod-processor-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_eod_processor_go_has_runtime_entrypoint():
