...
SERVICE_PREFIX = "/tenant-provisioning-py"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tenant-provisioning-py"
...
def test_tenant_provisioning_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tenant-provisioning-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tenant_provisioning_py_has_runtime_entrypoint():
