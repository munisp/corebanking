...
SERVICE_PREFIX = "/erp"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "erpnext-integration-service"
...
def test_erpnext_integration_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"erpnext-integration-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_erpnext_integration_service_has_runtime_entrypoint():
