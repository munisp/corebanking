...
SERVICE_PREFIX = "/multi-bureau-verification"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "multi-bureau-verification-go"
...
def test_multi_bureau_verification_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"multi-bureau-verification-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_multi_bureau_verification_go_has_runtime_entrypoint():
