...
SERVICE_PREFIX = "/apisix-gateway"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "apisix-gateway-go"
...
def test_apisix_gateway_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"apisix-gateway-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_apisix_gateway_go_has_runtime_entrypoint():
