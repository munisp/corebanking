...
SERVICE_PREFIX = "/browser-fingerprint"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "browser-fingerprint-go"
...
def test_browser_fingerprint_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"browser-fingerprint-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_browser_fingerprint_go_has_runtime_entrypoint():
