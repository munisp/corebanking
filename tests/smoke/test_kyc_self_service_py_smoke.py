...
SERVICE_PREFIX = "/kyc-self"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kyc-self-service-py"
...
def test_kyc_self_service_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kyc-self-service-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kyc_self_service_py_has_runtime_entrypoint():
