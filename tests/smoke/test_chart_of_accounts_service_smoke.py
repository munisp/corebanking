...
SERVICE_PREFIX = "/chart-of-accounts"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "chart-of-accounts-service"
...
def test_chart_of_accounts_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"chart-of-accounts-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_chart_of_accounts_service_has_runtime_entrypoint():
