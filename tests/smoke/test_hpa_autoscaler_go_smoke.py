...
SERVICE_PREFIX = "/hpa-autoscaler"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "hpa-autoscaler-go"
...
def test_hpa_autoscaler_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"hpa-autoscaler-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_hpa_autoscaler_go_has_runtime_entrypoint():
