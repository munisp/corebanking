...
SERVICE_PREFIX = "/credit-scoring"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "credit-scoring-py"
...
def test_credit_scoring_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"credit-scoring-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_credit_scoring_py_has_runtime_entrypoint():
