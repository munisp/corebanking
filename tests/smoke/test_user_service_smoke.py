...
SERVICE_PREFIX = "/user"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "user-service"
...
def test_user_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"user-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_user_service_has_runtime_entrypoint():
