...
SERVICE_PREFIX = "/debt-collection"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "debt-collection-go"
...
def test_debt_collection_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"debt-collection-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_debt_collection_go_has_runtime_entrypoint():
