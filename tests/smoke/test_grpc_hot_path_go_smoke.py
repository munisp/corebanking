...
SERVICE_PREFIX = "/grpc-hot-path"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "grpc-hot-path-go"
...
def test_grpc_hot_path_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"grpc-hot-path-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_grpc_hot_path_go_has_runtime_entrypoint():
