...
SERVICE_PREFIX = "/agent-kyc-capture"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-kyc-capture-go"
...
def test_agent_kyc_capture_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-kyc-capture-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_kyc_capture_go_has_runtime_entrypoint():
