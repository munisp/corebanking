...
SERVICE_PREFIX = "/ctr"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "nfiu-ctr-str-filing-py"
...
def test_nfiu_ctr_str_filing_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"nfiu-ctr-str-filing-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_nfiu_ctr_str_filing_py_has_runtime_entrypoint():
