...
SERVICE_PREFIX = "/utility-payments"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "utility-payments-go"
...
def test_utility_payments_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"utility-payments-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_utility_payments_go_has_runtime_entrypoint():
