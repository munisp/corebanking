...
SERVICE_PREFIX = "/sorted-set-ranking"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "sorted-set-ranking-go"
...
def test_sorted_set_ranking_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"sorted-set-ranking-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_sorted_set_ranking_go_has_runtime_entrypoint():
