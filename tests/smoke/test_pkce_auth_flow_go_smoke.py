...
SERVICE_PREFIX = "/pkce-auth-flow"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "pkce-auth-flow-go"
...
def test_pkce_auth_flow_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"pkce-auth-flow-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_pkce_auth_flow_go_has_runtime_entrypoint():
