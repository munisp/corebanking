...
SERVICE_PREFIX = "/anomaly-detector"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "anomaly-detector-py"
...
def test_anomaly_detector_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"anomaly-detector-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_anomaly_detector_py_has_runtime_entrypoint():
