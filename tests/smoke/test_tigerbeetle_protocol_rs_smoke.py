...
SERVICE_PREFIX = "/tigerbeetle-protocol"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tigerbeetle-protocol-rs"
...
def test_tigerbeetle_protocol_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tigerbeetle-protocol-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tigerbeetle_protocol_rs_has_runtime_entrypoint():
