...
SERVICE_PREFIX = "/payment-processing"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "payment-processing-service"
...
def test_payment_processing_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"payment-processing-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_payment_processing_service_has_runtime_entrypoint():
