...
SERVICE_PREFIX = "/graduated-rollout"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "graduated-rollout-rs"
...
def test_graduated_rollout_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"graduated-rollout-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_graduated_rollout_rs_has_runtime_entrypoint():
