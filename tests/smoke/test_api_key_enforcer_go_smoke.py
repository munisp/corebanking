...
SERVICE_PREFIX = "/api-key-enforcer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "api-key-enforcer-go"
...
def test_api_key_enforcer_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"api-key-enforcer-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_api_key_enforcer_go_has_runtime_entrypoint():
