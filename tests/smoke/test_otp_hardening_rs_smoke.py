...
SERVICE_PREFIX = "/otp-hardening"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "otp-hardening-rs"
...
def test_otp_hardening_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"otp-hardening-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_otp_hardening_rs_has_runtime_entrypoint():
