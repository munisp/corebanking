...
SERVICE_PREFIX = "/farm-boundary-mapping"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "farm-boundary-mapping-rs"
...
def test_farm_boundary_mapping_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"farm-boundary-mapping-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_farm_boundary_mapping_rs_has_runtime_entrypoint():
