...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "whatsapp-payment-integration"
...
def test_whatsapp_payment_integration_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"whatsapp-payment-integration not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_whatsapp_payment_integration_has_runtime_entrypoint():
