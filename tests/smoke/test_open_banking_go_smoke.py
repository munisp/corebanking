...
SERVICE_PREFIX = "/open-banking"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "open-banking-go"
...
def test_open_banking_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"open-banking-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_open_banking_go_has_runtime_entrypoint():
