...
SERVICE_PREFIX = "/treasury-liquidity"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "treasury-liquidity-py"
...
def test_treasury_liquidity_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"treasury-liquidity-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_treasury_liquidity_py_has_runtime_entrypoint():
