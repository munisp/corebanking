...
SERVICE_PREFIX = "/changelog-generator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "changelog-generator-py"
...
def test_changelog_generator_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"changelog-generator-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_changelog_generator_py_has_runtime_entrypoint():
