...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "chatbot-service"
...
def test_chatbot_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"chatbot-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_chatbot_service_has_runtime_entrypoint():
