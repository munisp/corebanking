...
SERVICE_PREFIX = "/sms-alert-notification"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "sms-alert-notification-py"
...
def test_sms_alert_notification_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"sms-alert-notification-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_sms_alert_notification_py_has_runtime_entrypoint():
