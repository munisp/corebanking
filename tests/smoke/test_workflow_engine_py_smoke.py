...
SERVICE_PREFIX = "/workflows"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "workflow-engine-py"
...
def test_workflow_engine_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"workflow-engine-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_workflow_engine_py_has_runtime_entrypoint():
