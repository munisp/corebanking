...
SERVICE_PREFIX = "/identity-verification"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "identity-verification-service"
...
def test_identity_verification_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"identity-verification-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_identity_verification_service_has_runtime_entrypoint():
