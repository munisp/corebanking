...
SERVICE_PREFIX = "/dapr-sidecar"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "dapr-sidecar-go"
...
def test_dapr_sidecar_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"dapr-sidecar-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_dapr_sidecar_go_has_runtime_entrypoint():
