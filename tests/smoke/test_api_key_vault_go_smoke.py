...
SERVICE_PREFIX = "/api-key-vault"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "api-key-vault-go"
...
def test_api_key_vault_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"api-key-vault-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_api_key_vault_go_has_runtime_entrypoint():
