...
SERVICE_PREFIX = "/tax"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tax-reporting-py"
...
def test_tax_reporting_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tax-reporting-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tax_reporting_py_has_runtime_entrypoint():
