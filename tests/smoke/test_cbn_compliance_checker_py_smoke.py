...
SERVICE_PREFIX = "/cbn-compliance-checker"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cbn-compliance-checker-py"
...
def test_cbn_compliance_checker_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cbn-compliance-checker-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cbn_compliance_checker_py_has_runtime_entrypoint():
