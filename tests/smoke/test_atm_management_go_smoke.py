...
SERVICE_PREFIX = "/atm-management"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "atm-management-go"
...
def test_atm_management_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"atm-management-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_atm_management_go_has_runtime_entrypoint():
