...
SERVICE_PREFIX = "/bank-guarantees"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "bank-guarantees-go"
...
def test_bank_guarantees_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"bank-guarantees-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_bank_guarantees_go_has_runtime_entrypoint():
