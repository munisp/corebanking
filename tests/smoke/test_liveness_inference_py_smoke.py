...
SERVICE_PREFIX = "/liveness-inference"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "liveness-inference-py"
...
def test_liveness_inference_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"liveness-inference-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_liveness_inference_py_has_runtime_entrypoint():
