...
SERVICE_PREFIX = "/billing-analytics"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "billing-analytics-py"
...
def test_billing_analytics_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"billing-analytics-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_billing_analytics_py_has_runtime_entrypoint():
