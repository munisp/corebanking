...
SERVICE_PREFIX = "/ndpr-compliance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ndpr-compliance-py"
...
def test_ndpr_compliance_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ndpr-compliance-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ndpr_compliance_py_has_runtime_entrypoint():
