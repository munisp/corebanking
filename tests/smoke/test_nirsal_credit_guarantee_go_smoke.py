...
SERVICE_PREFIX = "/nirsal-credit-guarantee"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "nirsal-credit-guarantee-go"
...
def test_nirsal_credit_guarantee_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"nirsal-credit-guarantee-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_nirsal_credit_guarantee_go_has_runtime_entrypoint():
