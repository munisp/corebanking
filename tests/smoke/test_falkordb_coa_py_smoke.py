...
SERVICE_PREFIX = "/falkordb-coa"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "falkordb-coa-py"
...
def test_falkordb_coa_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"falkordb-coa-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_falkordb_coa_py_has_runtime_entrypoint():
