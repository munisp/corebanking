...
SERVICE_PREFIX = "/mobile-bff"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "mobile-bff"
...
def test_mobile_bff_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"mobile-bff not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_mobile_bff_has_runtime_entrypoint():
