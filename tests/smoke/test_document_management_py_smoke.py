...
SERVICE_PREFIX = "/document-management"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "document-management-py"
...
def test_document_management_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"document-management-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_document_management_py_has_runtime_entrypoint():
