...
SERVICE_PREFIX = "/regulatory-reporting"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "regulatory-reporting-go"
...
def test_regulatory_reporting_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"regulatory-reporting-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_regulatory_reporting_go_has_runtime_entrypoint():
