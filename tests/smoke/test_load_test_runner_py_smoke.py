...
SERVICE_PREFIX = "/load-test-runner"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "load-test-runner-py"
...
def test_load_test_runner_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"load-test-runner-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_load_test_runner_py_has_runtime_entrypoint():
