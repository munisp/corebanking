...
SERVICE_PREFIX = "/interbank"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "interbank-lending-rs"
...
def test_interbank_lending_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"interbank-lending-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_interbank_lending_rs_has_runtime_entrypoint():
