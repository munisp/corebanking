...
SERVICE_PREFIX = "/ussd-gateway-service"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ussd-gateway-service"
...
def test_ussd_gateway_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ussd-gateway-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ussd_gateway_service_has_runtime_entrypoint():
