...
SERVICE_PREFIX = "/regulatory-sandbox"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "regulatory-sandbox-go"
...
def test_regulatory_sandbox_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"regulatory-sandbox-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_regulatory_sandbox_go_has_runtime_entrypoint():
