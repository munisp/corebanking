...
SERVICE_PREFIX = "/api-versioning"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "api-versioning-go"
...
def test_api_versioning_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"api-versioning-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_api_versioning_go_has_runtime_entrypoint():
