...
SERVICE_PREFIX = "/event-bus"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "event-bus-go"
...
def test_event_bus_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"event-bus-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_event_bus_go_has_runtime_entrypoint():
