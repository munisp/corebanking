...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "realtime-pricing-rs"
...
def test_realtime_pricing_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"realtime-pricing-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_realtime_pricing_rs_has_runtime_entrypoint():
