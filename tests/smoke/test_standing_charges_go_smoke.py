...
SERVICE_PREFIX = "/standing-charges"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "standing-charges-go"
...
def test_standing_charges_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"standing-charges-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_standing_charges_go_has_runtime_entrypoint():
