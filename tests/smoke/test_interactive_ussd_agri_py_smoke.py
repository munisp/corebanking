...
SERVICE_PREFIX = "/interactive-ussd-agri"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "interactive-ussd-agri-py"
...
def test_interactive_ussd_agri_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"interactive-ussd-agri-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_interactive_ussd_agri_py_has_runtime_entrypoint():
