...
SERVICE_PREFIX = "/liveness-orchestrator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "liveness-orchestrator-go"
...
def test_liveness_orchestrator_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"liveness-orchestrator-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_liveness_orchestrator_go_has_runtime_entrypoint():
