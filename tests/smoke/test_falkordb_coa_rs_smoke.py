...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "falkordb-coa-rs"
...
def test_falkordb_coa_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"falkordb-coa-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_falkordb_coa_rs_has_runtime_entrypoint():
