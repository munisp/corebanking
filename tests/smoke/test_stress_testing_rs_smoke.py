...
SERVICE_PREFIX = "/stress-testing"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "stress-testing-rs"
...
def test_stress_testing_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"stress-testing-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_stress_testing_rs_has_runtime_entrypoint():
