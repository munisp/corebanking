...
SERVICE_PREFIX = "/card-management"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "card-management-go"
...
def test_card_management_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"card-management-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_card_management_go_has_runtime_entrypoint():
