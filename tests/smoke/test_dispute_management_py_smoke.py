...
SERVICE_PREFIX = "/dispute-management"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "dispute-management-py"
...
def test_dispute_management_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"dispute-management-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_dispute_management_py_has_runtime_entrypoint():
