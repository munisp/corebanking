...
SERVICE_PREFIX = "/nibss-direct-debit"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "nibss-direct-debit-go"
...
def test_nibss_direct_debit_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"nibss-direct-debit-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_nibss_direct_debit_go_has_runtime_entrypoint():
