...
SERVICE_PREFIX = "/lcr-nsfr"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "lcr-nsfr-rs"
...
def test_lcr_nsfr_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"lcr-nsfr-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_lcr_nsfr_rs_has_runtime_entrypoint():
