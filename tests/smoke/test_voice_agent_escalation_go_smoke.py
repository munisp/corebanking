...
SERVICE_PREFIX = "/voice-agent-escalation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "voice-agent-escalation-go"
...
def test_voice_agent_escalation_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"voice-agent-escalation-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_voice_agent_escalation_go_has_runtime_entrypoint():
