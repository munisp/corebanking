...
SERVICE_PREFIX = "/lakehouse-etl"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "lakehouse-etl-py"
...
def test_lakehouse_etl_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"lakehouse-etl-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_lakehouse_etl_py_has_runtime_entrypoint():
