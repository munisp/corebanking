...
SERVICE_PREFIX = "/opensearch-optimizer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "opensearch-optimizer-py"
...
def test_opensearch_optimizer_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"opensearch-optimizer-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_opensearch_optimizer_py_has_runtime_entrypoint():
