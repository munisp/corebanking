...
SERVICE_PREFIX = "/auth-enforcer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "auth-enforcer-rs"
...
def test_auth_enforcer_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"auth-enforcer-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_auth_enforcer_rs_has_runtime_entrypoint():
