...
SERVICE_PREFIX = "/ddos-protection"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ddos-protection-go"
...
def test_ddos_protection_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ddos-protection-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ddos_protection_go_has_runtime_entrypoint():
