...
SERVICE_PREFIX = "/telegram-notification"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "telegram-notification-py"
...
def test_telegram_notification_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"telegram-notification-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_telegram_notification_py_has_runtime_entrypoint():
