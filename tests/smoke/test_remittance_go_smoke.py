...
SERVICE_PREFIX = "/remittance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "remittance-go"
...
def test_remittance_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"remittance-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_remittance_go_has_runtime_entrypoint():
