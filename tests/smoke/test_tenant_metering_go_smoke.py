...
SERVICE_PREFIX = "/tenant-metering"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tenant-metering-go"
...
def test_tenant_metering_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tenant-metering-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tenant_metering_go_has_runtime_entrypoint():
