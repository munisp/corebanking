...
SERVICE_PREFIX = "/livestock-management"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "livestock-management-rs"
...
def test_livestock_management_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"livestock-management-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_livestock_management_rs_has_runtime_entrypoint():
