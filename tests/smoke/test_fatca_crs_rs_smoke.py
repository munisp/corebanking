...
SERVICE_PREFIX = "/fatca-crs"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "fatca-crs-rs"
...
def test_fatca_crs_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"fatca-crs-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_fatca_crs_rs_has_runtime_entrypoint():
