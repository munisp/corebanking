...
SERVICE_PREFIX = "/stk"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "stk-service"
...
def test_stk_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"stk-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_stk_service_has_runtime_entrypoint():
