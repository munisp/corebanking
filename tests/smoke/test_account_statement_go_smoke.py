...
SERVICE_PREFIX = "/account-statement"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "account-statement-go"
...
def test_account_statement_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"account-statement-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_account_statement_go_has_runtime_entrypoint():
