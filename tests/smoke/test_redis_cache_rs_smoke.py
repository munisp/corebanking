...
SERVICE_PREFIX = "/redis-cache"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "redis-cache-rs"
...
def test_redis_cache_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"redis-cache-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_redis_cache_rs_has_runtime_entrypoint():
