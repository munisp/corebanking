...
SERVICE_PREFIX = "/pension"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "pension-py"
...
def test_pension_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"pension-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_pension_py_has_runtime_entrypoint():
