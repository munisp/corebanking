...
SERVICE_PREFIX = "/livestock-finance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "livestock-finance-rs"
...
def test_livestock_finance_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"livestock-finance-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_livestock_finance_rs_has_runtime_entrypoint():
