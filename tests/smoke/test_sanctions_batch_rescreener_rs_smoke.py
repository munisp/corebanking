...
SERVICE_PREFIX = "/sanctions-batch-rescreener"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "sanctions-batch-rescreener-rs"
...
def test_sanctions_batch_rescreener_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"sanctions-batch-rescreener-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_sanctions_batch_rescreener_rs_has_runtime_entrypoint():
