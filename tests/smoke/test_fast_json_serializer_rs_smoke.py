...
SERVICE_PREFIX = "/fast-json-serializer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "fast-json-serializer-rs"
...
def test_fast_json_serializer_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"fast-json-serializer-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_fast_json_serializer_rs_has_runtime_entrypoint():
