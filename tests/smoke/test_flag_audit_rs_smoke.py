...
SERVICE_PREFIX = "/flag-audit"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "flag-audit-rs"
...
def test_flag_audit_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"flag-audit-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_flag_audit_rs_has_runtime_entrypoint():
