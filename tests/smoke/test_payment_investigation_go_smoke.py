...
SERVICE_PREFIX = "/payment-investigation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "payment-investigation-go"
...
def test_payment_investigation_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"payment-investigation-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_payment_investigation_go_has_runtime_entrypoint():
