...
SERVICE_PREFIX = "/diaspora"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "diaspora-banking-py"
...
def test_diaspora_banking_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"diaspora-banking-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_diaspora_banking_py_has_runtime_entrypoint():
