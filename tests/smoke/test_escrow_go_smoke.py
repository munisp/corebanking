...
SERVICE_PREFIX = "/escrow/v1"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "escrow-go"
...
def test_escrow_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"escrow-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_escrow_go_has_runtime_entrypoint():
