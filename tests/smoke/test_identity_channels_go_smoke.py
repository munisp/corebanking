...
SERVICE_PREFIX = "/identity-channels"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "identity-channels-go"
...
def test_identity_channels_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"identity-channels-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_identity_channels_go_has_runtime_entrypoint():
