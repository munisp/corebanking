...
SERVICE_PREFIX = "/exchange-rate-service"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "exchange-rate-service"
...
def test_exchange_rate_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"exchange-rate-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_exchange_rate_service_has_runtime_entrypoint():
