...
SERVICE_PREFIX = "/agent-fraud-detection"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agent-fraud-detection-py"
...
def test_agent_fraud_detection_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agent-fraud-detection-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agent_fraud_detection_py_has_runtime_entrypoint():
