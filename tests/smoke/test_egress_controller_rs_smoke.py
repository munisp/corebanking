...
SERVICE_PREFIX = "/egress-controller"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "egress-controller-rs"
...
def test_egress_controller_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"egress-controller-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_egress_controller_rs_has_runtime_entrypoint():
