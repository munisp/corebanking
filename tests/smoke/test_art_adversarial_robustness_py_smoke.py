...
SERVICE_PREFIX = "/art-adversarial-robustness"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "art-adversarial-robustness-py"
...
def test_art_adversarial_robustness_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"art-adversarial-robustness-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_art_adversarial_robustness_py_has_runtime_entrypoint():
