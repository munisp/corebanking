...
SERVICE_PREFIX = "/token-rotation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "token-rotation-rs"
...
def test_token_rotation_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"token-rotation-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_token_rotation_rs_has_runtime_entrypoint():
