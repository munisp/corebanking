...
SERVICE_PREFIX = "/keepalive-tuner"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "keepalive-tuner-rs"
...
def test_keepalive_tuner_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"keepalive-tuner-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_keepalive_tuner_rs_has_runtime_entrypoint():
