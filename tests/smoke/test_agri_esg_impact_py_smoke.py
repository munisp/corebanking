...
SERVICE_PREFIX = "/agri-esg-impact"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agri-esg-impact-py"
...
def test_agri_esg_impact_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agri-esg-impact-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agri_esg_impact_py_has_runtime_entrypoint():
