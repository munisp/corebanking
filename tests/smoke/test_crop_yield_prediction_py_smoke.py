...
SERVICE_PREFIX = "/crop-yield-prediction"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "crop-yield-prediction-py"
...
def test_crop_yield_prediction_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"crop-yield-prediction-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_crop_yield_prediction_py_has_runtime_entrypoint():
