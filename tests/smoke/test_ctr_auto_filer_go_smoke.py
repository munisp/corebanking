...
SERVICE_PREFIX = "/ctr-auto-filer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ctr-auto-filer-go"
...
def test_ctr_auto_filer_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ctr-auto-filer-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ctr_auto_filer_go_has_runtime_entrypoint():
