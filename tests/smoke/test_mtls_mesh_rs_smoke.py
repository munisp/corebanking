...
SERVICE_PREFIX = "/mtls-mesh"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "mtls-mesh-rs"
...
def test_mtls_mesh_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"mtls-mesh-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_mtls_mesh_rs_has_runtime_entrypoint():
