...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "middleware-go"
...
def test_middleware_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"middleware-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_middleware_go_has_runtime_entrypoint():
