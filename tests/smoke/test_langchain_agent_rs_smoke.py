...
SERVICE_PREFIX = "/langchain-agent"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "langchain-agent-rs"
...
def test_langchain_agent_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"langchain-agent-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_langchain_agent_rs_has_runtime_entrypoint():
