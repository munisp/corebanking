...
SERVICE_PREFIX = "/saga-coordinator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "saga-coordinator-py"
...
def test_saga_coordinator_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"saga-coordinator-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_saga_coordinator_py_has_runtime_entrypoint():
