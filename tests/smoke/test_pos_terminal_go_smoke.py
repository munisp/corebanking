...
SERVICE_PREFIX = "/pos"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "pos-terminal-go"
...
def test_pos_terminal_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"pos-terminal-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_pos_terminal_go_has_runtime_entrypoint():
