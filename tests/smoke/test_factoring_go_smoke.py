...
SERVICE_PREFIX = "/factoring"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "factoring-go"
...
def test_factoring_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"factoring-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_factoring_go_has_runtime_entrypoint():
