...
SERVICE_PREFIX = "/tenant-ratelimit"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "tenant-ratelimit-rs"
...
def test_tenant_ratelimit_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"tenant-ratelimit-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_tenant_ratelimit_rs_has_runtime_entrypoint():
