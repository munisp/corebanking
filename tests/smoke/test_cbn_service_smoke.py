...
SERVICE_PREFIX = "/cbn"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cbn-service"
...
def test_cbn_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cbn-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cbn_service_has_runtime_entrypoint():
