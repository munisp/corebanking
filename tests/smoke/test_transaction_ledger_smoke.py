...
SERVICE_PREFIX = "/ledger"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "transaction-ledger"
...
def test_transaction_ledger_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"transaction-ledger not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_transaction_ledger_has_runtime_entrypoint():
