...
SERVICE_PREFIX = "/virtual-accounts"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "virtual-accounts-go"
...
def test_virtual_accounts_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"virtual-accounts-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_virtual_accounts_go_has_runtime_entrypoint():
