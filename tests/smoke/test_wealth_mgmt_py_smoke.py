...
SERVICE_PREFIX = "/wealth"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "wealth-mgmt-py"
...
def test_wealth_mgmt_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"wealth-mgmt-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_wealth_mgmt_py_has_runtime_entrypoint():
