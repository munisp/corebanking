...
SERVICE_PREFIX = "/temporal-sagas"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "temporal-sagas-go"
...
def test_temporal_sagas_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"temporal-sagas-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_temporal_sagas_go_has_runtime_entrypoint():
