...
SERVICE_PREFIX = "/vault-integration"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "vault-integration-rs"
...
def test_vault_integration_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"vault-integration-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_vault_integration_rs_has_runtime_entrypoint():
