...
SERVICE_PREFIX = "/acgsf-guarantee"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "acgsf-guarantee-go"
...
def test_acgsf_guarantee_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"acgsf-guarantee-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_acgsf_guarantee_go_has_runtime_entrypoint():
