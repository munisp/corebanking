...
SERVICE_PREFIX = "/sms-otp"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "sms-otp-service-rs"
...
def test_sms_otp_service_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"sms-otp-service-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_sms_otp_service_rs_has_runtime_entrypoint():
