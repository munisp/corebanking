...
SERVICE_PREFIX = "/backup-manager"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "backup-manager-py"
...
def test_backup_manager_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"backup-manager-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_backup_manager_py_has_runtime_entrypoint():
