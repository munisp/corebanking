...
SERVICE_PREFIX = "/synthetic-monitoring"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "synthetic-monitoring"
...
def test_synthetic_monitoring_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"synthetic-monitoring not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_synthetic_monitoring_has_runtime_entrypoint():
