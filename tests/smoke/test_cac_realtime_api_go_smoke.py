...
SERVICE_PREFIX = "/cac-realtime-api"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cac-realtime-api-go"
...
def test_cac_realtime_api_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cac-realtime-api-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cac_realtime_api_go_has_runtime_entrypoint():
