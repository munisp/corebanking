...
SERVICE_PREFIX = "/reconciliation"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "reconciliation-service"
...
def test_reconciliation_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"reconciliation-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_reconciliation_service_has_runtime_entrypoint():
