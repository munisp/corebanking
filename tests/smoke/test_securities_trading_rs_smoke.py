...
SERVICE_PREFIX = "/securities"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "securities-trading-rs"
...
def test_securities_trading_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"securities-trading-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_securities_trading_rs_has_runtime_entrypoint():
