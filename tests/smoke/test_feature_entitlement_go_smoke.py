...
SERVICE_PREFIX = "/feature-entitlement"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "feature-entitlement-go"
...
def test_feature_entitlement_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"feature-entitlement-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_feature_entitlement_go_has_runtime_entrypoint():
