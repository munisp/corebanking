...
SERVICE_PREFIX = "/ussd-sim-toolkit"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ussd-sim-toolkit-go"
...
def test_ussd_sim_toolkit_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ussd-sim-toolkit-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ussd_sim_toolkit_go_has_runtime_entrypoint():
