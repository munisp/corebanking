...
SERVICE_PREFIX = "/request-validator"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "request-validator-py"
...
def test_request_validator_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"request-validator-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_request_validator_py_has_runtime_entrypoint():
