...
SERVICE_PREFIX = "/customer-engagement"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "customer-engagement-py"
...
def test_customer_engagement_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"customer-engagement-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_customer_engagement_py_has_runtime_entrypoint():
