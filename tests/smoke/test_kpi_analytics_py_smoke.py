...
SERVICE_PREFIX = "/kpi-analytics"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "kpi-analytics-py"
...
def test_kpi_analytics_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"kpi-analytics-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_kpi_analytics_py_has_runtime_entrypoint():
