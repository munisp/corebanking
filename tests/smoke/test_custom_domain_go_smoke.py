...
SERVICE_PREFIX = "/custom-domain"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "custom-domain-go"
...
def test_custom_domain_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"custom-domain-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_custom_domain_go_has_runtime_entrypoint():
