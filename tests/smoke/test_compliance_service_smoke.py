...
SERVICE_PREFIX = "/compliance"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "compliance-service"
...
def test_compliance_service_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"compliance-service not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_compliance_service_has_runtime_entrypoint():
