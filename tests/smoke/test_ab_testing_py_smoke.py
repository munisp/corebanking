...
SERVICE_PREFIX = "/ab-testing"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ab-testing-py"
...
def test_ab_testing_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ab-testing-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ab_testing_py_has_runtime_entrypoint():
