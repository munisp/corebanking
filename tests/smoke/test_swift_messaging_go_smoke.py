...
_svc_dir = Path(__file__).parent.parent.parent / "services" / "swift-messaging-go"
...
def test_swift_messaging_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"swift-messaging-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_swift_messaging_go_has_runtime_entrypoint():
