...
SERVICE_PREFIX = "/document-intelligence"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "document-intelligence-py"
...
def test_document_intelligence_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"document-intelligence-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_document_intelligence_py_has_runtime_entrypoint():
