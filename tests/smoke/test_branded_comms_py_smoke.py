...
SERVICE_PREFIX = "/branded-comms"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "branded-comms-py"
...
def test_branded_comms_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"branded-comms-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_branded_comms_py_has_runtime_entrypoint():
