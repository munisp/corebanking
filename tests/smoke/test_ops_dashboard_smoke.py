...
SERVICE_PREFIX = "/ops-dashboard"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "ops-dashboard"
...
def test_ops_dashboard_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"ops-dashboard not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_ops_dashboard_has_runtime_entrypoint():
