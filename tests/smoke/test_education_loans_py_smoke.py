...
SERVICE_PREFIX = "/education-loans"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "education-loans-py"
...
def test_education_loans_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"education-loans-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_education_loans_py_has_runtime_entrypoint():
