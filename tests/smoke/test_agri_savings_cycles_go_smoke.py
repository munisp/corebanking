...
SERVICE_PREFIX = "/agri-savings-cycles"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "agri-savings-cycles-go"
...
def test_agri_savings_cycles_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"agri-savings-cycles-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_agri_savings_cycles_go_has_runtime_entrypoint():
