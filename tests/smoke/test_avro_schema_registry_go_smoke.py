...
SERVICE_PREFIX = "/avro-schema-registry"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "avro-schema-registry-go"
...
def test_avro_schema_registry_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"avro-schema-registry-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_avro_schema_registry_go_has_runtime_entrypoint():
