...
SERVICE_PREFIX = "/aml-training-tracker"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "aml-training-tracker-go"
...
def test_aml_training_tracker_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"aml-training-tracker-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_aml_training_tracker_go_has_runtime_entrypoint():
