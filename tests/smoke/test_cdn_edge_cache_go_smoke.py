...
SERVICE_PREFIX = "/cdn-edge-cache"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "cdn-edge-cache-go"
...
def test_cdn_edge_cache_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"cdn-edge-cache-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_cdn_edge_cache_go_has_runtime_entrypoint():
