...
def test_events_seed_exists():
    seed_path = Path(__file__).parent.parent.parent / "services/events/seed_data.json"
    if not seed_path.exists():
        pytest.skip(f"{seed_path} not found — run in CI or seed first")
    payload = json.loads(seed_path.read_text())
    assert payload["service"] == "events"
    assert len(payload["records"]) >= 2


def test_events_service_has_runtime_entrypoint():
    service_dir = Path(__file__).parent.parent.parent / "services/events"
