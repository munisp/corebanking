...
SERVICE_PREFIX = "/security-audit-logger"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "security-audit-logger-py"
...
def test_security_audit_logger_py_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"security-audit-logger-py not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_security_audit_logger_py_has_runtime_entrypoint():
