...
SERVICE_PREFIX = "/temporal-worker"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "temporal-worker-go"
...
def test_temporal_worker_go_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"temporal-worker-go not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_temporal_worker_go_has_runtime_entrypoint():
