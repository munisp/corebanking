...
SERVICE_PREFIX = "/http2-multiplexer"
_svc_dir = Path(__file__).parent.parent.parent / "services" / "http2-multiplexer-rs"
...
def test_http2_multiplexer_rs_reachable():
    ...
    for endpoint in ("/health", "/healthz", "/ready", "/livez"):
        try:
            ...
            continue
    pytest.skip(
        f"http2-multiplexer-rs not reachable at {BASE_URL}{SERVICE_PREFIX} — "
        "set CLUSTER_BASE_URL or ensure the service is deployed"
    )


def test_http2_multiplexer_rs_has_runtime_entrypoint():
