...
describe("Health & Monitoring Endpoints", () => {
  ...
  it("/healthz returns database connected status", async (ctx) => {
  ...
  it("/healthz includes memory metrics", async (ctx) => {
    if (!serverUp) return ctx.skip();
    const resp = await fetch(`${BASE}/healthz`);
    const data = await resp.json() as any;
  ...
  it("/api/platform/redis/status returns stats", async (ctx) => {
    if (!serverUp) return ctx.skip();
  ...
  it("/api/platform/kafka/status returns topics", async (ctx) => {
    if (!serverUp) return ctx.skip();
  ...
  it("/api/platform/sessions/stats returns session info", async (ctx) => {
    if (!serverUp) return ctx.skip();
