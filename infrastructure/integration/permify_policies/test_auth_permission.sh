...
curl -v -X POST "https://54link-dev.upi.dev/auth/permissions/check-permission?permission=${PERMISSION}&entity_type=${ENTITY_TYPE}&entity_id=${ENTITY_ID}&user_id=${USER_ID}" \
  -H "Content-Type: application/json" \
  -H "x-tenant-id: ${TENANT_ID}" \
  -H "x-keycloak-realm: 54link" \
