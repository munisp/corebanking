...
const STORAGE_KEYS = {
  activeCustomerId: "54bank.customer.active-id",
  transferDraft: "54bank.customer.transfer-draft",
  beneficiaries: "54bank.customer.beneficiaries",
  transfers: "54bank.customer.transfers",
  billPayments: "54bank.customer.bill-payments",
  notifications: "54bank.customer.notifications",
  cards: "54bank.customer.cards",
  cardEvents: "54bank.customer.card-events",
  savedBillers: "54bank.customer.saved-billers",
} as const;
...
export function buildSessionSummary(customers: CustomerRecord[], authContext: AuthContextResponse | null) {
  ...
  const visibleName = activeCustomer?.name ?? authContext?.actorId ?? "54Bank Customer";
