...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[agent-reconciliation-py] ready on :%d", PORT)
    logger.info(f"[agent-reconciliation-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="agent-reconciliation-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "agent-reconciliation-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "agent-reconciliation-py", "total_records": count}
    except Exception:
        return {"service": "agent-reconciliation-py", "total_records": 0}
