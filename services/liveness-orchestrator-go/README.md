...
```bash
# Build
go build -o liveness-orchestrator-go .

# Run
PORT=6379 ./liveness-orchestrator-go
```
...
```bash
docker build -t 54bank/liveness-orchestrator-go .
docker run -p 6379:6379 54bank/liveness-orchestrator-go
```
