...
// table-partitioner-rs — Database table partitioning
...
async fn partition(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let result = partition_name(table, date);
    let _result_data = json!({"endpoint": "partition"});
    db_persist(&state, "partition", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("table-partitioner-rs: upstream call ok"),
        Err(e) => eprintln!("table-partitioner-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "table-partitioner-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "table-partitioner-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"table-partitioner-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"table-partitioner-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "table_partitioner_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("table-partitioner-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8254);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("table-partitioner-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("table-partitioner-rs on port {}", port);
    start_grpc_server("table-partitioner-rs", 10398);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[table-partitioner-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/partition", web::post().to(partition))
...
mod tests {
    ...
    fn test_partition_name_exists() {
        // Verify partition_name compiles and is callable
        // Domain function: partition_name(table: &str, date: &str) -> String
        assert!(true, "partition_name should be defined");
    }

    #[test]
    fn test_partition_exists() {
        // Verify partition compiles and is callable
        // Domain function: partition(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "partition should be defined");
    }
    #[test]
