...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[table-partitioner-rs] starting");

    let db_name = "table-partitioner-rs".replace("-", "_");
    ...
    log::info!("[table-partitioner-rs] database connected, schema initialized");
    ...
    log::info!("[table-partitioner-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8749".to_string()).parse().unwrap_or(8749);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[table-partitioner-rs] ready on :{}", port);
