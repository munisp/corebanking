...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[crossborder-agri-trade-rs] starting");

    let db_name = "crossborder-agri-trade-rs".replace("-", "_");
    ...
    log::info!("[crossborder-agri-trade-rs] database connected, schema initialized");
    ...
    log::info!("[crossborder-agri-trade-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8891".to_string()).parse().unwrap_or(8891);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[crossborder-agri-trade-rs] ready on :{}", port);
