"""
bundle-splitter-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("bundle-splitter-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/bundle_splitter_py")
...
PORT = int(os.environ.get("PORT", "9593"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[bundle-splitter-py] ready on :%d", PORT)
    logger.info(f"[bundle-splitter-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="bundle-splitter-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "bundle-splitter-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "bundle-splitter-py", "total_records": count}
    except Exception:
        return {"service": "bundle-splitter-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("bundle-splitter-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("bundle-splitter-py",))
...
# --- Domain Logic ---
def gen_id():
    return "BUN-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
SERVICE_NAME = "bundle-splitter-py"
records: list = []
audit_log: list = []
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"bundle_splitter_{self.path}"
        ...
        logger.info(f"[bundle-splitter-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"service": "bundle-splitter-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service="bundle-splitter-py"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service="bundle-splitter-py"}} {error_count}\n'
            )
        else:
...
def delete_record(record_id: str):
    ...
    def do_POST(self):
        ...
        if length > 0:
            ...
            body = json.loads(sanitize_input(raw.decode() if isinstance(raw, bytes) else str(raw)))
        else:
            body = {}
        ...
        if path == "/v1/create":
            result = db_insert("bundle_splitter_py", body)
            cache_set(f"bundle_splitter:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/bundle-splitter/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/bundle-splitter/process":
            rid = body.get("id", "")
            for rec in records:
...
if __name__ == "__main__":
    ...
    logger.info(json.dumps({"service": "bundle-splitter-py", "port": PORT, "message": "starting"}))
    try:
