...
async fn health() -> HttpResponse {
        let _iot_data_valid = iot_data_valid(0);
...
async fn evaluate_trigger(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("parametric-insurance-iot-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("parametric-insurance-iot-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("parametric-insurance-iot-rs: upstream call join failed: {}", e),
    }
