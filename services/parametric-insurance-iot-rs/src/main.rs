...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[parametric-insurance-iot-rs] starting");

    let db_name = "parametric-insurance-iot-rs".replace("-", "_");
    ...
    log::info!("[parametric-insurance-iot-rs] database connected, schema initialized");
    ...
    log::info!("[parametric-insurance-iot-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8528".to_string()).parse().unwrap_or(8528);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[parametric-insurance-iot-rs] ready on :{}", port);
