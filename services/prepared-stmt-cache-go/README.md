...
```bash
# Build
go build -o prepared-stmt-cache-go .

# Run
PORT=6379 ./prepared-stmt-cache-go
```
...
```bash
docker build -t 54bank/prepared-stmt-cache-go .
docker run -p 6379:6379 54bank/prepared-stmt-cache-go
```
