...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("basel-engine.processed", &serde_json::json!({"status": "success"}));
...
async fn list_records(req: actix_web::HttpRequest, state: web::Data<AppState>, query: web::Query<std::collections::HashMap<String, String>>) -> HttpResponse {
    ...
    if let Some(ref client) = state.db_client {
        match client.query(
            ...
            &[&"basel_engine_rs", &(limit as i64), &(offset as i64)]
        ).await {
            Ok(rows) => {
                ...
                let total: i64 = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"basel_engine_rs"]).await.map(|r| r.get(0)).unwrap_or(0);
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    if let Some(ref client) = state.db_client {
        if let Ok(row) = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"basel_engine_rs"]).await {
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        ...
        let svc_name = String::from("basel-engine-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("basel-engine-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("basel-engine-rs", 10417);
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[basel-engine-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("risk.computation", "basel-engine");
    eprintln!("[basel-engine] Data flow initialized: topic=risk.computation");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "basel-engine-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "basel_engine");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "basel_engine");
    }
