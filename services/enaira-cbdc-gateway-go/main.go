...
// --- eNaira CBDC integration — wallet management, merchant payments ---
...
func handleHealthz(w http.ResponseWriter, r *http.Request) {
	...
	count := len(records)
	recordsMu.RUnlock()
	respondJSON(w, 200, map[string]interface{}{"status": "healthy", "service": "enaira-cbdc-gateway-go", "version": "1.0.0", "records": count})
}
...
func handleCreate(w http.ResponseWriter, r *http.Request) {
	...
	rec := Record{
		ID: fmt.Sprintf("ENA-%s", secureRandID()),
...
func main() {
	...
	mux.HandleFunc("/api/v1/enaira_cbdc_gateway", handleList)
	mux.HandleFunc("/api/v1/enaira_cbdc_gateway/create", handleCreate)
	log.Printf("eNaira CBDC integration — wallet management, merchant payments listening on :%s", PORT)
