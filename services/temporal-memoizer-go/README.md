...
```bash
# Build
go build -o temporal-memoizer-go .

# Run
PORT=6379 ./temporal-memoizer-go
```
...
```bash
docker build -t 54bank/temporal-memoizer-go .
docker run -p 6379:6379 54bank/temporal-memoizer-go
```
