...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[feature-flag-engine-rs] starting");

    let db_name = "feature-flag-engine-rs".replace("-", "_");
    ...
    log::info!("[feature-flag-engine-rs] database connected, schema initialized");
    ...
    log::info!("[feature-flag-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8808".to_string()).parse().unwrap_or(8808);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[feature-flag-engine-rs] ready on :{}", port);
