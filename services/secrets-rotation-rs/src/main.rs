...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[secrets-rotation-rs] starting");

    let db_name = "secrets-rotation-rs".replace("-", "_");
    ...
    log::info!("[secrets-rotation-rs] database connected, schema initialized");
    ...
    log::info!("[secrets-rotation-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8156".to_string()).parse().unwrap_or(8156);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[secrets-rotation-rs] ready on :{}", port);
