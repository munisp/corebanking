...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[qdrant-financial-search-rs] starting");

    let db_name = "qdrant-financial-search-rs".replace("-", "_");
    ...
    log::info!("[qdrant-financial-search-rs] database connected, schema initialized");
    ...
    log::info!("[qdrant-financial-search-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8992".to_string()).parse().unwrap_or(8992);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[qdrant-financial-search-rs] ready on :{}", port);
