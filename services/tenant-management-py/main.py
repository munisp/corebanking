"""
tenant-management-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("tenant-management-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/tenant_management_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[tenant-management-py] ready on :%d", PORT)
    logger.info(f"[tenant-management-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="tenant-management-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "tenant-management-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "tenant-management-py", "total_records": count}
    except Exception:
        return {"service": "tenant-management-py", "total_records": 0}
