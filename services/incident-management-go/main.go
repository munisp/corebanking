...
// --- Incident management — runbooks, escalation, status page, PIR tracking ---
...
func handleHealthz(w http.ResponseWriter, r *http.Request) {
	...
	count := len(records)
	recordsMu.RUnlock()
	respondJSON(w, 200, map[string]interface{}{"status": "healthy", "service": "incident-management-go", "version": "1.0.0", "records": count})
}
...
func main() {
	...
	mux.HandleFunc("/api/v1/incident_management", handleList)
	mux.HandleFunc("/api/v1/incident_management/create", handleCreate)
	log.Printf("Incident management — runbooks, escalation, status page, PIR tracking listening on :%s", PORT)
