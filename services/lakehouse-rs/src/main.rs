...
fn init_data_flow() -> EventBus {
    ...
    log::info!("[lakehouse] Data flow initialized: topic=data.pipeline");
    bus
}
...
fn init_consumer() -> EventConsumer {
    let consumer = EventConsumer::new(&["banking.payments", "accounting.ledger", "agriculture.operations"], "lakehouse");
    consumer.subscribe();
    consumer
}
