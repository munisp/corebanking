...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[offline-resilience-rs] starting");

    let db_name = "offline-resilience-rs".replace("-", "_");
    ...
    log::info!("[offline-resilience-rs] database connected, schema initialized");
    ...
    log::info!("[offline-resilience-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8821".to_string()).parse().unwrap_or(8821);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[offline-resilience-rs] ready on :{}", port);
