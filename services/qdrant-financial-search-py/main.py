"""
qdrant-financial-search-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("qdrant-financial-search-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/qdrant_financial_search_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[qdrant-financial-search-py] ready on :%d", PORT)
    logger.info(f"[qdrant-financial-search-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="qdrant-financial-search-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "qdrant-financial-search-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "qdrant-financial-search-py", "total_records": count}
    except Exception:
        return {"service": "qdrant-financial-search-py", "total_records": 0}
