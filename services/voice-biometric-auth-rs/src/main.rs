...
// voice-biometric-auth-rs — Voice biometric authentication
...
async fn verify_voice(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("voice-biometric-auth-rs: upstream call ok"),
        Err(e) => eprintln!("voice-biometric-auth-rs: upstream call failed: {}", e),
    }
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/v1/verify_voice", web::post().to(verify_voice))
