...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[voice-biometric-auth-rs] starting");

    let db_name = "voice-biometric-auth-rs".replace("-", "_");
    ...
    log::info!("[voice-biometric-auth-rs] database connected, schema initialized");
    ...
    log::info!("[voice-biometric-auth-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8151".to_string()).parse().unwrap_or(8151);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[voice-biometric-auth-rs] ready on :{}", port);
