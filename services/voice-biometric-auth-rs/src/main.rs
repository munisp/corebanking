...
async fn verify_voice(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "verify_voice"});
    db_persist(&state, "verify_voice", &_result_data).await;
    // Inter-service call
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("voice-biometric-auth-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("voice-biometric-auth-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("voice-biometric-auth-rs: upstream call join failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "voice-biometric-auth-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "voice-biometric-auth-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"voice-biometric-auth-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"voice-biometric-auth-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "voice_biometric_auth_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("voice-biometric-auth-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    start_grpc_server("voice-biometric-auth-rs", 10315);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[voice-biometric-auth-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .service(web::resource("/api/v1/kyc_records").wrap(actix_web::middleware::from_fn(jwt_route_guard)).route(web::post().to(create_record)))
            .service(web::resource("/api/v1/kyc_records/{id}").wrap(actix_web::middleware::from_fn(jwt_route_guard)).route(web::get().to(get_record)))
...
mod tests {
    ...
    #[test]
    fn test_voiceprint_similarity_exists() {
        // Verify voiceprint_similarity compiles and is callable
        // Domain function: voiceprint_similarity(embedding1: &[f64], embedding2: &[f64]) -> f64
        assert!(true, "voiceprint_similarity should be defined");
    }

    #[test]
    fn test_verify_voice_exists() {
        // Verify verify_voice compiles and is callable
        // Domain function: verify_voice(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "verify_voice should be defined");
    }
    #[test]
