...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[livestock-finance-rs] starting");

    let db_name = "livestock-finance-rs".replace("-", "_");
    ...
    log::info!("[livestock-finance-rs] database connected, schema initialized");
    ...
    log::info!("[livestock-finance-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8384".to_string()).parse().unwrap_or(8384);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[livestock-finance-rs] ready on :{}", port);
