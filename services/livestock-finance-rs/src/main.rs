...
// livestock-finance-rs — Livestock financing and insurance
...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("livestock-finance.processed", &serde_json::json!({"status": "success"}));
...
async fn assess_livestock(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "assess_livestock"});
    db_persist(&state, "assess_livestock", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("livestock-finance-rs: upstream call ok"),
        Err(e) => eprintln!("livestock-finance-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "livestock-finance-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "livestock-finance-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"livestock-finance-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"livestock-finance-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "livestock_finance_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("livestock-finance-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8175);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("livestock-finance-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("livestock-finance-rs on port {}", port);
    start_grpc_server("livestock-finance-rs", 10485);
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[livestock-finance-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/assess_livestock", web::post().to(assess_livestock))
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("agriculture.operations", "livestock-finance");
    eprintln!("[livestock-finance] Data flow initialized: topic=agriculture.operations");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "livestock-finance-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "livestock_finance");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "livestock_finance");
    }
