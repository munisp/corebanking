...
```bash
# Build
go build -o materialized-view-engine-go .

# Run
PORT=6379 ./materialized-view-engine-go
```
...
```bash
docker build -t 54bank/materialized-view-engine-go .
docker run -p 6379:6379 54bank/materialized-view-engine-go
```
