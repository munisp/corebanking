"""
commodity-price-intelligence-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("commodity-price-intelligence-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/commodity_price_intelligence_py")
...
def init_schema():
    ...
    try:
        ...
        cur.execute("INSERT INTO records (data, service) VALUES (%s, %s) RETURNING id, created_at",
                    (data, "commodity-price-intelligence-py"))
        row = cur.fetchone()
        record["id"] = str(row[0])
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[commodity-price-intelligence-py] ready on :%d", PORT)
    logger.info(f"[commodity-price-intelligence-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="commodity-price-intelligence-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "commodity-price-intelligence-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "commodity-price-intelligence-py", "total_records": count}
    except Exception:
        return {"service": "commodity-price-intelligence-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("commodity-price-intelligence-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("commodity-price-intelligence-py",))
...
# --- Domain Logic ---
def gen_id():
    return "COM-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"commodity_price_intelligence_{self.path}"
        ...
        logger.info(f"[commodity-price-intelligence-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"service": "commodity-price-intelligence-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service="commodity-price-intelligence-py"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service="commodity-price-intelligence-py"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if err:
            ...
            try:
                ...
                call_service("POST", f"{_upstream}/v1/screen", {"service": "commodity-price-intelligence-py", "action": "notify"})
            except Exception as _e:
                logger.warning(f"inter-service call failed: {_e}")
            return

        if path == "/v1/create":
            result = db_insert("commodity_price_intelligence_py", body)
            cache_set("commodity_price_intel:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/commodity-price-intelligence/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/commodity-price-intelligence/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/commodity-price-intelligence/analyze":
...
if __name__ == "__main__":
    ...
    logger.info(json.dumps({"service": "commodity-price-intelligence-py", "port": PORT, "message": "starting"}))
    try:
