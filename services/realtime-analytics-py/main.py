"""
realtime-analytics-py - Production-ready service with PostgreSQL persistence.
...
# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/realtime_analytics_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[realtime-analytics-py] ready on :%d", PORT)
    logger.info(f"[realtime-analytics-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="realtime-analytics-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "realtime-analytics-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "realtime-analytics-py", "total_records": count}
    except Exception:
        return {"service": "realtime-analytics-py", "total_records": 0}
