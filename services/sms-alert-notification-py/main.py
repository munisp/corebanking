"""
sms-alert-notification-py - Production-ready service with PostgreSQL persistence.
...
# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/sms_alert_notification_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[sms-alert-notification-py] ready on :%d", PORT)
    logger.info(f"[sms-alert-notification-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="sms-alert-notification-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "sms-alert-notification-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "sms-alert-notification-py", "total_records": count}
    except Exception:
        return {"service": "sms-alert-notification-py", "total_records": 0}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"sms_alert_notification_{self.path}"
        ...
        logger.info(f"[sms-alert-notification-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "sms-alert-notification-py", **_degrade.status()})
            elif path == "/v1/alerts":
...
@app.get("/api/v1/notifications/{record_id}")
def get_record(record_id: str):
    conn = get_db()
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute("SELECT id, status, created_at FROM notifications WHERE id = %s::uuid", (record_id,))
        row = cur.fetchone()
    if not row:
...
def update_record(record_id: str, body: UpdateRequest):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE notifications SET status = %s, updated_at = NOW() WHERE id = %s::uuid",
            (status, record_id)
        )
        payload = json.dumps({"id": record_id, "status": status})
        cur.execute(
            "INSERT INTO outbox (event_type, aggregate_id, payload) VALUES (%s, %s, %s::jsonb)",
            ("notifications.updated", record_id, payload)
        )
    conn.commit()
...
def delete_record(record_id: str):
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute("UPDATE notifications SET status = 'deleted', updated_at = NOW() WHERE id = %s::uuid", (record_id,))
        payload = json.dumps({"id": record_id})
        cur.execute(
            "INSERT INTO outbox (event_type, aggregate_id, payload) VALUES (%s, %s, %s::jsonb)",
            ("notifications.deleted", record_id, payload)
        )
    conn.commit()
    ...
    def do_POST(self):
        ...
        elif path == "/v1/sms-alert-notification/process":
            rid = body.get("id", "")
            for rec in records:
