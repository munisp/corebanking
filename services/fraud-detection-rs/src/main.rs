...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[fraud-detection-rs] starting");

    let db_name = "fraud-detection-rs".replace("-", "_");
    ...
    log::info!("[fraud-detection-rs] database connected, schema initialized");
    ...
    log::info!("[fraud-detection-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8078".to_string()).parse().unwrap_or(8078);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[fraud-detection-rs] ready on :{}", port);
