...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[http2-multiplexer-rs] starting");

    let db_name = "http2-multiplexer-rs".replace("-", "_");
    ...
    log::info!("[http2-multiplexer-rs] database connected, schema initialized");
    ...
    log::info!("[http2-multiplexer-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8019".to_string()).parse().unwrap_or(8019);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[http2-multiplexer-rs] ready on :{}", port);
