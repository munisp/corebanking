"""
nfiu-ctr-str-filing-py - Production-ready service with PostgreSQL persistence.
...
# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/nfiu_ctr_str_filing_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[nfiu-ctr-str-filing-py] ready on :%d", PORT)
    logger.info(f"[nfiu-ctr-str-filing-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="nfiu-ctr-str-filing-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "nfiu-ctr-str-filing-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "nfiu-ctr-str-filing-py", "total_records": count}
    except Exception:
        return {"service": "nfiu-ctr-str-filing-py", "total_records": 0}
