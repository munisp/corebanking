...
async fn scan_compliance(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "scan_compliance"});
    db_persist(&state, "scan_compliance", &_result_data).await;
    // Inter-service call
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("pci-scanner-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("pci-scanner-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("pci-scanner-rs: upstream call join failed: {}", e),
    }
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "pci-scanner-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"pci-scanner-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"pci-scanner-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "pci_scanner_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("pci-scanner-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("pci-scanner-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("pci-scanner-rs", 10465);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[pci-scanner-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    #[test]
    fn test_pan_detected_exists() {
        // Verify pan_detected compiles and is callable
        // Domain function: pan_detected(text: &str) -> bool
        assert!(true, "pan_detected should be defined");
    }

    #[test]
    fn test_scan_compliance_exists() {
        // Verify scan_compliance compiles and is callable
        // Domain function: scan_compliance(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "scan_compliance should be defined");
    }
    #[test]
