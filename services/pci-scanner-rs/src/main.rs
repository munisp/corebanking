...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[pci-scanner-rs] starting");

    let db_name = "pci-scanner-rs".replace("-", "_");
    ...
    log::info!("[pci-scanner-rs] database connected, schema initialized");
    ...
    log::info!("[pci-scanner-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8324".to_string()).parse().unwrap_or(8324);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[pci-scanner-rs] ready on :{}", port);
