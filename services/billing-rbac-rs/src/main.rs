...
async fn check_permission(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("billing-rbac-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("billing-rbac-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("billing-rbac-rs: upstream call join failed: {}", e),
    }
...
async fn list_records(req: actix_web::HttpRequest, state: web::Data<AppState>, query: web::Query<std::collections::HashMap<String, String>>) -> HttpResponse {
    ...
    if let Some(ref client) = state.db_client {
        match client.query(
            ...
            &[&"billing_rbac_rs", &(limit as i64), &(offset as i64)]
        ).await {
            Ok(rows) => {
                ...
                let total: i64 = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"billing_rbac_rs"]).await.map(|r| r.get(0)).unwrap_or(0);
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    if let Some(ref client) = state.db_client {
        if let Ok(row) = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"billing_rbac_rs"]).await {
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "billing-rbac-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"billing-rbac-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"billing-rbac-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        ...
        let svc_name = String::from("billing-rbac-rs");
...
async fn billing_rbac_enforce(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    if !auth.starts_with("Bearer ") {
        return HttpResponse::Unauthorized().json(json!({"error": "missing_token"}));
    }
    let input = body.into_inner();
    let permission = input.get("permission").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let tenant_id = input.get("tenant_id").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let actor_id  = input.get("actor_id").and_then(|v| v.as_str()).unwrap_or("").to_string();

    // Derive role from explicit field or from permission string
    let role: String = if let Some(r) = input.get("role").and_then(|v| v.as_str()) {
        r.to_string()
    } else if permission.contains("admin") {
        "admin".to_string()
    } else if permission.contains("write") || permission.contains("manage") {
        "manager".to_string()
    } else if permission.contains("rate") {
        "agent".to_string()
    } else {
        "viewer".to_string()
    };

    let action: String = if let Some(a) = input.get("action").and_then(|v| v.as_str()) {
        a.to_string()
    } else if permission.contains("invoice") || permission.contains("view") || permission.contains("read") {
        "view_bills".to_string()
    } else if permission.contains("manage") || permission.contains("write") {
        "approve_discount".to_string()
    } else {
        "view_bills".to_string()
    };
    ...
    let allowed = match role.as_str() {
        "admin"   => true,
        "manager" => matches!(action.as_str(), "approve_discount" | "view_reports" | "rate_override" | "view_bills"),
        "agent"   => matches!(action.as_str(), "view_bills" | "rate_transaction"),
        "viewer"  => matches!(action.as_str(), "view_bills" | "view_reports"),
        _         => false,
    };
    let decision = if allowed { "allow" } else { "deny" };
    let reason = if allowed {
        format!("role '{}' permits '{}'", role, action)
    } else {
        format!("role '{}' does not permit '{}'", role, action)
    };
    let id = format!("RBAC-{:X}", std::time::SystemTime::now()
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("billing-rbac-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("billing-rbac-rs", 10440);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[billing-rbac-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            // ── Billing UI endpoint ───────────────────────────────────────────
            .route("/v1/billing/rbac/enforce", web::post().to(billing_rbac_enforce))
    })
    .bind(("0.0.0.0", port))?
    .shutdown_timeout(30)
    .run()
