...
async fn main() -> std::io::Result<()> {
    ...
    let db_name = "ussd-transaction-engine-rs".replace("-", "_");
    ...
    log::info!("[ussd-transaction-engine-rs] database connected, schema initialized");
    ...
    log::info!("[ussd-transaction-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8657".to_string()).parse().unwrap_or(8657);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[ussd-transaction-engine-rs] ready on :{}", port);
...
async fn init_schema(pool: &PgPool) {
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_transactions_tenant ON transactions(tenant_id)")
        .execute(pool).await.ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_transactions_status ON transactions(status)")
        .execute(pool).await.ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_transactions_created ON transactions(created_at DESC)")
        .execute(pool).await.ok();
}
...
async fn list_records(data: web::Data<AppState>, req: actix_web::HttpRequest) -> HttpResponse {
    ...
    let rows = sqlx::query("SELECT id, status, created_at FROM transactions WHERE ($1 = '' OR tenant_id::text = $1) ORDER BY created_at DESC LIMIT 50")
        .bind(tenant_id)
        .fetch_all(&data.db)
        .await;
...
async fn create_record(data: web::Data<AppState>, body: web::Json<CreateRequest>, req: actix_web::HttpRequest) -> HttpResponse {
    ...
    let result = sqlx::query_scalar::<_, Uuid>(
        "INSERT INTO transactions (tenant_id, status) VALUES ($1::uuid, $2) RETURNING id"
    )
    .bind(&tenant_id)
    .bind(&status)
    .fetch_one(&data.db)
...
async fn get_record(data: web::Data<AppState>, path: web::Path<String>) -> HttpResponse {
    ...
    let result = sqlx::query("SELECT id, status, created_at FROM transactions WHERE id = $1::uuid")
        .bind(&id)
