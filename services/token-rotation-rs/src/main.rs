...
fn token_age_seconds(issued_at: u64) -> u64 { std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs() - issued_at }
...
async fn rotate_token(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("token-rotation-rs: upstream call ok"),
        Err(e) => eprintln!("token-rotation-rs: upstream call failed: {}", e),
    }
