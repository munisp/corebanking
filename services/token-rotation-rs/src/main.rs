...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[token-rotation-rs] starting");

    let db_name = "token-rotation-rs".replace("-", "_");
    ...
    log::info!("[token-rotation-rs] database connected, schema initialized");
    ...
    log::info!("[token-rotation-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8594".to_string()).parse().unwrap_or(8594);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[token-rotation-rs] ready on :{}", port);
