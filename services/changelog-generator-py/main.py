"""
changelog-generator-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("changelog-generator-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/changelog_generator_py")
...
PORT = int(os.environ.get("PORT", "9598"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[changelog-generator-py] ready on :%d", PORT)
    logger.info(f"[changelog-generator-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="changelog-generator-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "changelog-generator-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "changelog-generator-py", "total_records": count}
    except Exception:
        return {"service": "changelog-generator-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("changelog-generator-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("changelog-generator-py",))
...
# --- Domain Logic ---
def gen_id():
    return "CHA-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"changelog_generator_{self.path}"
        ...
        logger.info(f"[changelog-generator-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "changelog-generator-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"changelog-generator-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"changelog-generator-py\"}} {error_count}\n'
            )
        else:
...
def delete_record(record_id: str):
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("changelog_generator_py", body)
            cache_set(f"{self.get_tenant_id()}:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/changelog-generator/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/changelog-generator/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/changelog-generator/health-checks":
            result = run_health_checks(body.get("services", []))
            self.respond(200, result)
        elif path == "/v1/changelog-generator/sla-metrics":
