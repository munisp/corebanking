...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[hsm-key-manager-rs] starting");

    let db_name = "hsm-key-manager-rs".replace("-", "_");
    ...
    log::info!("[hsm-key-manager-rs] database connected, schema initialized");
    ...
    log::info!("[hsm-key-manager-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8610".to_string()).parse().unwrap_or(8610);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[hsm-key-manager-rs] ready on :{}", port);
