...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("hsm-key-manager.processed", &serde_json::json!({"status": "success"}));
...
async fn manage_key(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("hsm-key-manager-rs: upstream call ok"),
        Err(e) => eprintln!("hsm-key-manager-rs: upstream call failed: {}", e),
    }
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "hsm-key-manager-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"hsm-key-manager-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"hsm-key-manager-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "hsm_key_manager_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("hsm-key-manager-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    start_grpc_server("hsm-key-manager-rs", 10310);
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[hsm-key-manager-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("security.infra", "hsm-key-manager");
    eprintln!("[hsm-key-manager] Data flow initialized: topic=security.infra");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "hsm-key-manager-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "hsm_key_manager");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "hsm_key_manager");
    }
