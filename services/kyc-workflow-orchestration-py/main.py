...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[kyc-workflow-orchestration-py] ready on :%d", PORT)
    logger.info(f"[kyc-workflow-orchestration-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="kyc-workflow-orchestration-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "kyc-workflow-orchestration-py", "version": "1.0.0"}
