...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[contingent-liabilities-rs] starting");

    let db_name = "contingent-liabilities-rs".replace("-", "_");
    ...
    log::info!("[contingent-liabilities-rs] database connected, schema initialized");
    ...
    log::info!("[contingent-liabilities-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8552".to_string()).parse().unwrap_or(8552);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[contingent-liabilities-rs] ready on :{}", port);
