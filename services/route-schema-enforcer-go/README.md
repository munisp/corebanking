...
```bash
# Build
go build -o route-schema-enforcer-go .

# Run
PORT=6379 ./route-schema-enforcer-go
```
...
```bash
docker build -t 54bank/route-schema-enforcer-go .
docker run -p 6379:6379 54bank/route-schema-enforcer-go
```
