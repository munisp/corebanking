...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[pin-hasher-rs] starting");

    let db_name = "pin-hasher-rs".replace("-", "_");
    ...
    log::info!("[pin-hasher-rs] database connected, schema initialized");
    ...
    log::info!("[pin-hasher-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8954".to_string()).parse().unwrap_or(8954);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[pin-hasher-rs] ready on :{}", port);
