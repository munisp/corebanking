...
fn init_data_flow() -> EventBus {
    ...
    log::info!("[efass-generator] Data flow initialized: topic=accounting.ledger");
    bus
}
...
fn init_consumer() -> EventConsumer {
    let consumer = EventConsumer::new(&["accounting.ledger", "compliance.screening"], "efass-generator");
    consumer.subscribe();
    consumer
}
