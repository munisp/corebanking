"""
txn-pattern-analyzer-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("txn-pattern-analyzer-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/txn_pattern_analyzer_py")
...
PORT = int(os.environ.get("PORT", "9642"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[txn-pattern-analyzer-py] ready on :%d", PORT)
    logger.info(f"[txn-pattern-analyzer-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="txn-pattern-analyzer-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "txn-pattern-analyzer-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "txn-pattern-analyzer-py", "total_records": count}
    except Exception:
        return {"service": "txn-pattern-analyzer-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("txn-pattern-analyzer-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("txn-pattern-analyzer-py",))
...
# --- Domain Logic ---
def gen_id():
    return "TXN-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"txn_pattern_analyzer_{self.path}"
        ...
        logger.info(f"[txn-pattern-analyzer-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "txn-pattern-analyzer-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"txn-pattern-analyzer-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"txn-pattern-analyzer-py\"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("txn_pattern_analyzer_py", body)
            cache_set(f"{self.get_tenant_id()}:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/txn-pattern-analyzer/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
