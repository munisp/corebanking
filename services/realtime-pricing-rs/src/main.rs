...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[realtime-pricing-rs] starting");

    let db_name = "realtime-pricing-rs".replace("-", "_");
    ...
    log::info!("[realtime-pricing-rs] database connected, schema initialized");
    ...
    log::info!("[realtime-pricing-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8096".to_string()).parse().unwrap_or(8096);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[realtime-pricing-rs] ready on :{}", port);
