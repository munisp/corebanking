...
async fn config(req: actix_web::HttpRequest) -> impl Responder {
    if let Err(resp) = check_jwt(&req).await { return resp; } HttpResponse::Ok().json(json!({"service":"Real-Time Pricing","port":8343,"status":"active"})) }
async fn mw(req: actix_web::HttpRequest) -> impl Responder {
