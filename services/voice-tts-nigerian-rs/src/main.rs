...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[voice-tts-nigerian-rs] starting");

    let db_name = "voice-tts-nigerian-rs".replace("-", "_");
    ...
    log::info!("[voice-tts-nigerian-rs] database connected, schema initialized");
    ...
    log::info!("[voice-tts-nigerian-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8894".to_string()).parse().unwrap_or(8894);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[voice-tts-nigerian-rs] ready on :{}", port);
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/api/v1/channel_sessions", web::post().to(create_record))
            .route("/api/v1/channel_sessions/{id}", web::get().to(get_record))
...
async fn init_schema(pool: &PgPool) {
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_channel_sessions_tenant ON channel_sessions(tenant_id)")
        .execute(pool).await.ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_channel_sessions_status ON channel_sessions(status)")
        .execute(pool).await.ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_channel_sessions_created ON channel_sessions(created_at DESC)")
        .execute(pool).await.ok();
}
...
async fn metrics(data: web::Data<AppState>) -> HttpResponse {
    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM channel_sessions")
...
async fn list_records(data: web::Data<AppState>, req: actix_web::HttpRequest) -> HttpResponse {
    ...
    let rows = sqlx::query("SELECT id, status, created_at FROM channel_sessions WHERE ($1 = '' OR tenant_id::text = $1) ORDER BY created_at DESC LIMIT 50")
        .bind(tenant_id)
        .fetch_all(&data.db)
        .await;
...
async fn create_record(data: web::Data<AppState>, body: web::Json<CreateRequest>, req: actix_web::HttpRequest) -> HttpResponse {
    ...
    let result = sqlx::query_scalar::<_, Uuid>(
        "INSERT INTO channel_sessions (tenant_id, status) VALUES ($1::uuid, $2) RETURNING id"
    )
    .bind(&tenant_id)
    .bind(&status)
    .fetch_one(&data.db)
...
async fn get_record(data: web::Data<AppState>, path: web::Path<String>) -> HttpResponse {
    ...
    let result = sqlx::query("SELECT id, status, created_at FROM channel_sessions WHERE id = $1::uuid")
        .bind(&id)
