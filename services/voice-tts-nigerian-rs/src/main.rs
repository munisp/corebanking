...
async fn synthesize(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "synthesize"});
    db_persist(&state, "synthesize", &_result_data).await;
    // Inter-service call
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("voice-tts-nigerian-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("voice-tts-nigerian-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("voice-tts-nigerian-rs: upstream call join failed: {}", e),
    }
...
async fn available_voices(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    if !rl_allow() {
        return HttpResponse::TooManyRequests().json(json!({"error": "rate_limit_exceeded"}));
    }
    if let Err(resp) = check_jwt(&req) { return resp; }
    ...
    let _result_data = json!({"endpoint": "available_voices"});
    db_persist(&state, "available_voices", &_result_data).await;
...
async fn ssml_parse(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    if !rl_allow() {
        return HttpResponse::TooManyRequests().json(json!({"error": "rate_limit_exceeded"}));
    }
    if let Err(resp) = check_jwt(&req) { return resp; }
    ...
    let _result_data = json!({"endpoint": "ssml_parse"});
    db_persist(&state, "ssml_parse", &_result_data).await;
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "voice_tts_nigerian_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("voice-tts-nigerian-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    start_grpc_server("voice-tts-nigerian-rs", 10375);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .route("/api/v1/channel_sessions", web::get().to(list_records))
            .service(web::resource("/api/v1/channel_sessions").wrap(actix_web::middleware::from_fn(jwt_route_guard)).route(web::post().to(create_record)))
            .service(web::resource("/api/v1/channel_sessions/{id}").wrap(actix_web::middleware::from_fn(jwt_route_guard)).route(web::get().to(get_record)))
            .route("/api/v1/channel_sessions/{id}", web::put().to(update_record))
            .route("/api/v1/channel_sessions/{id}", web::delete().to(delete_record))
    })
    .bind(("0.0.0.0", port))?
    .shutdown_timeout(30)
    .run()
...
async fn init_schema(pool: &PgPool) {
    sqlx::query(r#"CREATE TABLE IF NOT EXISTS channel_sessions (
    ...
    .execute(pool)
    .await
    .expect("Failed to create channel_sessions table");
}
...
async fn update_record(data: web::Data<AppState>, path: web::Path<String>, body: web::Json<CreateRequest>, req: actix_web::HttpRequest) -> HttpResponse {
    ...
    let result = sqlx::query("UPDATE channel_sessions SET status = $1, updated_at = NOW() WHERE id = $2::uuid")
        .bind(&status)
        .bind(&id)
        .execute(&data.db)
        .await;
...
async fn delete_record(data: web::Data<AppState>, path: web::Path<String>, req: actix_web::HttpRequest) -> HttpResponse {
    if let Err(resp) = check_jwt(&req) { return resp; }
    let id = path.into_inner();
    sqlx::query("UPDATE channel_sessions SET status = 'deleted', updated_at = NOW() WHERE id = $1::uuid")
        .bind(&id)
        .execute(&data.db)
        .await
        .ok();
