...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[interest-computation-rs] starting");

    let db_name = "interest-computation-rs".replace("-", "_");
    ...
    log::info!("[interest-computation-rs] database connected, schema initialized");
    ...
    log::info!("[interest-computation-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8980".to_string()).parse().unwrap_or(8980);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[interest-computation-rs] ready on :{}", port);
