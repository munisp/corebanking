...
fn init_data_flow() -> EventBus {
    ...
    eprintln!("[interest-computation] Data flow initialized: topic=risk.computation");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "interest-computation-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "interest_computation");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "interest_computation");
    }
