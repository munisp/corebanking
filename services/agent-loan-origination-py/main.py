...
"""
agent-loan-origination-py — Smart Loan Origination Assistant — assesses risk, checks exposure, computes pricing, generates credit memo
...
AGENT_TOOLS = ["neo4j_graph", "qdrant_search", "core_banking", "gl_engine", "aml_engine"]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "agent-loan-origination-py", **_degrade.status()})
        elif path == "/v1/alerts":
    ...
    def do_POST(self):
        ...
        if path == "/v1/agent/assess-loan":
            ...
            _event_bus.emit("agent-loan-origination.processed", {"action": "process", "status": "success"})
            ...
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "assess_loan", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/check-exposure":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "check_exposure", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/price-loan":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "price_loan", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/credit-memo":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "credit_memo", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
...
_event_bus = EventBus("banking.lending", "agent-loan-origination")
