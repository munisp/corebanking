...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[agent-loan-origination-py] ready on :%d", PORT)
    logger.info(f"[agent-loan-origination-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="agent-loan-origination-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "agent-loan-origination-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "agent-loan-origination-py", "total_records": count}
    except Exception:
        return {"service": "agent-loan-origination-py", "total_records": 0}
