...
async fn main() -> std::io::Result<()> {
    ...
    let db_name = "falkordb-coa-rs".replace("-", "_");
    ...
    log::info!("[falkordb-coa-rs] database connected, schema initialized");
    ...
    log::info!("[falkordb-coa-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8764".to_string()).parse().unwrap_or(8764);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[falkordb-coa-rs] ready on :{}", port);
