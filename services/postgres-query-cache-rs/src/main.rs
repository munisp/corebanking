...
// postgres-query-cache-rs — PostgreSQL query cache
...
fn cache_key(sql: &str) -> u64 { sql.bytes().fold(0u64, |h, b| h.wrapping_mul(31).wrapping_add(b as u64)) }
...
async fn health() -> HttpResponse {
    HttpResponse::Ok().insert_header(("content-security-policy", "default-src 'self'")).json(json!({"status": "healthy", "service": "postgres-query-cache-rs"}))
}

async fn cached_query(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let sql_s = input.get("sql").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let sql = sql_s.as_str();
    let result = cache_key(sql);
    let _result_data = json!({"endpoint": "cached_query"});
    db_persist(&state, "cached_query", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("postgres-query-cache-rs: upstream call ok"),
        Err(e) => eprintln!("postgres-query-cache-rs: upstream call failed: {}", e),
    }

    HttpResponse::Ok().json(json!({
        "service": "postgres-query-cache-rs",
        "endpoint": "cached_query",
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "postgres-query-cache-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "postgres-query-cache-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"postgres-query-cache-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"postgres-query-cache-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "postgres_query_cache_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("postgres-query-cache-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8239);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("postgres-query-cache-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("postgres-query-cache-rs on port {}", port);
    start_grpc_server("postgres-query-cache-rs", 10495);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[postgres-query-cache-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/cached_query", web::post().to(cached_query))
...
mod tests {
    ...
    #[test]
    fn test_cache_key_exists() {
        // Verify cache_key compiles and is callable
        // Domain function: cache_key(sql: &str) -> u64
        assert!(true, "cache_key should be defined");
    }

    #[test]
    fn test_cached_query_exists() {
        // Verify cached_query compiles and is callable
        // Domain function: cached_query(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "cached_query should be defined");
    }
    #[test]
