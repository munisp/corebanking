...
// fast-json-serializer-rs — High-performance JSON serializer
...
async fn serialize(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "serialize"});
    db_persist(&state, "serialize", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("fast-json-serializer-rs: upstream call ok"),
        Err(e) => eprintln!("fast-json-serializer-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "fast-json-serializer-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "fast-json-serializer-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"fast-json-serializer-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"fast-json-serializer-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "fast_json_serializer_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("fast-json-serializer-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8198);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("fast-json-serializer-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("fast-json-serializer-rs on port {}", port);
    start_grpc_server("fast-json-serializer-rs", 10460);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[fast-json-serializer-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/serialize", web::post().to(serialize))
...
mod tests {
    ...
    #[test]
    fn test_estimate_json_size_exists() {
        // Verify estimate_json_size compiles and is callable
        // Domain function: estimate_json_size(fields: u32, avg_value_len: u32) -> u64
        assert!(true, "estimate_json_size should be defined");
    }

    #[test]
    fn test_should_compress_exists() {
        // Verify should_compress compiles and is callable
        // Domain function: should_compress(size_bytes: u64) -> bool
        assert!(true, "should_compress should be defined");
    }

    #[test]
    fn test_serialize_exists() {
        // Verify serialize compiles and is callable
        // Domain function: serialize(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "serialize should be defined");
    }
    #[test]
