...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[fast-json-serializer-rs] starting");

    let db_name = "fast-json-serializer-rs".replace("-", "_");
    ...
    log::info!("[fast-json-serializer-rs] database connected, schema initialized");
    ...
    log::info!("[fast-json-serializer-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8279".to_string()).parse().unwrap_or(8279);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[fast-json-serializer-rs] ready on :{}", port);
