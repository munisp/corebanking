"""
risk-based-approach-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("risk-based-approach-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/risk_based_approach_py")
...
RISK_ASSESSMENT_URL = os.environ.get("RISK_ASSESSMENT_URL", "http://localhost:8125")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[risk-based-approach-py] ready on :%d", PORT)
    logger.info(f"[risk-based-approach-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="risk-based-approach-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "risk-based-approach-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "risk-based-approach-py", "total_records": count}
    except Exception:
        return {"service": "risk-based-approach-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("risk-based-approach-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("risk-based-approach-py",))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"risk_based_approach_{self.path}"
        ...
        logger.info(f"[risk-based-approach-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self._json(200, {"service": "risk-based-approach-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"risk-based-approach-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"risk-based-approach-py\"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if err:
            ...
            # Inter-service call
            try:
                _upstream = os.environ.get("RISK_ASSESSMENT_URL", "http://localhost:8125")
                call_service("POST", f"{_upstream}/v1/assess", {"service": SERVICE_NAME, "action": "notify"})
            except Exception as _e:
                log_event("WARN", f"inter-service call failed: {_e}")
            return

        if path == "/v1/create":
            result = db_insert("risk_based_approach_py", body)
