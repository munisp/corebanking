...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[whatsapp-banking-flows-rs] starting");

    let db_name = "whatsapp-banking-flows-rs".replace("-", "_");
    ...
    log::info!("[whatsapp-banking-flows-rs] database connected, schema initialized");
    ...
    log::info!("[whatsapp-banking-flows-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8363".to_string()).parse().unwrap_or(8363);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[whatsapp-banking-flows-rs] ready on :{}", port);
