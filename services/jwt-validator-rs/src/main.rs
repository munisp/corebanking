...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[jwt-validator-rs] starting");

    let db_name = "jwt-validator-rs".replace("-", "_");
    ...
    log::info!("[jwt-validator-rs] database connected, schema initialized");
    ...
    log::info!("[jwt-validator-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8309".to_string()).parse().unwrap_or(8309);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[jwt-validator-rs] ready on :{}", port);
