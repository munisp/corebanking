...
```bash
# Build
go build -o beneficial-ownership-go .

# Run
PORT=6379 ./beneficial-ownership-go
```
...
```bash
docker build -t 54bank/beneficial-ownership-go .
docker run -p 6379:6379 54bank/beneficial-ownership-go
```
