...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[cloud-kms-bridge-rs] starting");

    let db_name = "cloud-kms-bridge-rs".replace("-", "_");
    ...
    log::info!("[cloud-kms-bridge-rs] database connected, schema initialized");
    ...
    log::info!("[cloud-kms-bridge-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8256".to_string()).parse().unwrap_or(8256);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[cloud-kms-bridge-rs] ready on :{}", port);
