...
async fn manage_key(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "manage_key"});
    db_persist(&state, "manage_key", &_result_data).await;
    // Inter-service call
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("cloud-kms-bridge-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("cloud-kms-bridge-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("cloud-kms-bridge-rs: upstream call join failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "cloud-kms-bridge-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "cloud-kms-bridge-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"cloud-kms-bridge-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"cloud-kms-bridge-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "cloud_kms_bridge_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("cloud-kms-bridge-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8195);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("cloud-kms-bridge-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("cloud-kms-bridge-rs on port {}", port);
    start_grpc_server("cloud-kms-bridge-rs", 10357);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[cloud-kms-bridge-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    #[test]
    fn test_key_rotation_due_exists() {
        // Verify key_rotation_due compiles and is callable
        // Domain function: key_rotation_due(last_rotation_days: u32, max_days: u32) -> bool
        assert!(true, "key_rotation_due should be defined");
    }

    #[test]
    fn test_manage_key_exists() {
        // Verify manage_key compiles and is callable
        // Domain function: manage_key(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "manage_key should be defined");
    }
    #[test]
