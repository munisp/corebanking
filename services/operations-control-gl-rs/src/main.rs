...
async fn limit_management_gl(req: actix_web::HttpRequest) -> HttpResponse {
    ...
    _bus.emit("operations-control-gl.processed", &serde_json::json!({"status": "success"}));
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("accounting.ledger", "operations-control-gl");
    eprintln!("[operations-control-gl] Data flow initialized: topic=accounting.ledger");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "operations-control-gl-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "operations_control_gl");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "operations_control_gl");
    }
