...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[cache-invalidation-rs] starting");

    let db_name = "cache-invalidation-rs".replace("-", "_");
    ...
    log::info!("[cache-invalidation-rs] database connected, schema initialized");
    ...
    log::info!("[cache-invalidation-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8747".to_string()).parse().unwrap_or(8747);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[cache-invalidation-rs] ready on :{}", port);
