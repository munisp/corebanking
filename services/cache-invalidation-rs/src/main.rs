...
// cache-invalidation-rs — Cache invalidation coordination
...
fn invalidation_pattern(key: &str) -> String { format!("{}*", key.split(':').take(2).collect::<Vec<_>>().join(":")) }
...
async fn health() -> HttpResponse {
    HttpResponse::Ok().insert_header(("content-security-policy", "default-src 'self'")).json(json!({"status": "healthy", "service": "cache-invalidation-rs"}))
}

async fn invalidate(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "invalidate"});
    db_persist(&state, "invalidate", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("cache-invalidation-rs: upstream call ok"),
        Err(e) => eprintln!("cache-invalidation-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "cache-invalidation-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "cache-invalidation-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"cache-invalidation-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"cache-invalidation-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "cache_invalidation_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("cache-invalidation-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8265);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("cache-invalidation-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("cache-invalidation-rs on port {}", port);
    start_grpc_server("cache-invalidation-rs", 10337);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[cache-invalidation-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/invalidate", web::post().to(invalidate))
...
mod tests {
    ...
    #[test]
    fn test_invalidation_pattern_exists() {
        // Verify invalidation_pattern compiles and is callable
        // Domain function: invalidation_pattern(key: &str) -> String
        assert!(true, "invalidation_pattern should be defined");
    }

    #[test]
    fn test_invalidate_exists() {
        // Verify invalidate compiles and is callable
        // Domain function: invalidate(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "invalidate should be defined");
    }
    #[test]
