...
async fn collateral_gl(req: actix_web::HttpRequest) -> HttpResponse {
    ...
    _bus.emit("banking-clearing-ops.processed", &serde_json::json!({"status": "success"}));
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("banking.payments", "banking-clearing-ops");
    eprintln!("[banking-clearing-ops] Data flow initialized: topic=banking.payments");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "banking-clearing-ops-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "banking_clearing_ops");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "banking_clearing_ops");
    }
