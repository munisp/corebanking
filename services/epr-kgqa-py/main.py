...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[epr-kgqa-py] ready on :%d", PORT)
    logger.info(f"[epr-kgqa-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="epr-kgqa-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "epr-kgqa-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "epr-kgqa-py", "total_records": count}
    except Exception:
        return {"service": "epr-kgqa-py", "total_records": 0}
