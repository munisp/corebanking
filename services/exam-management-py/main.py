"""
exam-management-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("exam-management-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/exam_management_py")
...
PORT = int(os.environ.get("PORT", "9612"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[exam-management-py] ready on :%d", PORT)
    logger.info(f"[exam-management-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="exam-management-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "exam-management-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "exam-management-py", "total_records": count}
    except Exception:
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("exam-management-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("exam-management-py",))
...
# --- Domain Logic ---
def gen_id():
    return "EXA-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"exam_management_{self.path}"
        ...
        logger.info(f"[exam-management-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"service": "exam-management-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"exam-management-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"exam-management-py\"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if err:
            ...
            try:
                ...
                call_service("POST", f"{_upstream}/v1/screen", {"service": "exam-management-py", "action": "notify"})
            except Exception as _e:
                logger.warning(f"inter-service call failed: {_e}")
            return

        if path == "/v1/create":
            result = db_insert("exam_management_py", body)
            cache_set("exam_management:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/exam-management/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/exam-management/process":
            rid = body.get("id", "")
            for rec in records:
...
if __name__ == "__main__":
    ...
    logger.info(json.dumps({"service": "exam-management-py", "port": PORT, "message": "starting"}))
    try:
