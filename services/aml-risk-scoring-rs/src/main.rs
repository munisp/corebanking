...
async fn main() -> std::io::Result<()> {
    ...
    let db_name = "aml-risk-scoring-rs".replace("-", "_");
    ...
    log::info!("[aml-risk-scoring-rs] database connected, schema initialized");
    ...
    log::info!("[aml-risk-scoring-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8163".to_string()).parse().unwrap_or(8163);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[aml-risk-scoring-rs] ready on :{}", port);
