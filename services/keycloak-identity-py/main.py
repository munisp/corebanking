"""
keycloak-identity-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("keycloak-identity-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/keycloak_identity_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[keycloak-identity-py] ready on :%d", PORT)
    logger.info(f"[keycloak-identity-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="keycloak-identity-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "keycloak-identity-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "keycloak-identity-py", "total_records": count}
    except Exception:
        return {"service": "keycloak-identity-py", "total_records": 0}
