...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[sanctions-batch-rescreener-rs] starting");

    let db_name = "sanctions-batch-rescreener-rs".replace("-", "_");
    ...
    log::info!("[sanctions-batch-rescreener-rs] database connected, schema initialized");
    ...
    log::info!("[sanctions-batch-rescreener-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8494".to_string()).parse().unwrap_or(8494);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[sanctions-batch-rescreener-rs] ready on :{}", port);
...
async fn init_schema(pool: &PgPool) {
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_compliance_records_tenant ON compliance_records(tenant_id)")
        .execute(pool).await.ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_compliance_records_status ON compliance_records(status)")
        .execute(pool).await.ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_compliance_records_created ON compliance_records(created_at DESC)")
        .execute(pool).await.ok();
}
...
async fn list_records(data: web::Data<AppState>, req: actix_web::HttpRequest) -> HttpResponse {
    ...
    let rows = sqlx::query("SELECT id, status, created_at FROM compliance_records WHERE ($1 = '' OR tenant_id::text = $1) ORDER BY created_at DESC LIMIT 50")
        .bind(tenant_id)
        .fetch_all(&data.db)
        .await;
...
async fn create_record(data: web::Data<AppState>, body: web::Json<CreateRequest>, req: actix_web::HttpRequest) -> HttpResponse {
    ...
    let result = sqlx::query_scalar::<_, Uuid>(
        "INSERT INTO compliance_records (tenant_id, status) VALUES ($1::uuid, $2) RETURNING id"
    )
    .bind(&tenant_id)
    .bind(&status)
    .fetch_one(&data.db)
    ...
    match result {
        Ok(id) => {
            ...
            sqlx::query("INSERT INTO outbox (event_type, aggregate_id, payload) VALUES ($1, $2, $3)")
                .bind("compliance_records.created")
                .bind(id.to_string())
                .bind(&payload)
...
async fn get_record(data: web::Data<AppState>, path: web::Path<String>) -> HttpResponse {
    ...
    let result = sqlx::query("SELECT id, status, created_at FROM compliance_records WHERE id = $1::uuid")
        .bind(&id)
