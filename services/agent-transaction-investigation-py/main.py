...
def init_schema():
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS idx_transactions_tenant ON transactions(tenant_id)")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[agent-transaction-investigation-py] ready on :%d", PORT)
    logger.info(f"[agent-transaction-investigation-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="agent-transaction-investigation-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "agent-transaction-investigation-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "agent-transaction-investigation-py", "total_records": count}
    except Exception:
        return {"service": "agent-transaction-investigation-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        if x_tenant_id:
            cur.execute(
                "SELECT id, status, created_at FROM transactions WHERE tenant_id = %s::uuid ORDER BY created_at DESC LIMIT 50",
                (x_tenant_id,)
            )
        else:
            cur.execute("SELECT id, status, created_at FROM transactions ORDER BY created_at DESC LIMIT 50")
        rows = cur.fetchall()
...
def create_record(body: CreateRequest, x_tenant_id: Optional[str] = Header(None)):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO transactions (id, tenant_id, status) VALUES (%s::uuid, %s::uuid, %s)",
        ...
        cur.execute(
            ...
            ("transactions.created", record_id, payload)
        )
    conn.commit()
...
def get_record(record_id: str):
    conn = get_db()
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute("SELECT id, status, created_at FROM transactions WHERE id = %s::uuid", (record_id,))
        row = cur.fetchone()
    if not row:
...
@app.put("/api/v1/transactions/{record_id}")
def update_record(record_id: str, body: UpdateRequest):
    status = body.status or "updated"
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE transactions SET status = %s, updated_at = NOW() WHERE id = %s::uuid",
            (status, record_id)
        )
        payload = json.dumps({"id": record_id, "status": status})
        cur.execute(
            "INSERT INTO outbox (event_type, aggregate_id, payload) VALUES (%s, %s, %s::jsonb)",
            ("transactions.updated", record_id, payload)
        )
    conn.commit()
    return {"id": record_id, "status": status}


@app.delete("/api/v1/transactions/{record_id}", status_code=204)
def delete_record(record_id: str):
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute("UPDATE transactions SET status = 'deleted', updated_at = NOW() WHERE id = %s::uuid", (record_id,))
        payload = json.dumps({"id": record_id})
        cur.execute(
            "INSERT INTO outbox (event_type, aggregate_id, payload) VALUES (%s, %s, %s::jsonb)",
            ("transactions.deleted", record_id, payload)
        )
    conn.commit()
