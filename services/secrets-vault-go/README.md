...
54Bank Secrets Vault service

## Quick Start

```bash
# Build
go build -o secrets-vault-go .

# Run
PORT=8100 ./secrets-vault-go
```
...
```bash
docker build -t 54bank/secrets-vault-go .
docker run -p 8100:8100 54bank/secrets-vault-go
```
