...
// --- Centralized secrets management — dynamic credentials, transit encryption ---
...
func handleHealthz(w http.ResponseWriter, r *http.Request) {
	...
	count := len(records)
	recordsMu.RUnlock()
	respondJSON(w, 200, map[string]interface{}{"status": "healthy", "service": "secrets-vault-go", "version": "1.0.0", "records": count})
}
...
func main() {
	...
	mux.HandleFunc("/api/v1/secrets_vault", handleList)
	mux.HandleFunc("/api/v1/secrets_vault/create", handleCreate)
	log.Printf("Centralized secrets management — dynamic credentials, transit encryption listening on :%s", PORT)
