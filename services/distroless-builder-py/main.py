"""
distroless-builder-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("distroless-builder-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/distroless_builder_py")
...
PORT = int(os.environ.get("PORT", "9606"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[distroless-builder-py] ready on :%d", PORT)
    logger.info(f"[distroless-builder-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="distroless-builder-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "distroless-builder-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "distroless-builder-py", "total_records": count}
    except Exception:
        return {"service": "distroless-builder-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("distroless-builder-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("distroless-builder-py",))
...
# --- Domain Logic ---
def gen_id():
    return "DIS-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"distroless_builder_{self.path}"
        ...
        logger.info(f"[distroless-builder-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"service": "distroless-builder-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service="distroless-builder-py"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service="distroless-builder-py"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if err:
            ...
            try:
                ...
                call_service("POST", f"{_upstream}/v1/screen", {"service": "distroless-builder-py", "action": "notify"})
            except Exception as _e:
                logger.warning(f"inter-service call failed: {_e}")
            return

        if path == "/v1/create":
            result = db_insert("distroless_builder_py", body)
            cache_set("distroless_builder_py:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/distroless-builder/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/distroless-builder/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/distroless-builder/validate-config":
            result = validate_config(body.get("config", body))
            self.respond(200, result)
        elif path == "/v1/distroless-builder/resource-usage":
...
if __name__ == "__main__":
    ...
    logger.info(json.dumps({"service": "distroless-builder-py", "port": PORT, "message": "starting"}))
    try:
