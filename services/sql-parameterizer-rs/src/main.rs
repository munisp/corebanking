...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[sql-parameterizer-rs] starting");

    let db_name = "sql-parameterizer-rs".replace("-", "_");
    ...
    log::info!("[sql-parameterizer-rs] database connected, schema initialized");
    ...
    log::info!("[sql-parameterizer-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8175".to_string()).parse().unwrap_or(8175);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[sql-parameterizer-rs] ready on :{}", port);
