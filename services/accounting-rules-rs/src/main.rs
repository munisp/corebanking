...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[accounting-rules-rs] starting");

    let db_name = "accounting-rules-rs".replace("-", "_");
    ...
    log::info!("[accounting-rules-rs] database connected, schema initialized");
    ...
    log::info!("[accounting-rules-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8606".to_string()).parse().unwrap_or(8606);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[accounting-rules-rs] ready on :{}", port);
...
async fn init_schema(pool: &PgPool) {
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_accounts_tenant ON accounts(tenant_id)")
        .execute(pool).await.ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_accounts_status ON accounts(status)")
        .execute(pool).await.ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_accounts_created ON accounts(created_at DESC)")
        .execute(pool).await.ok();
}
