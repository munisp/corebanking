"""
backup-manager-py - Production-ready service with PostgreSQL persistence.
...
# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/backup_manager_py")
...
PORT = int(os.environ.get("PORT", "9592"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[backup-manager-py] ready on :%d", PORT)
    logger.info(f"[backup-manager-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="backup-manager-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "backup-manager-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "backup-manager-py", "total_records": count}
    except Exception:
        return {"service": "backup-manager-py", "total_records": 0}
...
# --- Domain Logic ---
def gen_id():
    return "BAC-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"service": "backup-manager-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service="backup-manager-py"}} {request_count}\n'
...
if __name__ == "__main__":
    ...
    logger.info(json.dumps({"service": "backup-manager-py", "port": PORT, "message": "starting"}))
    try:
