"""
load-test-runner-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("load-test-runner-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/load_test_runner_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[load-test-runner-py] ready on :%d", PORT)
    logger.info(f"[load-test-runner-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="load-test-runner-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "load-test-runner-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "load-test-runner-py", "total_records": count}
    except Exception:
        return {"service": "load-test-runner-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("load-test-runner-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("load-test-runner-py",))
...
# --- Domain Logic ---
def gen_id():
    return "LOA-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
SERVICE_NAME = "load-test-runner-py"
records: list = []
audit_log: list = []
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"load_test_runner_{self.path}"
        ...
        logger.info(f"[load-test-runner-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"service": "load-test-runner-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service="load-test-runner-py"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service="load-test-runner-py"}} {error_count}\n'
            )
        else:
...
def delete_record(record_id: str):
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            ...
            cache_set(f"load-test-runner-py:last_post", str(body))
        ...
        elif path == "/v1/load-test-runner/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/load-test-runner/health-checks":
            result = run_health_checks(body.get("services", []))
            self.respond(200, result)
        elif path == "/v1/load-test-runner/sla-metrics":
