...
"""
agent-customer-360-py — Customer 360 Agent — unified view of customer across all products, transactions, risk, and relationships
...
SERVICE_NAME = "agent-customer-360-py"
AGENT_TOOLS = ["core_banking", "neo4j_graph", "qdrant_search", "aml_engine", "gl_engine"]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "agent-customer-360-py", **_degrade.status()})
        elif path == "/v1/alerts":
    ...
    def do_POST(self):
        ...
        if path == "/v1/agent/customer-view":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            _event_bus.emit("agent-customer-360.processed", {"action": "process", "status": "success"})
            ...
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "customer_view", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/similar-customers":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "similar_customers", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/cross-sell":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "cross_sell", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
...
_event_bus = EventBus("platform.events", "agent-customer-360")
