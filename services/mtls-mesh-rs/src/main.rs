...
async fn verify_cert(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("mtls-mesh-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("mtls-mesh-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("mtls-mesh-rs: upstream call join failed: {}", e),
    }
