...
fn cert_valid(not_before: u64, not_after: u64) -> bool { let now = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs(); now >= not_before && now <= not_after }
...
async fn verify_cert(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "verify_cert"});
    db_persist(&state, "verify_cert", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("mtls-mesh-rs: upstream call ok"),
        Err(e) => eprintln!("mtls-mesh-rs: upstream call failed: {}", e),
    }
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "mtls-mesh-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"mtls-mesh-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"mtls-mesh-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "mtls_mesh_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("mtls-mesh-rs");
...
fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("mtls-mesh-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("mtls-mesh-rs", 10371);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[mtls-mesh-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    #[test]
    fn test_cert_valid_exists() {
        // Verify cert_valid compiles and is callable
        // Domain function: cert_valid(not_before: u64, not_after: u64) -> bool
        assert!(true, "cert_valid should be defined");
    }

    #[test]
    fn test_verify_cert_exists() {
        // Verify verify_cert compiles and is callable
        // Domain function: verify_cert(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "verify_cert should be defined");
    }
    #[test]
