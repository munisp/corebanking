...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[mtls-mesh-rs] starting");

    let db_name = "mtls-mesh-rs".replace("-", "_");
    ...
    log::info!("[mtls-mesh-rs] database connected, schema initialized");
    ...
    log::info!("[mtls-mesh-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8312".to_string()).parse().unwrap_or(8312);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[mtls-mesh-rs] ready on :{}", port);
