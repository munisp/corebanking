...
async fn process_stream(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "process_stream"});
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "fluvio-streams-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"fluvio-streams-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"fluvio-streams-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "fluvio_streams_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("fluvio-streams-rs");
...
fn main() -> std::io::Result<()> {
    ...
    start_grpc_server("fluvio-streams-rs", 10363);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[fluvio-streams-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    #[test]
    fn test_partition_key_exists() {
        // Verify partition_key compiles and is callable
        // Domain function: partition_key(key: &str, partitions: u32) -> u32
        assert!(true, "partition_key should be defined");
    }

    #[test]
    fn test_process_stream_exists() {
        // Verify process_stream compiles and is callable
        // Domain function: process_stream(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "process_stream should be defined");
    }
    #[test]
