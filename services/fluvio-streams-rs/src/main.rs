...
async fn health(state: web::Data<AppState>) -> HttpResponse {
    ...
    _bus.emit("fluvio-streams.processed", &serde_json::json!({"status": "success"}));
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("infra.events", "fluvio-streams");
    eprintln!("[fluvio-streams] Data flow initialized: topic=infra.events");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "fluvio-streams-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "fluvio_streams");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "fluvio_streams");
    }
