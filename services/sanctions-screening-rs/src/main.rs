...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[sanctions-screening-rs] starting");

    let db_name = "sanctions-screening-rs".replace("-", "_");
    ...
    log::info!("[sanctions-screening-rs] database connected, schema initialized");
    ...
    log::info!("[sanctions-screening-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8884".to_string()).parse().unwrap_or(8884);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[sanctions-screening-rs] ready on :{}", port);
