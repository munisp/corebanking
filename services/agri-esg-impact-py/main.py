"""
agri-esg-impact-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("agri-esg-impact-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/agri_esg_impact_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[agri-esg-impact-py] ready on :%d", PORT)
    logger.info(f"[agri-esg-impact-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="agri-esg-impact-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "agri-esg-impact-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "agri-esg-impact-py", "total_records": count}
    except Exception:
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("agri-esg-impact-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("agri-esg-impact-py",))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"agri_esg_impact_{self.path}"
        ...
        logger.info(f"[agri-esg-impact-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"service": "agri-esg-impact-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service="agri-esg-impact-py"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service="agri-esg-impact-py"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if err:
            ...
            try:
                ...
                call_service("POST", f"{_upstream}/v1/screen", {"service": "agri-esg-impact-py", "action": "notify"})
            except Exception as _e:
                logger.warning(f"inter-service call failed: {_e}")
            return

        if path == "/v1/create":
            result = db_insert("agri_esg_impact_py", body)
            cache_set("agri_esg_impact:last_post", str(body))
...
if __name__ == "__main__":
    ...
    logger.info(json.dumps({"service": "agri-esg-impact-py", "port": PORT, "message": "starting"}))
    try:
