...
logger = logging.getLogger("soc2-evidence-collector-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/soc2_evidence_collector_py")
...
PORT = int(os.environ.get("PORT", "9636"))
...
def init_schema():
    ...
    try:
        ...
        cur.execute("INSERT INTO records (data, service) VALUES (%s, %s) RETURNING id, created_at",
                    (data, "soc2-evidence-collector-py"))
        row = cur.fetchone()
        record["id"] = str(row[0])
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[soc2-evidence-collector-py] ready on :%d", PORT)
    logger.info(f"[soc2-evidence-collector-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="soc2-evidence-collector-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "soc2-evidence-collector-py", "version": "1.0.0"}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("soc2-evidence-collector-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("soc2-evidence-collector-py",))
...
# --- Domain Logic ---
def gen_id():
    return "SOC-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"soc2_evidence_collector_{self.path}"
        ...
        logger.info(f"[soc2-evidence-collector-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self._json(200, {"service": "soc2-evidence-collector-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"soc2-evidence-collector-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"soc2-evidence-collector-py\"}} {error_count}\n'
            )
        else:
            cur.execute("SELECT id, status, created_at FROM payments ORDER BY created_at DESC LIMIT 50")
        rows = cur.fetchall()
...
def delete_record(record_id: str):
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("soc2_evidence_collector_py", body)
            cache_set(f"{self.get_tenant_id()}:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/soc2-evidence-collector/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/soc2-evidence-collector/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/soc2-evidence-collector/assess":
            result = assess_security_posture(body.get("controls", []))
            self.respond(200, result)
        elif path == "/v1/soc2-evidence-collector/log-event":
