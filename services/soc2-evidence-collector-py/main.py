...
"""
soc2-evidence-collector-py — Production-hardened service
"""
import os
import sys
import json
    ...
    def do_GET(self):
        ...
        if path == "/v1/cache-metrics":
            ...
            if err:
                ...
                return
            items, total = db_query("soc2_evidence_collector_py")
...
_event_bus = EventBus("platform.events", "soc2-evidence-collector")


# --- Data Flow Emit Point ---
def emit_processing_event(action: str, data: dict) -> None:
    """Called by handlers after successful processing."""
    _event_bus.emit("soc2-evidence-collector." + action, data)

if __name__ == "__main__":
    get_db()
    server = HTTPServer(("0.0.0.0", PORT), Handler)
    logger.info(json.dumps({"service": "soc2-evidence-collector-py", "port": PORT, "message": "starting"}))
    try:
