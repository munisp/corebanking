...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[connection-pooler-rs] starting");

    let db_name = "connection-pooler-rs".replace("-", "_");
    ...
    log::info!("[connection-pooler-rs] database connected, schema initialized");
    ...
    log::info!("[connection-pooler-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8602".to_string()).parse().unwrap_or(8602);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[connection-pooler-rs] ready on :{}", port);
