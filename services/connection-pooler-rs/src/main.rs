...
// connection-pooler-rs — Database connection pooling
...
async fn pool_stats(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "pool_stats"});
    db_persist(&state, "pool_stats", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("connection-pooler-rs: upstream call ok"),
        Err(e) => eprintln!("connection-pooler-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "connection-pooler-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "connection-pooler-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"connection-pooler-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"connection-pooler-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "connection_pooler_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("connection-pooler-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    start_grpc_server("connection-pooler-rs", 10301);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[connection-pooler-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/pool_stats", web::post().to(pool_stats))
...
mod tests {
    ...
    #[test]
    fn test_optimal_pool_size_exists() {
        // Verify optimal_pool_size compiles and is callable
        // Domain function: optimal_pool_size(cpu_cores: u32) -> u32
        assert!(true, "optimal_pool_size should be defined");
    }

    #[test]
    fn test_pool_stats_exists() {
        // Verify pool_stats compiles and is callable
        // Domain function: pool_stats(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "pool_stats should be defined");
    }
    #[test]
