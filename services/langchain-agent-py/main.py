...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "langchain-agent-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "list_tools", "items": [], "source": "postgresql" if not _DB_URL else "postgres"})
    ...
    def do_POST(self):
        ...
        if path == "/v1/agent/query":
            ...
            _event_bus.emit("langchain-agent.processed", {"action": "process", "status": "success"})
...
_event_bus = EventBus("platform.events", "langchain-agent")
