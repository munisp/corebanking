"""
savings-products-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("savings-products-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/savings_products_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[savings-products-py] ready on :%d", PORT)
    logger.info(f"[savings-products-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="savings-products-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "savings-products-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "savings-products-py", "total_records": count}
    except Exception:
        return {"service": "savings-products-py", "total_records": 0}
