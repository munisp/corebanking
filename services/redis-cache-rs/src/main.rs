...
async fn health(state: web::Data<AppState>) -> HttpResponse {
    ...
    _bus.emit("redis-cache.processed", &serde_json::json!({"status": "success"}));
    HttpResponse::Ok().json(json!({
        "status": overall, "service": "redis-cache-rs",
...
async fn cache_get(req: actix_web::HttpRequest, path: web::Path<String>) -> HttpResponse {
    REQUEST_COUNT.fetch_add(1, AtomicOrdering::Relaxed);
    let key = path.into_inner();
    if let Ok(cache) = CACHE.read() {
        if let Some((value, created)) = cache.get(&key) {
            if created.elapsed() < Duration::from_secs(300) {
                return HttpResponse::Ok()
                    .insert_header(("X-Cache", "HIT"))
                    .json(json!({"key": key, "value": value, "ttl_remaining": 300 - created.elapsed().as_secs()}));
            }
        }
    }
    HttpResponse::NotFound()
        .insert_header(("X-Cache", "MISS"))
        .json(json!({"key": key, "error": "not_found"}))
}

async fn cache_set(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
    REQUEST_COUNT.fetch_add(1, AtomicOrdering::Relaxed);
    ...
    if key.is_empty() { return HttpResponse::BadRequest().json(json!({"error": "key required"})); }
    if let Ok(mut cache) = CACHE.write() {
        cache.insert(key.clone(), (value, Instant::now()));
    }
    HttpResponse::Ok().json(json!({"key": key, "status": "cached"}))
}

async fn cache_del(req: actix_web::HttpRequest, path: web::Path<String>) -> HttpResponse {
    REQUEST_COUNT.fetch_add(1, AtomicOrdering::Relaxed);
    if let Err(resp) = check_jwt(&req) { return resp; }
    let key = path.into_inner();
    if let Ok(mut cache) = CACHE.write() { cache.remove(&key); }
    HttpResponse::Ok().json(json!({"key": key, "status": "deleted"}))
}

async fn cache_stats() -> HttpResponse {
    let size = CACHE.read().map(|c| c.len()).unwrap_or(0);
    HttpResponse::Ok().json(json!({"entries": size, "max_ttl": 300, "eviction_policy": "ttl"}))
}
...
async fn healthz() -> HttpResponse {
    HttpResponse::Ok().json(serde_json::json!({"status": "healthy", "service": "redis-cache-rs"}))
}
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("infra.cache", "redis-cache");
    eprintln!("[redis-cache] Data flow initialized: topic=infra.cache");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "redis-cache-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "redis_cache");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "redis_cache");
    }
