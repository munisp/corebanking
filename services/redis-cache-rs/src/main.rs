...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[redis-cache-rs] starting");

    let db_name = "redis-cache-rs".replace("-", "_");
    ...
    log::info!("[redis-cache-rs] database connected, schema initialized");
    ...
    log::info!("[redis-cache-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8963".to_string()).parse().unwrap_or(8963);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[redis-cache-rs] ready on :{}", port);
