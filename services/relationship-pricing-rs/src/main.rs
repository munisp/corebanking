...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[relationship-pricing-rs] starting");

    let db_name = "relationship-pricing-rs".replace("-", "_");
    ...
    log::info!("[relationship-pricing-rs] database connected, schema initialized");
    ...
    log::info!("[relationship-pricing-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8549".to_string()).parse().unwrap_or(8549);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[relationship-pricing-rs] ready on :{}", port);
