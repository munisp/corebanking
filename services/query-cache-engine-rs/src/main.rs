...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[query-cache-engine-rs] starting");

    let db_name = "query-cache-engine-rs".replace("-", "_");
    ...
    log::info!("[query-cache-engine-rs] database connected, schema initialized");
    ...
    log::info!("[query-cache-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8534".to_string()).parse().unwrap_or(8534);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[query-cache-engine-rs] ready on :{}", port);
