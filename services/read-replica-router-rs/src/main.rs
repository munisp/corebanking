...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[read-replica-router-rs] starting");

    let db_name = "read-replica-router-rs".replace("-", "_");
    ...
    log::info!("[read-replica-router-rs] database connected, schema initialized");
    ...
    log::info!("[read-replica-router-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8216".to_string()).parse().unwrap_or(8216);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[read-replica-router-rs] ready on :{}", port);
