"""
tax-reporting-py - Production-ready service with PostgreSQL persistence.
...
# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/tax_reporting_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[tax-reporting-py] ready on :%d", PORT)
    logger.info(f"[tax-reporting-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="tax-reporting-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "tax-reporting-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "tax-reporting-py", "total_records": count}
    except Exception:
        return {"service": "tax-reporting-py", "total_records": 0}
