...
"""
tax-reporting-py — Production-hardened service
"""
import os
import sys
import json
...
logger = logging.getLogger("tax-reporting-py")
...
PORT = int(os.environ.get("PORT", "9639"))
...
def db_query(table, page=1, limit=50):
    ...
    try:
        ...
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("tax-reporting-py",))
...
# --- Domain Logic ---
def gen_id():
    return "TAX-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"tax_reporting_{self.path}"
        ...
        logger.info(f"[tax-reporting-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "tax-reporting-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"tax-reporting-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"tax-reporting-py\"}} {error_count}\n'
            )
            self.send_response(200)
            ...
            if err:
                ...
                return
            items, total = db_query("tax_reporting_py")
    ...
    def do_POST(self):
        ...
        elif path == "/v1/tax-reporting/process":
            rid = body.get("id", "")
            for rec in records:
...
_event_bus = EventBus("platform.events", "tax-reporting")


# --- Data Flow Emit Point ---
def emit_processing_event(action: str, data: dict) -> None:
    """Called by handlers after successful processing."""
    _event_bus.emit("tax-reporting." + action, data)

if __name__ == "__main__":
    get_db()
    server = HTTPServer(("0.0.0.0", PORT), Handler)
    logger.info(json.dumps({"service": "tax-reporting-py", "port": PORT, "message": "starting"}))
    try:
