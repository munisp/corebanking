...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[bulk-payments-rs] starting");

    let db_name = "bulk-payments-rs".replace("-", "_");
    ...
    log::info!("[bulk-payments-rs] database connected, schema initialized");
    ...
    log::info!("[bulk-payments-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8839".to_string()).parse().unwrap_or(8839);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[bulk-payments-rs] ready on :{}", port);
