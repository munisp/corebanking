...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[response-compressor-rs] starting");

    let db_name = "response-compressor-rs".replace("-", "_");
    ...
    log::info!("[response-compressor-rs] database connected, schema initialized");
    ...
    log::info!("[response-compressor-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8121".to_string()).parse().unwrap_or(8121);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[response-compressor-rs] ready on :{}", port);
