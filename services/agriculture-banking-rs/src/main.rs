...
async fn main() -> std::io::Result<()> {
    ...
    let db_name = "agriculture-banking-rs".replace("-", "_");
    ...
    log::info!("[agriculture-banking-rs] database connected, schema initialized");
    ...
    log::info!("[agriculture-banking-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8578".to_string()).parse().unwrap_or(8578);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[agriculture-banking-rs] ready on :{}", port);
