...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[event-dedup-engine-rs] starting");

    let db_name = "event-dedup-engine-rs".replace("-", "_");
    ...
    log::info!("[event-dedup-engine-rs] database connected, schema initialized");
    ...
    log::info!("[event-dedup-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8896".to_string()).parse().unwrap_or(8896);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[event-dedup-engine-rs] ready on :{}", port);
