...
```bash
# Build
go build -o sms-email-gateway-go .

# Run
PORT=6379 ./sms-email-gateway-go
```
...
```bash
docker build -t 54bank/sms-email-gateway-go .
docker run -p 6379:6379 54bank/sms-email-gateway-go
```
