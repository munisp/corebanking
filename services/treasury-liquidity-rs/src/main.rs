...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[treasury-liquidity-rs] starting");

    let db_name = "treasury-liquidity-rs".replace("-", "_");
    ...
    log::info!("[treasury-liquidity-rs] database connected, schema initialized");
    ...
    log::info!("[treasury-liquidity-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8400".to_string()).parse().unwrap_or(8400);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[treasury-liquidity-rs] ready on :{}", port);
