...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[telegram-kyc-bot-rs] starting");

    let db_name = "telegram-kyc-bot-rs".replace("-", "_");
    ...
    log::info!("[telegram-kyc-bot-rs] database connected, schema initialized");
    ...
    log::info!("[telegram-kyc-bot-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8702".to_string()).parse().unwrap_or(8702);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[telegram-kyc-bot-rs] ready on :{}", port);
