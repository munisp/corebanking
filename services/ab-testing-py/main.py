"""
ab-testing-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("ab-testing-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/ab_testing_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[ab-testing-py] ready on :%d", PORT)
    logger.info(f"[ab-testing-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="ab-testing-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "ab-testing-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "ab-testing-py", "total_records": count}
    except Exception:
        return {"service": "ab-testing-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("ab-testing-py",))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"ab_testing_{self.path}"
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "ab-testing-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"ab-testing-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"ab-testing-py\"}} {error_count}\n'
            )
        else:
