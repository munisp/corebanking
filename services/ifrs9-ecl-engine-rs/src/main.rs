...
fn init_data_flow() -> EventBus {
    ...
    log::info!("[ifrs9-ecl-engine] Data flow initialized: topic=risk.computation");
    bus
}
...
fn init_consumer() -> EventConsumer {
    let consumer = EventConsumer::new(&["accounting.ledger", "banking.lending"], "ifrs9-ecl-engine");
    consumer.subscribe();
    consumer
}
