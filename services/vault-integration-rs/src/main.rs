...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[vault-integration-rs] starting");

    let db_name = "vault-integration-rs".replace("-", "_");
    ...
    log::info!("[vault-integration-rs] database connected, schema initialized");
    ...
    log::info!("[vault-integration-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8926".to_string()).parse().unwrap_or(8926);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[vault-integration-rs] ready on :{}", port);
