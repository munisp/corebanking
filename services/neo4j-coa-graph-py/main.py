"""
neo4j-coa-graph-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("neo4j-coa-graph-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/neo4j_coa_graph_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[neo4j-coa-graph-py] ready on :%d", PORT)
    logger.info(f"[neo4j-coa-graph-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="neo4j-coa-graph-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "neo4j-coa-graph-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "neo4j-coa-graph-py", "total_records": count}
    except Exception:
        return {"service": "neo4j-coa-graph-py", "total_records": 0}
