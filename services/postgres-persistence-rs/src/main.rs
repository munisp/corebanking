...
// postgres-persistence-rs — PostgreSQL persistence layer
...
async fn persist(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "persist"});
    db_persist(&state, "persist", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("postgres-persistence-rs: upstream call ok"),
        Err(e) => eprintln!("postgres-persistence-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "postgres-persistence-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "postgres-persistence-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"postgres-persistence-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"postgres-persistence-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "postgres_persistence_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("postgres-persistence-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8238);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("postgres-persistence-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("postgres-persistence-rs on port {}", port);
    start_grpc_server("postgres-persistence-rs", 10378);
    HttpServer::new(move || {
        App::new()
            .wrap(
                Cors::default()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[postgres-persistence-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/persist", web::post().to(persist))
...
mod tests {
    ...
    #[test]
    fn test_build_upsert_sql_exists() {
        // Verify build_upsert_sql compiles and is callable
        // Domain function: build_upsert_sql(table: &str, columns: &[&str]) -> String
        assert!(true, "build_upsert_sql should be defined");
    }

    #[test]
    fn test_persist_exists() {
        // Verify persist compiles and is callable
        // Domain function: persist(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "persist should be defined");
    }
    #[test]
