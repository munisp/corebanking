...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[continuous-liveness-rs] starting");

    let db_name = "continuous-liveness-rs".replace("-", "_");
    ...
    log::info!("[continuous-liveness-rs] database connected, schema initialized");
    ...
    log::info!("[continuous-liveness-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8636".to_string()).parse().unwrap_or(8636);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[continuous-liveness-rs] ready on :{}", port);
