...
func TestJWTRequired(t *testing.T) {
	...
	w := httptest.NewRecorder()
	handler := jwtAuthMiddleware(http.HandlerFunc((&ExpenseService{}).listItems))
...
func TestRateLimiting(t *testing.T) {
	for i := 0; i < 200; i++ {
		req := httptest.NewRequest("GET", "/healthz", nil)
		w := httptest.NewRecorder()
		rateLimitMiddleware(http.HandlerFunc((&ExpenseService{}).healthz)).ServeHTTP(w, req)
	}
}
