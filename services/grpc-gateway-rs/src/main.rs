...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[grpc-gateway-rs] starting");

    let db_name = "grpc-gateway-rs".replace("-", "_");
    ...
    log::info!("[grpc-gateway-rs] database connected, schema initialized");
    ...
    log::info!("[grpc-gateway-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8362".to_string()).parse().unwrap_or(8362);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[grpc-gateway-rs] ready on :{}", port);
