...
// grpc-gateway-rs — gRPC to REST gateway
...
async fn transcode(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "transcode"});
    db_persist(&state, "transcode", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("grpc-gateway-rs: upstream call ok"),
        Err(e) => eprintln!("grpc-gateway-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "grpc-gateway-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "grpc-gateway-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"grpc-gateway-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"grpc-gateway-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "grpc_gateway_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("grpc-gateway-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("grpc-gateway-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("grpc-gateway-rs on port {}", port);
    start_grpc_server("grpc-gateway-rs", 10304);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[grpc-gateway-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/transcode", web::post().to(transcode))
...
mod tests {
    ...
    #[test]
    fn test_grpc_status_to_http_exists() {
        // Verify grpc_status_to_http compiles and is callable
        // Domain function: grpc_status_to_http(code: u32) -> u16
        assert!(true, "grpc_status_to_http should be defined");
    }

    #[test]
    fn test_transcode_exists() {
        // Verify transcode compiles and is callable
        // Domain function: transcode(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "transcode should be defined");
    }
    #[test]
