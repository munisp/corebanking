...
// route-trie-optimizer-rs — URL route trie optimizer
...
async fn optimize_routes(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "optimize_routes"});
    db_persist(&state, "optimize_routes", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("route-trie-optimizer-rs: upstream call ok"),
        Err(e) => eprintln!("route-trie-optimizer-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "route-trie-optimizer-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "route-trie-optimizer-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"route-trie-optimizer-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"route-trie-optimizer-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "route_trie_optimizer_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("route-trie-optimizer-rs");
...
fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8246);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("route-trie-optimizer-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("route-trie-optimizer-rs on port {}", port);
    start_grpc_server("route-trie-optimizer-rs", 10440);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[route-trie-optimizer-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/optimize_routes", web::post().to(optimize_routes))
...
mod tests {
    ...
    #[test]
    fn test_trie_depth_exists() {
        // Verify trie_depth compiles and is callable
        // Domain function: trie_depth(path: &str) -> u32
        assert!(true, "trie_depth should be defined");
    }

    #[test]
    fn test_optimize_routes_exists() {
        // Verify optimize_routes compiles and is callable
        // Domain function: optimize_routes(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "optimize_routes should be defined");
    }
    #[test]
