...
async fn loading_state(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("skeleton-loading-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("skeleton-loading-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("skeleton-loading-rs: upstream call join failed: {}", e),
    }
