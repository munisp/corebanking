...
// skeleton-loading-rs — Skeleton loading state management
...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("skeleton-loading.processed", &serde_json::json!({"status": "success"}));
...
async fn loading_state(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "loading_state"});
    db_persist(&state, "loading_state", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("skeleton-loading-rs: upstream call ok"),
        Err(e) => eprintln!("skeleton-loading-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "skeleton-loading-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "skeleton-loading-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"skeleton-loading-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"skeleton-loading-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "skeleton_loading_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("skeleton-loading-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8250);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("skeleton-loading-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("skeleton-loading-rs on port {}", port);
    start_grpc_server("skeleton-loading-rs", 10487);
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[skeleton-loading-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/loading_state", web::post().to(loading_state))
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("platform.general", "skeleton-loading");
    eprintln!("[skeleton-loading] Data flow initialized: topic=platform.general");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "skeleton-loading-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "skeleton_loading");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "skeleton_loading");
    }
