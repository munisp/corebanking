...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[skeleton-loading-rs] starting");

    let db_name = "skeleton-loading-rs".replace("-", "_");
    ...
    log::info!("[skeleton-loading-rs] database connected, schema initialized");
    ...
    log::info!("[skeleton-loading-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8677".to_string()).parse().unwrap_or(8677);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[skeleton-loading-rs] ready on :{}", port);
