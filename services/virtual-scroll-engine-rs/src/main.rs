...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[virtual-scroll-engine-rs] starting");

    let db_name = "virtual-scroll-engine-rs".replace("-", "_");
    ...
    log::info!("[virtual-scroll-engine-rs] database connected, schema initialized");
    ...
    log::info!("[virtual-scroll-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8152".to_string()).parse().unwrap_or(8152);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[virtual-scroll-engine-rs] ready on :{}", port);
