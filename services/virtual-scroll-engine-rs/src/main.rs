...
async fn compute_window(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("virtual-scroll-engine-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("virtual-scroll-engine-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("virtual-scroll-engine-rs: upstream call join failed: {}", e),
    }
...
mod tests {
    ...
    #[test]
    fn test_compute_window_exists() {
        // Verify compute_window compiles and is callable
        // Domain function: compute_window(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "compute_window should be defined");
    }
    #[test]
