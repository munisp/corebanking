"""
docker-hardener-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("docker-hardener-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/docker_hardener_py")
...
PORT = int(os.environ.get("PORT", "9607"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[docker-hardener-py] ready on :%d", PORT)
    logger.info(f"[docker-hardener-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="docker-hardener-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "docker-hardener-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "docker-hardener-py", "total_records": count}
    except Exception:
        return {"service": "docker-hardener-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("docker-hardener-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("docker-hardener-py",))
...
# --- Domain Logic ---
def gen_id():
    return "DOC-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"docker_hardener_{self.path}"
        ...
        logger.info(f"[docker-hardener-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"service": "docker-hardener-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service="docker-hardener-py"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service="docker-hardener-py"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if err:
            ...
            try:
                ...
                call_service("POST", f"{_upstream}/v1/screen", {"service": "docker-hardener-py", "action": "notify"})
            except Exception as _e:
                logger.warning(f"inter-service call failed: {_e}")
            return

        if path == "/v1/create":
            result = db_insert("docker_hardener_py", body)
            cache_set("docker_hardener_py:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/docker-hardener/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/docker-hardener/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/docker-hardener/validate-config":
            result = validate_config(body.get("config", body))
            self.respond(200, result)
        elif path == "/v1/docker-hardener/resource-usage":
...
if __name__ == "__main__":
    ...
    logger.info(json.dumps({"service": "docker-hardener-py", "port": PORT, "message": "starting"}))
    try:
