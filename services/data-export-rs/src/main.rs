...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[data-export-rs] starting");

    let db_name = "data-export-rs".replace("-", "_");
    ...
    log::info!("[data-export-rs] database connected, schema initialized");
    ...
    log::info!("[data-export-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8724".to_string()).parse().unwrap_or(8724);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[data-export-rs] ready on :{}", port);
