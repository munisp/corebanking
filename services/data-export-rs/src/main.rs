...
// data-export-rs — Data export service (CSV, JSON, Parquet)
...
async fn export_data(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "export_data"});
    db_persist(&state, "export_data", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("data-export-rs: upstream call ok"),
        Err(e) => eprintln!("data-export-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "data-export-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "data-export-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"data-export-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"data-export-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "data_export_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("data-export-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8267);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("data-export-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("data-export-rs on port {}", port);
    start_grpc_server("data-export-rs", 10351);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[data-export-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/export_data", web::post().to(export_data))
...
mod tests {
    ...
    #[test]
    fn test_estimate_export_size_exists() {
        // Verify estimate_export_size compiles and is callable
        // Domain function: estimate_export_size(rows: u64, avg_row_bytes: u64) -> u64
        assert!(true, "estimate_export_size should be defined");
    }

    #[test]
    fn test_export_data_exists() {
        // Verify export_data compiles and is callable
        // Domain function: export_data(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "export_data should be defined");
    }
    #[test]
