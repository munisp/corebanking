...
class AccountLookupError(Exception):
    """Raised when account metadata cannot be retrieved from account-service.

    Treated as non-retryable in the transaction recording path: the event is
    dead-lettered for manual review rather than silently creating a phantom
    transaction with missing account metadata.
    """
