...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("gl-engine.processed", &serde_json::json!({"status": "success"}));
...
fn init_data_flow() -> EventBus {
    ...
    log::info!("[gl-engine] Data flow initialized: topic=accounting.ledger");
    bus
}
...
fn init_consumer() -> EventConsumer {
    let consumer = EventConsumer::new(&["banking.payments", "banking.lending", "treasury.operations"], "gl-engine");
    consumer.subscribe();
    consumer
}
