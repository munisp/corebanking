...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[gl-engine-rs] starting");

    let db_name = "gl-engine-rs".replace("-", "_");
    ...
    log::info!("[gl-engine-rs] database connected, schema initialized");
    ...
    log::info!("[gl-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8919".to_string()).parse().unwrap_or(8919);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[gl-engine-rs] ready on :{}", port);
