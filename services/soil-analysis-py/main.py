"""
soil-analysis-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("soil-analysis-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/soil_analysis_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[soil-analysis-py] ready on :%d", PORT)
    logger.info(f"[soil-analysis-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="soil-analysis-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "soil-analysis-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "soil-analysis-py", "total_records": count}
    except Exception:
        return {"service": "soil-analysis-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("soil-analysis-py",))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"soil_analysis_{self.path}"
        ...
        logger.info(f"[soil-analysis-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "soil-analysis-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"soil-analysis-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"soil-analysis-py\"}} {error_count}\n'
            )
        else:
...
def delete_record(record_id: str):
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("soil_analysis_py", body)
