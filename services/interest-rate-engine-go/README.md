...
```bash
# Build
go build -o interest-rate-engine-go .

# Run
PORT=6379 ./interest-rate-engine-go
```
...
```bash
docker build -t 54bank/interest-rate-engine-go .
docker run -p 6379:6379 54bank/interest-rate-engine-go
```
