...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[risk-scoring-rs] starting");

    let db_name = "risk-scoring-rs".replace("-", "_");
    ...
    log::info!("[risk-scoring-rs] database connected, schema initialized");
    ...
    log::info!("[risk-scoring-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8321".to_string()).parse().unwrap_or(8321);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[risk-scoring-rs] ready on :{}", port);
