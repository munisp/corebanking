...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[sri-validator-rs] starting");

    let db_name = "sri-validator-rs".replace("-", "_");
    ...
    log::info!("[sri-validator-rs] database connected, schema initialized");
    ...
    log::info!("[sri-validator-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8011".to_string()).parse().unwrap_or(8011);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[sri-validator-rs] ready on :{}", port);
