"""
push-notification-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("push-notification-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/push_notification_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[push-notification-py] ready on :%d", PORT)
    logger.info(f"[push-notification-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="push-notification-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "push-notification-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "push-notification-py", "total_records": count}
    except Exception:
        return {"service": "push-notification-py", "total_records": 0}
