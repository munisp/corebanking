...
"""
agent-fraud-detection-py — Fraud Pattern Detection Agent — detects structuring, mule networks, and unusual patterns across the graph
...
SERVICE_NAME = "agent-fraud-detection-py"
AGENT_TOOLS = ["aml_engine", "neo4j_graph", "qdrant_search", "falkordb"]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "agent-fraud-detection-py", **_degrade.status()})
        elif path == "/v1/alerts":
        ...
        elif path == "/v1/agent/alerts":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "active_alerts", "description": "List active fraud alerts", "tools": AGENT_TOOLS})
    ...
    def do_POST(self):
        ...
        if path == "/v1/agent/detect-patterns":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            _event_bus.emit("agent-fraud-detection.processed", {"action": "process", "status": "success"})
            ...
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "detect_patterns", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/network-analysis":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "network_analysis", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/risk-score":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "risk_score", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
...
_event_bus = EventBus("compliance.screening", "agent-fraud-detection")
