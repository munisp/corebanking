...
```bash
# Build
go build -o pgbouncer-manager-go .

# Run
PORT=6379 ./pgbouncer-manager-go
```
...
```bash
docker build -t 54bank/pgbouncer-manager-go .
docker run -p 6379:6379 54bank/pgbouncer-manager-go
```
