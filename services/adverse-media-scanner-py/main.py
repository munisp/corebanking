"""
adverse-media-scanner-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("adverse-media-scanner-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/adverse_media_scanner_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[adverse-media-scanner-py] ready on :%d", PORT)
    logger.info(f"[adverse-media-scanner-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="adverse-media-scanner-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "adverse-media-scanner-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "adverse-media-scanner-py", "total_records": count}
    except Exception:
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("adverse-media-scanner-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("adverse-media-scanner-py",))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"adverse_media_scanner_{self.path}"
        ...
        logger.info(f"[adverse-media-scanner-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self._json(200, {"service": "adverse-media-scanner-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"adverse-media-scanner-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"adverse-media-scanner-py\"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("adverse_media_scanner_py", body)
