...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[langchain-agent-rs] starting");

    let db_name = "langchain-agent-rs".replace("-", "_");
    ...
    log::info!("[langchain-agent-rs] database connected, schema initialized");
    ...
    log::info!("[langchain-agent-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8848".to_string()).parse().unwrap_or(8848);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[langchain-agent-rs] ready on :{}", port);
