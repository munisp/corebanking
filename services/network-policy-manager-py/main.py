"""
network-policy-manager-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("network-policy-manager-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/network_policy_manager_py")
...
PORT = int(os.environ.get("PORT", "9623"))
...
def init_schema():
    ...
    try:
        ...
        cur.execute("INSERT INTO records (data, service) VALUES (%s, %s) RETURNING id, created_at",
                    (data, "network-policy-manager-py"))
        row = cur.fetchone()
        record["id"] = str(row[0])
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[network-policy-manager-py] ready on :%d", PORT)
    logger.info(f"[network-policy-manager-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="network-policy-manager-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "network-policy-manager-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "network-policy-manager-py", "total_records": count}
    except Exception:
        return {"service": "network-policy-manager-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("network-policy-manager-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("network-policy-manager-py",))
...
# --- Domain Logic ---
def gen_id():
    return "NET-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"network_policy_manager_{self.path}"
        ...
        logger.info(f"[network-policy-manager-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "network-policy-manager-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"network-policy-manager-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"network-policy-manager-py\"}} {error_count}\n'
            )
        else:
...
def delete_record(record_id: str):
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("network_policy_manager_py", body)
            cache_set(f"{self.get_tenant_id()}:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/network-policy-manager/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/network-policy-manager/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/network-policy-manager/validate-config":
            result = validate_config(body.get("config", body))
            self.respond(200, result)
        elif path == "/v1/network-policy-manager/resource-usage":
