...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[tigerbeetle-batch-engine-rs] starting");

    let db_name = "tigerbeetle-batch-engine-rs".replace("-", "_");
    ...
    log::info!("[tigerbeetle-batch-engine-rs] database connected, schema initialized");
    ...
    log::info!("[tigerbeetle-batch-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8057".to_string()).parse().unwrap_or(8057);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[tigerbeetle-batch-engine-rs] ready on :{}", port);
