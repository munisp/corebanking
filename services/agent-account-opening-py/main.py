...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[agent-account-opening-py] ready on :%d", PORT)
    logger.info(f"[agent-account-opening-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="agent-account-opening-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "agent-account-opening-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "agent-account-opening-py", "total_records": count}
    except Exception:
        return {"service": "agent-account-opening-py", "total_records": 0}
