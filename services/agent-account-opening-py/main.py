...
"""
agent-account-opening-py — Conversational Account Opening Agent — orchestrates KYC, BVN validation, product selection, GL mapping
...
SERVICE_NAME = "agent-account-opening-py"
AGENT_TOOLS = ["core_banking", "kyc", "gl_engine", "aml_engine"]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "agent-account-opening-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "agent_status", "description": "Agent status and capabilities", "tools": AGENT_TOOLS})
    ...
    def do_POST(self):
        ...
        if path == "/v1/agent/open-account":
            ...
            _event_bus.emit("agent-account-opening.processed", {"action": "process", "status": "success"})
            ...
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "open_account", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/verify-customer":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "verify_customer", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/recommend-product":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "recommend_product", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
...
_event_bus = EventBus("banking.accounts", "agent-account-opening")
