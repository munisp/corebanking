...
async fn rollout_check(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "rollout_check"});
    db_persist(&state, "rollout_check", &_result_data).await;
    // Inter-service call
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("graduated-rollout-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("graduated-rollout-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("graduated-rollout-rs: upstream call join failed: {}", e),
    }
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "graduated-rollout-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"graduated-rollout-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"graduated-rollout-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "graduated_rollout_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("graduated-rollout-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("graduated-rollout-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("graduated-rollout-rs", 10377);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[graduated-rollout-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    #[test]
    fn test_rollout_eligible_exists() {
        // Verify rollout_eligible compiles and is callable
        // Domain function: rollout_eligible(user_hash: u32, current_pct: u32) -> bool
        assert!(true, "rollout_eligible should be defined");
    }

    #[test]
    fn test_rollout_check_exists() {
        // Verify rollout_check compiles and is callable
        // Domain function: rollout_check(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "rollout_check should be defined");
    }
    #[test]
