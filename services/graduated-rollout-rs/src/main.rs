...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[graduated-rollout-rs] starting");

    let db_name = "graduated-rollout-rs".replace("-", "_");
    ...
    log::info!("[graduated-rollout-rs] database connected, schema initialized");
    ...
    log::info!("[graduated-rollout-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8315".to_string()).parse().unwrap_or(8315);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[graduated-rollout-rs] ready on :{}", port);
