...
async fn check_dormancy(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let last_txn_days = input.get("last_txn_days").and_then(|v| v.as_u64()).unwrap_or(0) as u32;
    let result = dormancy_status(last_txn_days);
    let _result_data = json!({"endpoint": "check_dormancy"});
    db_persist(&state, "check_dormancy", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("dormancy-management-rs: upstream call ok"),
        Err(e) => eprintln!("dormancy-management-rs: upstream call failed: {}", e),
    }
...
async fn activate_dormant(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    if !rl_allow() {
        return HttpResponse::TooManyRequests().json(json!({"error": "rate_limit_exceeded"}));
    }
    if let Err(resp) = check_jwt(&req) { return resp; }
    let input = body.into_inner();
    let status_s = input.get("status").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let status = status_s.as_str();
    let result = restriction_level(status);
    let _result_data = json!({"endpoint": "activate_dormant"});
    db_persist(&state, "activate_dormant", &_result_data).await;
...
async fn unclaimed_funds(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    if !rl_allow() {
        return HttpResponse::TooManyRequests().json(json!({"error": "rate_limit_exceeded"}));
    }
    if let Err(resp) = check_jwt(&req) { return resp; }
    ...
    let result = cbn_unclaimed_threshold_years();
    let _result_data = json!({"endpoint": "unclaimed_funds"});
    db_persist(&state, "unclaimed_funds", &_result_data).await;
...
async fn list_records(req: actix_web::HttpRequest, state: web::Data<AppState>, query: web::Query<std::collections::HashMap<String, String>>) -> HttpResponse {
    ...
    if let Some(ref client) = state.db_client {
        match client.query(
            ...
            &[&"dormancy_management_rs", &(limit as i64), &(offset as i64)]
        ).await {
            Ok(rows) => {
                ...
                let total: i64 = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"dormancy_management_rs"]).await.map(|r| r.get(0)).unwrap_or(0);
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    if let Some(ref client) = state.db_client {
        if let Ok(row) = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"dormancy_management_rs"]).await {
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "dormancy_management_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("dormancy-management-rs");
...
fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("dormancy-management-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("dormancy-management-rs", 10384);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[dormancy-management-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    #[test]
    fn test_cbn_unclaimed_threshold_years_exists() {
        // Verify cbn_unclaimed_threshold_years compiles and is callable
        // Domain function: cbn_unclaimed_threshold_years() -> u32
        assert!(true, "cbn_unclaimed_threshold_years should be defined");
    }

    #[test]
    fn test_reactivation_requirements_exists() {
        // Verify reactivation_requirements compiles and is callable
        // Domain function: reactivation_requirements(status: &str) -> Vec
        assert!(true, "reactivation_requirements should be defined");
    }

    #[test]
    fn test_check_dormancy_exists() {
        // Verify check_dormancy compiles and is callable
        // Domain function: check_dormancy(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "check_dormancy should be defined");
    }

    #[test]
    fn test_activate_dormant_exists() {
        // Verify activate_dormant compiles and is callable
        // Domain function: activate_dormant(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "activate_dormant should be defined");
    }
    #[test]
