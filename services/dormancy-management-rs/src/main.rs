...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[dormancy-management-rs] starting");

    let db_name = "dormancy-management-rs".replace("-", "_");
    ...
    log::info!("[dormancy-management-rs] database connected, schema initialized");
    ...
    log::info!("[dormancy-management-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8327".to_string()).parse().unwrap_or(8327);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[dormancy-management-rs] ready on :{}", port);
