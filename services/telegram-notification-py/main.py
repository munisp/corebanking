"""
telegram-notification-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("telegram-notification-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/telegram_notification_py")
...
PORT = int(os.environ.get("PORT", "9640"))
...
def init_schema():
    ...
    try:
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS idx_notifications_status ON notifications(status)")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[telegram-notification-py] ready on :%d", PORT)
    logger.info(f"[telegram-notification-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="telegram-notification-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "telegram-notification-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "telegram-notification-py", "total_records": count}
    except Exception:
        return {"service": "telegram-notification-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("telegram-notification-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("telegram-notification-py",))
...
# --- Domain Logic ---
def gen_id():
    return "TEL-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))


def now_iso():
    return time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())

def format_telegram_message(template, params):
    message = template
    for key, val in params.items():
        placeholder = "{" + key + "}"
        message = message.replace(placeholder, str(val))
    return {"channel": "telegram", "message": message, "params": params, "formatted_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "length": len(message)}

def batch_send_telegram(recipients, message):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"telegram_notification_{self.path}"
        ...
        logger.info(f"[telegram-notification-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "telegram-notification-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"telegram-notification-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"telegram-notification-py\"}} {error_count}\n'
            )
        else:
            cur.execute("SELECT id, status, created_at FROM notifications ORDER BY created_at DESC LIMIT 50")
        rows = cur.fetchall()
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("telegram_notification_py", body)
            _batch_send_telegram_result = batch_send_telegram(body.get("data", {}))
            cache_set(f"{self.get_tenant_id()}:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/telegram-notification/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/telegram-notification/process":
            rid = body.get("id", "")
            for rec in records:
            ...
            result = format_telegram_message(**body) if isinstance(body, dict) else format_telegram_message(body)
            self.respond(200, {"service": "telegram-notification-py", "result": result})
...
def create_record(body: CreateRequest, x_tenant_id: Optional[str] = Header(None)):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO notifications (id, tenant_id, status) VALUES (%s::uuid, %s::uuid, %s)",
        ...
        cur.execute(
            ...
            ("notifications.created", record_id, payload)
        )
    conn.commit()
