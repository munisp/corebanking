...
```bash
# Build
go build -o redis-session-store-go .

# Run
PORT=6379 ./redis-session-store-go
```
...
```bash
docker build -t 54bank/redis-session-store-go .
docker run -p 6379:6379 54bank/redis-session-store-go
```
