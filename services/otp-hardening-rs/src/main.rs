...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[otp-hardening-rs] starting");

    let db_name = "otp-hardening-rs".replace("-", "_");
    ...
    log::info!("[otp-hardening-rs] database connected, schema initialized");
    ...
    log::info!("[otp-hardening-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8030".to_string()).parse().unwrap_or(8030);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[otp-hardening-rs] ready on :{}", port);
