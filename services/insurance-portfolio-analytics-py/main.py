"""
insurance-portfolio-analytics-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("insurance-portfolio-analytics-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/insurance_portfolio_analytics_py")
...
PORT = int(os.environ.get("PORT", "9614"))
...
def init_schema():
    ...
    try:
        ...
        cur.execute("INSERT INTO records (data, service) VALUES (%s, %s) RETURNING id, created_at",
                    (data, "insurance-portfolio-analytics-py"))
        row = cur.fetchone()
        record["id"] = str(row[0])
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[insurance-portfolio-analytics-py] ready on :%d", PORT)
    logger.info(f"[insurance-portfolio-analytics-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="insurance-portfolio-analytics-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "insurance-portfolio-analytics-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "insurance-portfolio-analytics-py", "total_records": count}
    except Exception:
        return {"service": "insurance-portfolio-analytics-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("insurance-portfolio-analytics-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("insurance-portfolio-analytics-py",))
...
# --- Domain Logic ---
def gen_id():
    return "INS-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"insurance_portfolio_analytics_{self.path}"
        ...
        logger.info(f"[insurance-portfolio-analytics-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"service": "insurance-portfolio-analytics-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service="insurance-portfolio-analytics-py"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service="insurance-portfolio-analytics-py"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if err:
            ...
            try:
                ...
                call_service("POST", f"{_upstream}/v1/screen", {"service": "insurance-portfolio-analytics-py", "action": "notify"})
        ...
        if path == "/v1/create":
            ...
            cache_set("insurance_portfolio:last_post", str(body))
        ...
        elif path == "/v1/insurance-portfolio-analytics/process":
            rid = body.get("id", "")
            for rec in records:
...
if __name__ == "__main__":
    ...
    logger.info(json.dumps({"service": "insurance-portfolio-analytics-py", "port": PORT, "message": "starting"}))
    try:
