...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "insurance-portfolio-analytics-py", **_degrade.status()})
        elif path == "/v1/alerts":
...
_event_bus = EventBus("platform.events", "insurance-portfolio-analytics")


# --- Data Flow Emit Point ---
def emit_processing_event(action: str, data: dict) -> None:
    """Called by handlers after successful processing."""
    _event_bus.emit("insurance-portfolio-analytics." + action, data)
