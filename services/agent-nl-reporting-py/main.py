...
"""
agent-nl-reporting-py — Natural Language Financial Reporting Agent — answers ad-hoc financial questions in plain English
...
SERVICE_NAME = "agent-nl-reporting-py"
AGENT_TOOLS = ["neo4j_graph", "gl_engine", "falkordb", "kgqa", "qdrant_search"]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "agent-nl-reporting-py", **_degrade.status()})
        elif path == "/v1/alerts":
        ...
        elif path == "/v1/agent/trending":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "trending_metrics", "description": "Current trending metrics", "tools": AGENT_TOOLS})
    ...
    def do_POST(self):
        ...
        if path == "/v1/agent/ask":
            ...
            _event_bus.emit("agent-nl-reporting.processed", {"action": "process", "status": "success"})
            ...
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "ask_question", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/report":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "generate_report", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
        elif path == "/v1/agent/compare":
            if not self.check_jwt(): return
            if not _rl_allow():
                inc_errors(); self.respond(429, {"error": "rate_limit_exceeded"}); return
            ...
            db_insert(SERVICE_NAME, {"query": query, "intent": intent, "steps": len(steps)})
            cache_set(f"{self.get_tenant_id()}:{SERVICE_NAME}_last", json.dumps({"query": query}))
            self.respond(200, {"service": SERVICE_NAME, "endpoint": "compare_periods", "query": query, "intent": intent, "steps": steps, "result": response, "tools_used": selected_tools})
...
_event_bus = EventBus("platform.events", "agent-nl-reporting")
