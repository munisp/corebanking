...
"""
mojaloop-crossborder-py — Production-hardened service
"""
import os
import sys
import json
...
logger = logging.getLogger("mojaloop-crossborder-py")
...
def db_query(table, page=1, limit=50):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("mojaloop-crossborder-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("mojaloop-crossborder-py",))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"mojaloop_crossborder_{self.path}"
        ...
        logger.info(f"[mojaloop-crossborder-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "mojaloop-crossborder-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"mojaloop-crossborder-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"mojaloop-crossborder-py\"}} {error_count}\n'
            )
            self.send_response(200)
            ...
            if err:
                ...
                return
            items, total = db_query("mojaloop_crossborder_py")
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            try:
                result = db_insert("mojaloop_crossborder_py", body)
                ...
                return
        elif path == "/v1/mojaloop-crossborder/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/mojaloop-crossborder/process":
            rid = body.get("id", "")
            for rec in records:
...
_event_bus = EventBus("platform.events", "mojaloop-crossborder")


# --- Data Flow Emit Point ---
def emit_processing_event(action: str, data: dict) -> None:
    """Called by handlers after successful processing."""
    _event_bus.emit("mojaloop-crossborder." + action, data)

if __name__ == "__main__":
    get_db()
    server = HTTPServer(("0.0.0.0", PORT), Handler)
    logger.info(json.dumps({"service": "mojaloop-crossborder-py", "port": PORT, "message": "starting"}))
    try:
