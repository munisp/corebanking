"""
mojaloop-crossborder-py - Production-ready service with PostgreSQL persistence.
...
# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/mojaloop_crossborder_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[mojaloop-crossborder-py] ready on :%d", PORT)
    logger.info(f"[mojaloop-crossborder-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="mojaloop-crossborder-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "mojaloop-crossborder-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "mojaloop-crossborder-py", "total_records": count}
    except Exception:
        return {"service": "mojaloop-crossborder-py", "total_records": 0}
