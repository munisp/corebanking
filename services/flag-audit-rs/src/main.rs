...
async fn audit_change(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("flag-audit-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("flag-audit-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("flag-audit-rs: upstream call join failed: {}", e),
    }
...
mod tests {
    ...
    #[test]
    fn test_audit_change_exists() {
        // Verify audit_change compiles and is callable
        // Domain function: audit_change(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "audit_change should be defined");
    }
    #[test]
