...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[flag-audit-rs] starting");

    let db_name = "flag-audit-rs".replace("-", "_");
    ...
    log::info!("[flag-audit-rs] database connected, schema initialized");
    ...
    log::info!("[flag-audit-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8623".to_string()).parse().unwrap_or(8623);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[flag-audit-rs] ready on :{}", port);
