...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[auth-enforcer-rs] starting");

    let db_name = "auth-enforcer-rs".replace("-", "_");
    ...
    log::info!("[auth-enforcer-rs] database connected, schema initialized");
    ...
    log::info!("[auth-enforcer-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8986".to_string()).parse().unwrap_or(8986);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[auth-enforcer-rs] ready on :{}", port);
