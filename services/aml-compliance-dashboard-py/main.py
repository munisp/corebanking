"""
aml-compliance-dashboard-py - Production-ready service with PostgreSQL persistence.
...
# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/aml_compliance_dashboard_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[aml-compliance-dashboard-py] ready on :%d", PORT)
    logger.info(f"[aml-compliance-dashboard-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="aml-compliance-dashboard-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "aml-compliance-dashboard-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "aml-compliance-dashboard-py", "total_records": count}
    except Exception:
        return {"service": "aml-compliance-dashboard-py", "total_records": 0}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"aml_compliance_dashboard_{self.path}"
        ...
        logger.info(f"[aml-compliance-dashboard-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self._json(200, {"service": "aml-compliance-dashboard-py", **_degrade.status()})
        elif path == "/v1/alerts":
