"""
area-yield-index-insurance-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("area-yield-index-insurance-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/area_yield_index_insurance_py")
...
PORT = int(os.environ.get("PORT", "9590"))
...
def init_schema():
    ...
    try:
        ...
        cur.execute("INSERT INTO records (data, service) VALUES (%s, %s) RETURNING id, created_at",
                    (data, "area-yield-index-insurance-py"))
        row = cur.fetchone()
        record["id"] = str(row[0])
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS idx_products_tenant ON products(tenant_id)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_products_status ON products(status)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_products_created ON products(created_at DESC)")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[area-yield-index-insurance-py] ready on :%d", PORT)
    logger.info(f"[area-yield-index-insurance-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="area-yield-index-insurance-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "area-yield-index-insurance-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "area-yield-index-insurance-py", "total_records": count}
    except Exception:
        return {"service": "area-yield-index-insurance-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("area-yield-index-insurance-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("area-yield-index-insurance-py",))
...
# --- Domain Logic ---
def gen_id():
    return "ARE-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"area_yield_index_insurance_{self.path}"
        ...
        logger.info(f"[area-yield-index-insurance-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {
                "status": "healthy",
                "service": "area-yield-index-insurance-py",
                "version": "2.0.0",
            ...
            self.respond(200, {"service": "area-yield-index-insurance-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"area-yield-index-insurance-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"area-yield-index-insurance-py\"}} {error_count}\n'
            )
        else:
            cur.execute("SELECT id, status, created_at FROM products ORDER BY created_at DESC LIMIT 50")
        rows = cur.fetchall()
...
def create_record(body: CreateRequest, x_tenant_id: Optional[str] = Header(None)):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO products (id, tenant_id, status) VALUES (%s::uuid, %s::uuid, %s)",
        ...
        cur.execute(
            ...
            ("products.created", record_id, payload)
        )
    conn.commit()
    return {"id": record_id, "status": "created"}


@app.get("/api/v1/products/{record_id}")
def get_record(record_id: str):
    conn = get_db()
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute("SELECT id, status, created_at FROM products WHERE id = %s::uuid", (record_id,))
        row = cur.fetchone()
    if not row:
...
@app.put("/api/v1/products/{record_id}")
def update_record(record_id: str, body: UpdateRequest):
    status = body.status or "updated"
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE products SET status = %s, updated_at = NOW() WHERE id = %s::uuid",
            (status, record_id)
        )
        payload = json.dumps({"id": record_id, "status": status})
        cur.execute(
            "INSERT INTO outbox (event_type, aggregate_id, payload) VALUES (%s, %s, %s::jsonb)",
            ("products.updated", record_id, payload)
        )
    conn.commit()
    return {"id": record_id, "status": status}


@app.delete("/api/v1/products/{record_id}", status_code=204)
def delete_record(record_id: str):
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute("UPDATE products SET status = 'deleted', updated_at = NOW() WHERE id = %s::uuid", (record_id,))
        payload = json.dumps({"id": record_id})
        cur.execute(
            "INSERT INTO outbox (event_type, aggregate_id, payload) VALUES (%s, %s, %s::jsonb)",
            ("products.deleted", record_id, payload)
        )
    conn.commit()
    ...
    def do_POST(self):
        ...
        if err:
            ...
            try:
                ...
                call_service("POST", f"{_upstream}/v1/screen", {"service": "area-yield-index-insurance-py", "action": "notify"})
            except Exception as _e:
                logger.warning(f"inter-service call failed: {_e}")
            return

        if path == "/v1/create":
            result = db_insert("area_yield_index_insurance_py", body)
            cache_set("area_yield_index_insurance:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/area-yield-index-insurance/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/area-yield-index-insurance/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/area-yield-index-insurance/compute-premium":
            result = compute_premium(body.get("sum_assured",0), body.get("risk_factor",1.0), body.get("tenure_years",1))
            self.respond(200, result)
        elif path == "/v1/area-yield-index-insurance/assess-claim":
...
if __name__ == "__main__":
    ...
    logger.info(json.dumps({"service": "area-yield-index-insurance-py", "port": PORT, "message": "starting"}))
    try:
