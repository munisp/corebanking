...
```bash
# Build
go build -o mojaloop-connector-go .

# Run
PORT=6379 ./mojaloop-connector-go
```
...
```bash
docker build -t 54bank/mojaloop-connector-go .
docker run -p 6379:6379 54bank/mojaloop-connector-go
```
