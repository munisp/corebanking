...
func TestParseMICR(t *testing.T) {
	...
	if bank != "044" {
		t.Errorf("expected bank=044, got %s", bank)
	}
	if branch != "001" {
		t.Errorf("expected branch=001, got %s", branch)
	}
	if serial != "00012345" {
		t.Errorf("expected serial=00012345, got %s", serial)
	}

	bank, branch, serial = parseMICR("")
	if bank != "" {
		t.Error("expected empty bank for empty input")
	}
}

func TestClearingCycle(t *testing.T) {
	if c := clearingCycle(50000000); c != "same_day" {
		t.Errorf("expected same_day, got %s", c)
	}
	if c := clearingCycle(5000); c != "t_plus_1" {
		t.Errorf("expected t_plus_1, got %s", c)
	}
}
...
func TestStaleCheque(t *testing.T) {
	if !staleCheque(181) {
		t.Error("181 days should be stale")
	}
	if staleCheque(180) {
		t.Error("180 days should not be stale")
	}
	if staleCheque(30) {
		t.Error("30 days should not be stale")
	}
}
