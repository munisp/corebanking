...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[face-match-rs] starting");

    let db_name = "face-match-rs".replace("-", "_");
    ...
    log::info!("[face-match-rs] database connected, schema initialized");
    ...
    log::info!("[face-match-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8869".to_string()).parse().unwrap_or(8869);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[face-match-rs] ready on :{}", port);
