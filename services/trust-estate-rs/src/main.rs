...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[trust-estate-rs] starting");

    let db_name = "trust-estate-rs".replace("-", "_");
    ...
    log::info!("[trust-estate-rs] database connected, schema initialized");
    ...
    log::info!("[trust-estate-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8845".to_string()).parse().unwrap_or(8845);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[trust-estate-rs] ready on :{}", port);
