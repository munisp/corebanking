...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("animal-id-traceability.processed", &serde_json::json!({"status": "success"}));
...
async fn trace_animal(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    db_persist(&state, "trace_animal", &_result_data).await;
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "animal-id-traceability-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"animal-id-traceability-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"animal-id-traceability-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "animal_id_traceability_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("animal-id-traceability-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("animal-id-traceability-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("animal-id-traceability-rs", 10394);
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[animal-id-traceability-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("agriculture.operations", "animal-id-traceability");
    eprintln!("[animal-id-traceability] Data flow initialized: topic=agriculture.operations");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "animal-id-traceability-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "animal_id_traceability");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "animal_id_traceability");
    }
