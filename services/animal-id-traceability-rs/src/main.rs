...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[animal-id-traceability-rs] starting");

    let db_name = "animal-id-traceability-rs".replace("-", "_");
    ...
    log::info!("[animal-id-traceability-rs] database connected, schema initialized");
    ...
    log::info!("[animal-id-traceability-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8531".to_string()).parse().unwrap_or(8531);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[animal-id-traceability-rs] ready on :{}", port);
