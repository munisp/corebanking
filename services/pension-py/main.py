#!/usr/bin/env python3
"""
pension-py — 54link-dev Microservice
...
PORT = int(os.environ.get("PORT", 9526))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[pension-py] ready on :%d", PORT)
    logger.info(f"[pension-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="pension-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "pension-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "pension-py", "total_records": count}
    except Exception:
...
def validate_jwt(headers):
    # dict(self.headers) lowercases keys when APISIX forwards via HTTP/2,
    # so check both cases explicitly.
...
def process_pension(data):
    """Core processing function for pension-py."""
    record_id = f"pension_{int(time.time()*1e6)}"
    return {"processed": True, "id": record_id, "service": "pension-py", "input_keys": list(data.keys()) if isinstance(data, dict) else []}

def validate_pension_input(data):
    """Validate input for pension-py."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/pension-py/pension_accounts":
            qs = parse_qs(urlparse(self.path).query)
            tenant_id = qs.get("tenantId", [None])[0]
            records = db_query() or []
            accounts = [
                {"id": r["id"], "service": r["service"], "type": r.get("type", "pension"),
                 "status": r.get("status", "active"), "data": r.get("data", {}),
                 "created_at": r.get("created_at", "")}
                for r in records
            ]
            respond(self, 200, {"pension_accounts": accounts, "total": len(accounts),
                                "tenant_id": tenant_id, "service": SERVICE_NAME})
            return

        if path == "/v1/pension-py/stats":
            qs = parse_qs(urlparse(self.path).query)
            tenant_id = qs.get("tenantId", [None])[0]
            records = db_query() or []
            ...
            respond(self, 200, {
                ...
                "inactive_accounts": len(records) - active,
                "tenant_id": tenant_id,
                "uptime_secs": int(time.time() - _start_time),
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            ...
            _validate_pension_input_result = validate_pension_input(body.get("data", {}))
            _process_pension_result = process_pension(body.get("data", {}))
