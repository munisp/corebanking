"""
unit-test-runner-py - Production-ready service with PostgreSQL persistence.
...
PORT = int(os.environ.get("PORT", "9643"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[unit-test-runner-py] ready on :%d", PORT)
    logger.info(f"[unit-test-runner-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="unit-test-runner-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "unit-test-runner-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "unit-test-runner-py", "total_records": count}
    except Exception:
        return {"service": "unit-test-runner-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("unit-test-runner-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("unit-test-runner-py",))
...
# --- Domain Logic ---
def gen_id():
    return "UNI-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
SERVICE_NAME = "unit-test-runner-py"
records: list = []
audit_log: list = []
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("unit_test_runner_py", body)
            cache_set(f"unit-test-runner-py:last_post", str(body))
