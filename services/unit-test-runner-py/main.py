...
"""
unit-test-runner-py — Production-hardened service
"""
import os
import sys
import json
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "unit-test-runner-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            if err:
                ...
                return
            items, total = db_query("unit_test_runner_py")
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            ...
            _event_bus.emit("unit-test-runner.processed", {"action": "process", "status": "success"})
...
_event_bus = EventBus("platform.events", "unit-test-runner")

if __name__ == "__main__":
    get_db()
    server = HTTPServer(("0.0.0.0", PORT), Handler)
    logger.info(json.dumps({"service": "unit-test-runner-py", "port": PORT, "message": "starting"}))
    try:
