"""
whatsapp-notification-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("whatsapp-notification-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/whatsapp_notification_py")
...
PORT = int(os.environ.get("PORT", "9648"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[whatsapp-notification-py] ready on :%d", PORT)
    logger.info(f"[whatsapp-notification-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="whatsapp-notification-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "whatsapp-notification-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "whatsapp-notification-py", "total_records": count}
    except Exception:
        return {"service": "whatsapp-notification-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("whatsapp-notification-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("whatsapp-notification-py",))
...
# --- Domain Logic ---
def gen_id():
    return "WHA-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"whatsapp_notification_{self.path}"
        ...
        logger.info(f"[whatsapp-notification-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "whatsapp-notification-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"whatsapp-notification-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"whatsapp-notification-py\"}} {error_count}\n'
            )
        else:
