...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[biometric-auth-rs] starting");

    let db_name = "biometric-auth-rs".replace("-", "_");
    ...
    log::info!("[biometric-auth-rs] database connected, schema initialized");
    ...
    log::info!("[biometric-auth-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8368".to_string()).parse().unwrap_or(8368);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[biometric-auth-rs] ready on :{}", port);
