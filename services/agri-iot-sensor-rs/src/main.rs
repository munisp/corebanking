...
async fn main() -> std::io::Result<()> {
    ...
    let db_name = "agri-iot-sensor-rs".replace("-", "_");
    ...
    log::info!("[agri-iot-sensor-rs] database connected, schema initialized");
    ...
    log::info!("[agri-iot-sensor-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8499".to_string()).parse().unwrap_or(8499);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[agri-iot-sensor-rs] ready on :{}", port);
