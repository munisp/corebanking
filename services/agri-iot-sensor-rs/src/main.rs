...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("agri-iot-sensor.processed", &serde_json::json!({"status": "success"}));
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("agri-iot-sensor-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    let grpc_svc_name = "agri-iot-sensor-rs".to_string();
    tokio::spawn(async move {
        grpc_service::start_grpc_server(&grpc_svc_name, 9107).await;
    });
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[agri-iot-sensor-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
fn init_data_flow() -> EventBus {
    ...
    log::info!("[agri-iot-sensor] Data flow initialized: topic=agriculture.operations");
    bus
}
