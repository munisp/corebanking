...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[ubo-ownership-graph-rs] starting");

    let db_name = "ubo-ownership-graph-rs".replace("-", "_");
    ...
    log::info!("[ubo-ownership-graph-rs] database connected, schema initialized");
    ...
    log::info!("[ubo-ownership-graph-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8650".to_string()).parse().unwrap_or(8650);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[ubo-ownership-graph-rs] ready on :{}", port);
