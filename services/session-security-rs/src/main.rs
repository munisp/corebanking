...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[session-security-rs] starting");

    let db_name = "session-security-rs".replace("-", "_");
    ...
    log::info!("[session-security-rs] database connected, schema initialized");
    ...
    log::info!("[session-security-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8273".to_string()).parse().unwrap_or(8273);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[session-security-rs] ready on :{}", port);
