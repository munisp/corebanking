...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("otc-derivatives.processed", &serde_json::json!({"status": "success"}));
...
async fn price_swap(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("otc-derivatives-rs: upstream call ok"),
        Err(e) => eprintln!("otc-derivatives-rs: upstream call failed: {}", e),
    }
...
async fn list_records(req: actix_web::HttpRequest, state: web::Data<AppState>, query: web::Query<std::collections::HashMap<String, String>>) -> HttpResponse {
    ...
    if let Some(ref client) = state.db_client {
        match client.query(
            ...
            &[&"otc_derivatives_rs", &(limit as i64), &(offset as i64)]
        ).await {
            Ok(rows) => {
                ...
                let total: i64 = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"otc_derivatives_rs"]).await.map(|r| r.get(0)).unwrap_or(0);
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    if let Some(ref client) = state.db_client {
        if let Ok(row) = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"otc_derivatives_rs"]).await {
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        ...
        let svc_name = String::from("otc-derivatives-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("otc-derivatives-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("otc-derivatives-rs", 10363);
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[otc-derivatives-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("trading.operations", "otc-derivatives");
    eprintln!("[otc-derivatives] Data flow initialized: topic=trading.operations");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "otc-derivatives-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "otc_derivatives");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "otc_derivatives");
    }
