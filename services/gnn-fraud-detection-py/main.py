"""
gnn-fraud-detection-py - Production-ready service with PostgreSQL persistence.
...
# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/gnn_fraud_detection_py")
...
FRAUD_DETECTION_URL = os.environ.get("FRAUD_DETECTION_URL", "http://localhost:8121")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[gnn-fraud-detection-py] ready on :%d", PORT)
    logger.info(f"[gnn-fraud-detection-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="gnn-fraud-detection-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "gnn-fraud-detection-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "gnn-fraud-detection-py", "total_records": count}
    except Exception:
        return {"service": "gnn-fraud-detection-py", "total_records": 0}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"gnn_fraud_detection_{self.path}"
...
def delete_record(record_id: str):
    ...
    def do_POST(self):
        ...
        if err:
            ...
            # Inter-service call
            try:
                _upstream = os.environ.get("FRAUD_DETECTION_URL", "http://localhost:8121")
                call_service("POST", f"{_upstream}/v1/detect", {"service": SERVICE_NAME, "action": "notify"})
