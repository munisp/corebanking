...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[livestock-management-rs] starting");

    let db_name = "livestock-management-rs".replace("-", "_");
    ...
    log::info!("[livestock-management-rs] database connected, schema initialized");
    ...
    log::info!("[livestock-management-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8461".to_string()).parse().unwrap_or(8461);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[livestock-management-rs] ready on :{}", port);
