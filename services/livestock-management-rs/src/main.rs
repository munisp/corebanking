...
async fn health() -> HttpResponse {
    HttpResponse::Ok().insert_header(("content-security-policy", "default-src 'self'")).json(json!({"status": "healthy", "service": "livestock-management-rs"}))
}
...
async fn manage_herd(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("livestock-management-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("livestock-management-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("livestock-management-rs: upstream call join failed: {}", e),
    }
