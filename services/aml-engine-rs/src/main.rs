...
async fn main() -> std::io::Result<()> {
    ...
    let db_name = "aml-engine-rs".replace("-", "_");
    ...
    log::info!("[aml-engine-rs] database connected, schema initialized");
    ...
    log::info!("[aml-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8988".to_string()).parse().unwrap_or(8988);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[aml-engine-rs] ready on :{}", port);
