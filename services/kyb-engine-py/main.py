"""
kyb-engine-py - Production-ready service with PostgreSQL persistence.
...
# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/kyb_engine_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[kyb-engine-py] ready on :%d", PORT)
    logger.info(f"[kyb-engine-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="kyb-engine-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "kyb-engine-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "kyb-engine-py", "total_records": count}
    except Exception:
        return {"service": "kyb-engine-py", "total_records": 0}
