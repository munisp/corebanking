...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"kyb_engine_{self.path}"
        ...
        logger.info(f"[kyb-engine-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "kyb-engine-py", **_degrade.status()})
        elif path == "/v1/alerts":
