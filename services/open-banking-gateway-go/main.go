...
// --- CBN Open Banking Regulatory Framework — AISP + PISP APIs ---
...
func handleHealthz(w http.ResponseWriter, r *http.Request) {
	...
	count := len(records)
	recordsMu.RUnlock()
	respondJSON(w, 200, map[string]interface{}{"status": "healthy", "service": "open-banking-gateway-go", "version": "1.0.0", "records": count})
}
...
func handleCreate(w http.ResponseWriter, r *http.Request) {
	...
	rec := Record{
		ID: fmt.Sprintf("OPE-%s", secureRandID()),
...
func main() {
	...
	mux.HandleFunc("/api/v1/open_banking_gateway", handleList)
	mux.HandleFunc("/api/v1/open_banking_gateway/create", handleCreate)
	log.Printf("CBN Open Banking Regulatory Framework — AISP + PISP APIs listening on :%s", PORT)
