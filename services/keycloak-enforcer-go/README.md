...
```bash
# Build
go build -o keycloak-enforcer-go .

# Run
PORT=6379 ./keycloak-enforcer-go
```
...
```bash
docker build -t 54bank/keycloak-enforcer-go .
docker run -p 6379:6379 54bank/keycloak-enforcer-go
```
