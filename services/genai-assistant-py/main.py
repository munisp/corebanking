"""
genai-assistant-py - Production-ready service with PostgreSQL persistence.
...
# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/genai_assistant_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[genai-assistant-py] ready on :%d", PORT)
    logger.info(f"[genai-assistant-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="genai-assistant-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "genai-assistant-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "genai-assistant-py", "total_records": count}
    except Exception:
        return {"service": "genai-assistant-py", "total_records": 0}
