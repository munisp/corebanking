...
// --- Cross-border remittance corridors — SWIFT GPI, Mojaloop, Africa corridors ---
...
func handleHealthz(w http.ResponseWriter, r *http.Request) {
	...
	count := len(records)
	recordsMu.RUnlock()
	respondJSON(w, 200, map[string]interface{}{"status": "healthy", "service": "cross-border-remittance-go", "version": "1.0.0", "records": count})
}
...
func handleCreate(w http.ResponseWriter, r *http.Request) {
	...
	rec := Record{
		ID: fmt.Sprintf("CRO-%s", secureRandID()),
...
func main() {
	...
	mux.HandleFunc("/api/v1/cross_border_remittance", handleList)
	mux.HandleFunc("/api/v1/cross_border_remittance/create", handleCreate)
	log.Printf("Cross-border remittance corridors — SWIFT GPI, Mojaloop, Africa corridors listening on :%s", PORT)
