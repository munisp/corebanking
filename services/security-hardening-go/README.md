...
```bash
# Build
go build -o security-hardening-go .

# Run
PORT=6379 ./security-hardening-go
```
...
```bash
docker build -t 54bank/security-hardening-go .
docker run -p 6379:6379 54bank/security-hardening-go
```
