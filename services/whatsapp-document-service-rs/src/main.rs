...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[whatsapp-document-service-rs] starting");

    let db_name = "whatsapp-document-service-rs".replace("-", "_");
    ...
    log::info!("[whatsapp-document-service-rs] database connected, schema initialized");
    ...
    log::info!("[whatsapp-document-service-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8369".to_string()).parse().unwrap_or(8369);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[whatsapp-document-service-rs] ready on :{}", port);
