...
// redis-cache-middleware-rs — Redis caching middleware
...
async fn cache_middleware(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "cache_middleware"});
    db_persist(&state, "cache_middleware", &_result_data).await;
    // Inter-service call
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("redis-cache-middleware-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("redis-cache-middleware-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("redis-cache-middleware-rs: upstream call join failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>, req: actix_web::HttpRequest) -> HttpResponse {
    if let Err(resp) = check_jwt(&req).await { return resp; }
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "redis-cache-middleware-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "redis-cache-middleware-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"redis-cache-middleware-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"redis-cache-middleware-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "redis_cache_middleware_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("redis-cache-middleware-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8242);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("redis-cache-middleware-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("redis-cache-middleware-rs on port {}", port);
    start_grpc_server("redis-cache-middleware-rs", 10473);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[redis-cache-middleware-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/cache_middleware", web::post().to(cache_middleware))
...
mod tests {
    ...
    #[test]
    fn test_cache_control_exists() {
        // Verify cache_control compiles and is callable
        // Domain function: cache_control(method: &str, path: &str) -> u64
        assert!(true, "cache_control should be defined");
    }

    #[test]
    fn test_cache_middleware_exists() {
        // Verify cache_middleware compiles and is callable
        // Domain function: cache_middleware(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "cache_middleware should be defined");
    }
    #[test]
