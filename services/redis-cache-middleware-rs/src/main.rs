...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[redis-cache-middleware-rs] starting");

    let db_name = "redis-cache-middleware-rs".replace("-", "_");
    ...
    log::info!("[redis-cache-middleware-rs] database connected, schema initialized");
    ...
    log::info!("[redis-cache-middleware-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8736".to_string()).parse().unwrap_or(8736);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[redis-cache-middleware-rs] ready on :{}", port);
