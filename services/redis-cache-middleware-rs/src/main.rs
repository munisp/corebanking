...
async fn health(state: web::Data<AppState>) -> HttpResponse {
    ...
    _bus.emit("redis-cache-middleware.processed", &serde_json::json!({"status": "success"}));
...
fn configure_routes(cfg: &mut web::ServiceConfig) {
    cfg.route("/proxy/{path:.*}", web::to(cached_proxy))
       .route("/v1/redis-cache-middleware/invalidate", web::post().to(invalidate))
...
async fn healthz() -> HttpResponse {
    HttpResponse::Ok().json(serde_json::json!({"status": "healthy", "service": "redis-cache-middleware-rs"}))
}
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("infra.cache", "redis-cache-middleware");
    eprintln!("[redis-cache-middleware] Data flow initialized: topic=infra.cache");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "redis-cache-middleware-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "redis_cache_middleware");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "redis_cache_middleware");
    }
