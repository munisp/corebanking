...
```bash
# Build
go build -o postgres-query-optimizer-go .

# Run
PORT=6379 ./postgres-query-optimizer-go
```
...
```bash
docker build -t 54bank/postgres-query-optimizer-go .
docker run -p 6379:6379 54bank/postgres-query-optimizer-go
```
