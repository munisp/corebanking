...
```bash
# Build
go build -o approval-workflow-go .

# Run
PORT=6379 ./approval-workflow-go
```
...
```bash
docker build -t 54bank/approval-workflow-go .
docker run -p 6379:6379 54bank/approval-workflow-go
```
