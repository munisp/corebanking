...
SERVICE_NAME = "kyc-analytics-dashboard-py"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"kyc_analytics_dashboard_{self.path}"
        ...
        logger.info(f"[kyc-analytics-dashboard-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            body = (
                ...
                f'requests_total{{service=\"kyc-analytics-dashboard-py\"}} {request_count}\n'
