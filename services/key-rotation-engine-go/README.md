...
```bash
# Build
go build -o key-rotation-engine-go .

# Run
PORT=6379 ./key-rotation-engine-go
```
...
```bash
docker build -t 54bank/key-rotation-engine-go .
docker run -p 6379:6379 54bank/key-rotation-engine-go
```
