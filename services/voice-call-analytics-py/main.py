"""
voice-call-analytics-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("voice-call-analytics-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/voice_call_analytics_py")
...
PORT = int(os.environ.get("PORT", "9646"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[voice-call-analytics-py] ready on :%d", PORT)
    logger.info(f"[voice-call-analytics-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="voice-call-analytics-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "voice-call-analytics-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "voice-call-analytics-py", "total_records": count}
    except Exception:
        return {"service": "voice-call-analytics-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("voice-call-analytics-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("voice-call-analytics-py",))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"voice_call_analytics_{self.path}"
        ...
        logger.info(f"[voice-call-analytics-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "voice-call-analytics-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"voice-call-analytics-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"voice-call-analytics-py\"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("voice_call_analytics_py", body)
            cache_set(f"{self.get_tenant_id()}:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/voice-call-analytics/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
