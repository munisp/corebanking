...
async fn cascade_rate(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("rate-cascade-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("rate-cascade-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("rate-cascade-rs: upstream call join failed: {}", e),
    }
...
mod tests {
    ...
    #[test]
    fn test_cascade_rate_exists() {
        // Verify cascade_rate compiles and is callable
        // Domain function: cascade_rate(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "cascade_rate should be defined");
    }
    #[test]
