...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[rate-cascade-rs] starting");

    let db_name = "rate-cascade-rs".replace("-", "_");
    ...
    log::info!("[rate-cascade-rs] database connected, schema initialized");
    ...
    log::info!("[rate-cascade-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8305".to_string()).parse().unwrap_or(8305);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[rate-cascade-rs] ready on :{}", port);
