...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("ip-allowlist.processed", &serde_json::json!({"status": "success"}));
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("security.infra", "ip-allowlist");
    eprintln!("[ip-allowlist] Data flow initialized: topic=security.infra");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "ip-allowlist-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "ip_allowlist");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "ip_allowlist");
    }
