...
async fn check_ip(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "check_ip"});
    db_persist(&state, "check_ip", &_result_data).await;
    // Inter-service call
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("ip-allowlist-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("ip-allowlist-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("ip-allowlist-rs: upstream call join failed: {}", e),
    }
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "ip_allowlist_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("ip-allowlist-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("ip-allowlist-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("ip-allowlist-rs", 10385);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[ip-allowlist-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    #[test]
    fn test_ip_in_cidr_exists() {
        // Verify ip_in_cidr compiles and is callable
        // Domain function: ip_in_cidr(ip: &str, cidr: &str) -> bool
        assert!(true, "ip_in_cidr should be defined");
    }

    #[test]
    fn test_check_ip_exists() {
        // Verify check_ip compiles and is callable
        // Domain function: check_ip(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "check_ip should be defined");
    }
    #[test]
