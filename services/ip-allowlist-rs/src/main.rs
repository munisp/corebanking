...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[ip-allowlist-rs] starting");

    let db_name = "ip-allowlist-rs".replace("-", "_");
    ...
    log::info!("[ip-allowlist-rs] database connected, schema initialized");
    ...
    log::info!("[ip-allowlist-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8591".to_string()).parse().unwrap_or(8591);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[ip-allowlist-rs] ready on :{}", port);
