...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[mojaloop-fspiop-callbacks-rs] starting");

    let db_name = "mojaloop-fspiop-callbacks-rs".replace("-", "_");
    ...
    log::info!("[mojaloop-fspiop-callbacks-rs] database connected, schema initialized");
    ...
    log::info!("[mojaloop-fspiop-callbacks-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8593".to_string()).parse().unwrap_or(8593);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[mojaloop-fspiop-callbacks-rs] ready on :{}", port);
