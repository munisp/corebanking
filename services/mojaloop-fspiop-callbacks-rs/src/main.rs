...
async fn health(state: web::Data<AppState>) -> HttpResponse {
    ...
    _bus.emit("mojaloop-fspiop-callbacks.processed", &serde_json::json!({"status": "success"}));
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("messaging.interbank", "mojaloop-fspiop-callbacks");
    eprintln!("[mojaloop-fspiop-callbacks] Data flow initialized: topic=messaging.interbank");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "mojaloop-fspiop-callbacks-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "mojaloop_fspiop_callbacks");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "mojaloop_fspiop_callbacks");
    }
