...
async fn rate_transaction(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/charge", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("billing-rating-rs: charge ok"),
        Ok(Err(e)) => eprintln!("billing-rating-rs: charge failed: {}", e),
        Err(e) => eprintln!("billing-rating-rs: charge join failed: {}", e),
    }
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        ...
        let svc_name = String::from("billing-rating-rs");
...
async fn billing_rate_cards(req: actix_web::HttpRequest) -> HttpResponse {
    if let Err(resp) = check_jwt(&req) { return resp; }
    let rate_cards = json!([
        {"id": "BRC-001", "tenant_id": null, "meter_key": "active_customer",  "product_key": "customer",    "amount": 10.0,       "currency": "NGN", "included_units": 200000, "description": "Per active customer above 200K"},
        {"id": "BRC-002", "tenant_id": null, "meter_key": "named_user",        "product_key": "operations",  "amount": 50000.0,    "currency": "NGN", "included_units": 150,    "description": "Per named seat above 150"},
        {"id": "BRC-003", "tenant_id": null, "meter_key": "transfer_posted",   "product_key": "payments",    "amount": 25.0,       "currency": "NGN", "included_units": 10000,  "description": "Per transfer above 10K/mo"},
        {"id": "BRC-004", "tenant_id": null, "meter_key": "branch",            "product_key": "operations",  "amount": 350000.0,   "currency": "NGN", "included_units": 25,     "description": "Per branch above 25"},
        {"id": "BRC-005", "tenant_id": null, "meter_key": "environment",       "product_key": "platform",    "amount": 1500000.0,  "currency": "NGN", "included_units": 2,      "description": "Per extra environment"},
        {"id": "BRC-006", "tenant_id": null, "meter_key": "api_call",          "product_key": "platform",    "amount": 0.5,        "currency": "NGN", "included_units": 1000000, "description": "Per API call above 1M/mo"},
    ]);
    HttpResponse::Ok().json(json!({"items": rate_cards, "total": 6}))
}

async fn billing_rate(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
    if let Err(resp) = check_jwt(&req) { return resp; }
    if !rl_allow() { return HttpResponse::TooManyRequests().json(json!({"error": "rate_limit_exceeded"})); }
    let input = body.into_inner();
    let meter_key = input.get("meter_key").and_then(|v| v.as_str()).unwrap_or("transfer_posted");
    let quantity = input.get("quantity").and_then(|v| v.as_f64()).unwrap_or(1.0);
    let (unit_rate, included_units): (f64, f64) = match meter_key {
        "active_customer"  => (10.0, 200_000.0),
        "named_user"       => (50_000.0, 150.0),
        "transfer_posted"  => (25.0, 10_000.0),
        "branch"           => (350_000.0, 25.0),
        "environment"      => (1_500_000.0, 2.0),
        "api_call"         => (0.5, 1_000_000.0),
        _                  => (1.0, 0.0),
    };
    let billable_units = (quantity - included_units).max(0.0);
    let amount_accrued = billable_units * unit_rate;
    HttpResponse::Ok().json(json!({
        "meter_key": meter_key,
        "quantity": quantity,
...
async fn main() -> std::io::Result<()> {
    ...
    start_grpc_server("billing-rating-rs", 10448);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            // ── Billing UI endpoints ──────────────────────────────────────────
            .route("/v1/billing/rate-cards", web::get().to(billing_rate_cards))
            .route("/v1/billing/rate", web::post().to(billing_rate))
    })
    .bind(("0.0.0.0", port))?
    .shutdown_timeout(30)
    .run()
