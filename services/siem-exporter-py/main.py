...
"""
siem-exporter-py — Production-hardened service
"""
import os
import sys
import json
    ...
    def do_GET(self):
        ...
        if path == "/v1/cache-metrics":
            ...
            if err:
                ...
                return
            items, total = db_query("siem_exporter_py")
...
_event_bus = EventBus("platform.events", "siem-exporter")


# --- Data Flow Emit Point ---
def emit_processing_event(action: str, data: dict) -> None:
    """Called by handlers after successful processing."""
    _event_bus.emit("siem-exporter." + action, data)

if __name__ == "__main__":
    get_db()
    server = HTTPServer(("0.0.0.0", PORT), Handler)
    logger.info(json.dumps({"service": "siem-exporter-py", "port": PORT, "message": "starting"}))
    try:
