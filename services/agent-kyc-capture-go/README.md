...
```bash
# Build
go build -o agent-kyc-capture-go .

# Run
PORT=6379 ./agent-kyc-capture-go
```
...
```bash
docker build -t 54bank/agent-kyc-capture-go .
docker run -p 6379:6379 54bank/agent-kyc-capture-go
```
