...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[satellite-crop-monitor-rs] starting");

    let db_name = "satellite-crop-monitor-rs".replace("-", "_");
    ...
    log::info!("[satellite-crop-monitor-rs] database connected, schema initialized");
    ...
    log::info!("[satellite-crop-monitor-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8349".to_string()).parse().unwrap_or(8349);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[satellite-crop-monitor-rs] ready on :{}", port);
