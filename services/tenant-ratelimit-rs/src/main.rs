...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[tenant-ratelimit-rs] starting");

    let db_name = "tenant-ratelimit-rs".replace("-", "_");
    ...
    log::info!("[tenant-ratelimit-rs] database connected, schema initialized");
    ...
    log::info!("[tenant-ratelimit-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8831".to_string()).parse().unwrap_or(8831);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[tenant-ratelimit-rs] ready on :{}", port);
