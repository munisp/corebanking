...
// tenant-ratelimit-rs — Per-tenant rate limiting
...
async fn check_tenant_rate(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "check_tenant_rate"});
    db_persist(&state, "check_tenant_rate", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("tenant-ratelimit-rs: upstream call ok"),
        Err(e) => eprintln!("tenant-ratelimit-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "tenant-ratelimit-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "tenant-ratelimit-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"tenant-ratelimit-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"tenant-ratelimit-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "tenant_ratelimit_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("tenant-ratelimit-rs");
...
fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8255);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("tenant-ratelimit-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("tenant-ratelimit-rs on port {}", port);
    start_grpc_server("tenant-ratelimit-rs", 10394);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[tenant-ratelimit-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/check_tenant_rate", web::post().to(check_tenant_rate))
...
mod tests {
    ...
    #[test]
    fn test_tenant_limit_exists() {
        // Verify tenant_limit compiles and is callable
        // Domain function: tenant_limit(tier: &str) -> u64
        assert!(true, "tenant_limit should be defined");
    }

    #[test]
    fn test_check_tenant_rate_exists() {
        // Verify check_tenant_rate compiles and is callable
        // Domain function: check_tenant_rate(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "check_tenant_rate should be defined");
    }
    #[test]
