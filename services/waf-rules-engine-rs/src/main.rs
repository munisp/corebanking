...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[waf-rules-engine-rs] starting");

    let db_name = "waf-rules-engine-rs".replace("-", "_");
    ...
    log::info!("[waf-rules-engine-rs] database connected, schema initialized");
    ...
    log::info!("[waf-rules-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8411".to_string()).parse().unwrap_or(8411);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[waf-rules-engine-rs] ready on :{}", port);
