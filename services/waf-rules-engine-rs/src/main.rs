...
async fn evaluate_waf(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("waf-rules-engine-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("waf-rules-engine-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("waf-rules-engine-rs: upstream call join failed: {}", e),
    }
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "waf_rules_engine_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("waf-rules-engine-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("waf-rules-engine-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("waf-rules-engine-rs", 10458);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[waf-rules-engine-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    fn test_match_rule_exists() {
        // Verify match_rule compiles and is callable
        // Domain function: match_rule(input: &str, pattern: &str, mode: &str) -> bool
        assert!(true, "match_rule should be defined");
    }

    #[test]
    fn test_evaluate_waf_exists() {
        // Verify evaluate_waf compiles and is callable
        // Domain function: evaluate_waf(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "evaluate_waf should be defined");
    }
    #[test]
