...
```bash
# Build
go build -o db-migration-manager-go .

# Run
PORT=6379 ./db-migration-manager-go
```
...
```bash
docker build -t 54bank/db-migration-manager-go .
docker run -p 6379:6379 54bank/db-migration-manager-go
```
