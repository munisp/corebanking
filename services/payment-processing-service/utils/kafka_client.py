...
class PaymentProcessingTopics:
    TRANSACTION = "payment.processing.transaction"
    PAYOUT = "payment.processing.payout"
    LOAN = "payment.processing.loan"
    LPO = "payment.processing.lpo"
    DEPOSIT = "payment.processing.deposit"
    TRANSFER = "payment.processing.transfer"
    INSURANCE_PREMIUM = "payment.processing.insurance.premium"
    SUPPLY_CHAIN_FINANCE = "payment.processing.supply.chain.finance"
...
class KafkaClient:
    ...
    def publish_goal_event(self, event_type: str, goal_id: str, tenant_id: str, status: str = None, metadata: Optional[Dict[str, Any]] = None):
        event = {
            "type": event_type,
            "goal_id": goal_id,
            "tenant_id": tenant_id,
        ...
        return self.publish_event(PaymentProcessingTopics.GOAL, event, key=str(goal_id))
