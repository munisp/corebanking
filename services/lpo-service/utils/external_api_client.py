        ...
        try:
            ...
            try:
                ...
                if isinstance(err_json.get("message"), str):
                    raise Exception(err_json["message"]) from e
                
                # Raise a custom error with the error message if it's a string
                if isinstance(err_json.get("detail"), str):
                    raise Exception(err_json["detail"]) from e

                # Optionally, handle cases where the error structure might differ
                raise Exception("An unknown error occurred", code=response.status_code, payload=err_json) from e
