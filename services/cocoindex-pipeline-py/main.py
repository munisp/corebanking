"""
cocoindex-pipeline-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("cocoindex-pipeline-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/cocoindex_pipeline_py")
...
PORT = int(os.environ.get("PORT", "9599"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[cocoindex-pipeline-py] ready on :%d", PORT)
    logger.info(f"[cocoindex-pipeline-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="cocoindex-pipeline-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "cocoindex-pipeline-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "cocoindex-pipeline-py", "total_records": count}
    except Exception:
        return {"service": "cocoindex-pipeline-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("cocoindex-pipeline-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("cocoindex-pipeline-py",))
...
# --- Domain Logic ---
def gen_id():
    return "COC-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"cocoindex_pipeline_{self.path}"
        ...
        logger.info(f"[cocoindex-pipeline-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "cocoindex-pipeline-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"cocoindex-pipeline-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"cocoindex-pipeline-py\"}} {error_count}\n'
            )
        else:
...
def delete_record(record_id: str):
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("cocoindex_pipeline_py", body)
            cache_set(f"{self.get_tenant_id()}:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/cocoindex-pipeline/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/cocoindex-pipeline/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/cocoindex-pipeline/validate-config":
            result = validate_pipeline_config(body.get("config", body))
            self.respond(200, result)
        elif path == "/v1/cocoindex-pipeline/estimate-throughput":
