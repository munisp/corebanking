...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[contract-test-rs] starting");

    let db_name = "contract-test-rs".replace("-", "_");
    ...
    log::info!("[contract-test-rs] database connected, schema initialized");
    ...
    log::info!("[contract-test-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8937".to_string()).parse().unwrap_or(8937);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[contract-test-rs] ready on :{}", port);
