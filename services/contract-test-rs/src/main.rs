...
// contract-test-rs — Contract testing engine
...
fn schema_match(expected: &str, actual: &str) -> bool { expected == actual }
...
async fn health() -> HttpResponse {
    HttpResponse::Ok().insert_header(("content-security-policy", "default-src 'self'")).json(json!({"status": "healthy", "service": "contract-test-rs"}))
}

async fn validate_contract(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "validate_contract"});
    db_persist(&state, "validate_contract", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("contract-test-rs: upstream call ok"),
        Err(e) => eprintln!("contract-test-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "contract-test-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "contract-test-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"contract-test-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"contract-test-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "contract_test_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("contract-test-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8211);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("contract-test-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("contract-test-rs on port {}", port);
    start_grpc_server("contract-test-rs", 10306);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[contract-test-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/validate_contract", web::post().to(validate_contract))
...
mod tests {
    ...
    #[test]
    fn test_schema_match_exists() {
        // Verify schema_match compiles and is callable
        // Domain function: schema_match(expected: &str, actual: &str) -> bool
        assert!(true, "schema_match should be defined");
    }

    #[test]
    fn test_validate_contract_exists() {
        // Verify validate_contract compiles and is callable
        // Domain function: validate_contract(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "validate_contract should be defined");
    }
    #[test]
