...
async fn health() -> HttpResponse {
    HttpResponse::Ok().insert_header(("content-security-policy", "default-src 'self'")).json(json!({"status": "healthy", "service": "clickjack-defender-rs"}))
}
...
async fn check_frame(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    // Parse allowed as a JSON array of origins, or fall back to a comma-separated string
    let allowed_vec: Vec<String> = input.get("allowed")
    ...
    let allowed_refs: Vec<&str> = allowed_vec.iter().map(|s| s.as_str()).collect();
    let result = frame_policy(origin, &allowed_refs);
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("clickjack-defender-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("clickjack-defender-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("clickjack-defender-rs: upstream call join failed: {}", e),
    }
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "clickjack_defender_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("clickjack-defender-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("clickjack-defender-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("clickjack-defender-rs", 10359);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[clickjack-defender-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    #[test]
    fn test_frame_policy_exists() {
        // Verify frame_policy compiles and is callable
        // Domain function: frame_policy(origin: &str, allowed: &[&str]) -> bool
        assert!(true, "frame_policy should be defined");
    }

    #[test]
    fn test_check_frame_exists() {
        // Verify check_frame compiles and is callable
        // Domain function: check_frame(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "check_frame should be defined");
    }
    #[test]
