...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[clickjack-defender-rs] starting");

    let db_name = "clickjack-defender-rs".replace("-", "_");
    ...
    log::info!("[clickjack-defender-rs] database connected, schema initialized");
    ...
    log::info!("[clickjack-defender-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8669".to_string()).parse().unwrap_or(8669);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[clickjack-defender-rs] ready on :{}", port);
