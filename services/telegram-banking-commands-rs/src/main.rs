...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[telegram-banking-commands-rs] starting");

    let db_name = "telegram-banking-commands-rs".replace("-", "_");
    ...
    log::info!("[telegram-banking-commands-rs] database connected, schema initialized");
    ...
    log::info!("[telegram-banking-commands-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8177".to_string()).parse().unwrap_or(8177);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[telegram-banking-commands-rs] ready on :{}", port);
