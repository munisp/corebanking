...
async fn process_command(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/create", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("telegram-banking-commands-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("telegram-banking-commands-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("telegram-banking-commands-rs: upstream call join failed: {}", e),
    }
