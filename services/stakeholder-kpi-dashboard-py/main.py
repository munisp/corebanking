...
logger = logging.getLogger("stakeholder-kpi-dashboard-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/stakeholder_kpi_dashboard_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[stakeholder-kpi-dashboard-py] ready on :%d", PORT)
    logger.info(f"[stakeholder-kpi-dashboard-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="stakeholder-kpi-dashboard-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "stakeholder-kpi-dashboard-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "stakeholder-kpi-dashboard-py", "total_records": count}
    except Exception:
        return {"service": "stakeholder-kpi-dashboard-py", "total_records": 0}
