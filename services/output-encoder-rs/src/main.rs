...
// output-encoder-rs — Output encoding for XSS prevention
...
fn html_encode(input: &str) -> String { input.replace('&', "&amp;").replace('<', "&lt;").replace('>', "&gt;").replace('"', "&quot;") }
...
async fn health() -> HttpResponse {
    HttpResponse::Ok().insert_header(("content-security-policy", "default-src 'self'")).json(json!({"status": "healthy", "service": "output-encoder-rs"}))
}

async fn encode_output(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "encode_output"});
    db_persist(&state, "encode_output", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("output-encoder-rs: upstream call ok"),
        Err(e) => eprintln!("output-encoder-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "output-encoder-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "output-encoder-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"output-encoder-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"output-encoder-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "output_encoder_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("output-encoder-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8232);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("output-encoder-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("output-encoder-rs on port {}", port);
    start_grpc_server("output-encoder-rs", 10495);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[output-encoder-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/encode_output", web::post().to(encode_output))
...
mod tests {
    ...
    #[test]
    fn test_html_encode_exists() {
        // Verify html_encode compiles and is callable
        // Domain function: html_encode(input: &str) -> String
        assert!(true, "html_encode should be defined");
    }

    #[test]
    fn test_encode_output_exists() {
        // Verify encode_output compiles and is callable
        // Domain function: encode_output(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "encode_output should be defined");
    }
    #[test]
