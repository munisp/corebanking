...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[output-encoder-rs] starting");

    let db_name = "output-encoder-rs".replace("-", "_");
    ...
    log::info!("[output-encoder-rs] database connected, schema initialized");
    ...
    log::info!("[output-encoder-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8003".to_string()).parse().unwrap_or(8003);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[output-encoder-rs] ready on :{}", port);
