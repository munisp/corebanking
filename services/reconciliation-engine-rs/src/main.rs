...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[reconciliation-engine-rs] starting");

    let db_name = "reconciliation-engine-rs".replace("-", "_");
    ...
    log::info!("[reconciliation-engine-rs] database connected, schema initialized");
    ...
    log::info!("[reconciliation-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8941".to_string()).parse().unwrap_or(8941);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[reconciliation-engine-rs] ready on :{}", port);
