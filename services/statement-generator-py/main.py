...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[statement-generator-py] ready on :%d", PORT)
    logger.info(f"[statement-generator-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "statement-generator-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "statement-generator-py", "total_records": count}
    except Exception:
...
# --- Domain Logic ---
def gen_id():
    return "STA-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"service": "statement-generator-py", **_degrade.status()})
        elif path == "/v1/alerts":
