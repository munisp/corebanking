...
// hot-data-cache-rs — Hot data caching layer
...
async fn cache_get(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "cache_get"});
    db_persist(&state, "cache_get", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("hot-data-cache-rs: upstream call ok"),
        Err(e) => eprintln!("hot-data-cache-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap();
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "hot-data-cache-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "hot-data-cache-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"hot-data-cache-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"hot-data-cache-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "hot_data_cache_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("hot-data-cache-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8219);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("hot-data-cache-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("hot-data-cache-rs on port {}", port);
    start_grpc_server("hot-data-cache-rs", 10491);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[hot-data-cache-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/cache_get", web::post().to(cache_get))
...
mod tests {
    ...
    #[test]
    fn test_cache_hit_rate_exists() {
        // Verify cache_hit_rate compiles and is callable
        // Domain function: cache_hit_rate(hits: u64, misses: u64) -> f64
        assert!(true, "cache_hit_rate should be defined");
    }

    #[test]
    fn test_cache_get_exists() {
        // Verify cache_get compiles and is callable
        // Domain function: cache_get(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "cache_get should be defined");
    }
    #[test]
