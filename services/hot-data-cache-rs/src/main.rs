...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[hot-data-cache-rs] starting");

    let db_name = "hot-data-cache-rs".replace("-", "_");
    ...
    log::info!("[hot-data-cache-rs] database connected, schema initialized");
    ...
    log::info!("[hot-data-cache-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8417".to_string()).parse().unwrap_or(8417);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[hot-data-cache-rs] ready on :{}", port);
