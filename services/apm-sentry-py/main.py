"""
apm-sentry-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("apm-sentry-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/apm_sentry_py")
...
PORT = int(os.environ.get("PORT", "9589"))
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[apm-sentry-py] ready on :%d", PORT)
    logger.info(f"[apm-sentry-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="apm-sentry-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "apm-sentry-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "apm-sentry-py", "total_records": count}
    except Exception:
        return {"service": "apm-sentry-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("apm-sentry-py",))
...
# --- Domain Logic ---
def gen_id():
    return "APM-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"apm_sentry_{self.path}"
        ...
        logger.info(f"[apm-sentry-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "apm-sentry-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"apm-sentry-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"apm-sentry-py\"}} {error_count}\n'
            )
        else:
...
def delete_record(record_id: str):
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("apm_sentry_py", body)
            cache_set(f"{self.get_tenant_id()}:last_post", str(body))
            self.respond(201, {"created": True, "data": result})
        elif path == "/v1/apm-sentry/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/apm-sentry/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/apm-sentry/health-checks":
            result = run_health_checks(body.get("services", []))
            self.respond(200, result)
        elif path == "/v1/apm-sentry/sla-metrics":
