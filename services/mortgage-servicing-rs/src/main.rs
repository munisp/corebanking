...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[mortgage-servicing-rs] starting");
    ...
    log::info!("[mortgage-servicing-rs] database connected, schema initialized");
    ...
    log::info!("[mortgage-servicing-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8995".to_string()).parse().unwrap_or(8995);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[mortgage-servicing-rs] ready on :{}", port);
...
async fn list_records(data: web::Data<AppState>, req: actix_web::HttpRequest) -> HttpResponse {
    ...
    let rows = sqlx::query("SELECT id, status, created_at FROM loans WHERE tenant_id::text = $1 ORDER BY created_at DESC LIMIT 50")
        .bind(tenant_id)
        .fetch_all(&data.db)
        .await;
