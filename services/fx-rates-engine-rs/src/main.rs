...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[fx-rates-engine-rs] starting");

    let db_name = "fx-rates-engine-rs".replace("-", "_");
    ...
    log::info!("[fx-rates-engine-rs] database connected, schema initialized");
    ...
    log::info!("[fx-rates-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8982".to_string()).parse().unwrap_or(8982);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[fx-rates-engine-rs] ready on :{}", port);
