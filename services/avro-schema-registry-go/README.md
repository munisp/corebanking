...
```bash
# Build
go build -o avro-schema-registry-go .

# Run
PORT=6379 ./avro-schema-registry-go
```
...
```bash
docker build -t 54bank/avro-schema-registry-go .
docker run -p 6379:6379 54bank/avro-schema-registry-go
```
