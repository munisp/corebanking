...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[billing-event-processor-py] ready on :%d", PORT)
    logger.info(f"[billing-event-processor-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="billing-event-processor-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "billing-event-processor-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "billing-event-processor-py", "total_records": count}
    except Exception:
        return {"service": "billing-event-processor-py", "total_records": 0}
...
def process_billing_event(event_type, amount, customer_tier="standard"):
    multipliers = {"standard": 1.0, "premium": 0.8, "enterprise": 0.6}
    mult = multipliers.get(customer_tier, 1.0)
    return {"event": event_type, "base_amount": amount, "billed": round(amount * mult, 2), "tier": customer_tier}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            ...
            _process_billing_event_result = process_billing_event(body.get("data", {}))
