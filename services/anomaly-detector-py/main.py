"""
anomaly-detector-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("anomaly-detector-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/anomaly_detector_py")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[anomaly-detector-py] ready on :%d", PORT)
    logger.info(f"[anomaly-detector-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="anomaly-detector-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "anomaly-detector-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "anomaly-detector-py", "total_records": count}
    except Exception:
        return {"service": "anomaly-detector-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("anomaly-detector-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("anomaly-detector-py",))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"anomaly_detector_{self.path}"
        ...
        logger.info(f"[anomaly-detector-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/healthz":
            ...
            self.respond(200, {"alive": True})
                ...
                self._json(200, {"service": "anomaly-detector-py", **_degrade.status()})
            elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"anomaly-detector-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"anomaly-detector-py\"}} {error_count}\n'
            )
        else:
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            result = db_insert("anomaly_detector_py", body)
