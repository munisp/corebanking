...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[mojaloop-tb-bridge-rs] starting");

    let db_name = "mojaloop-tb-bridge-rs".replace("-", "_");
    ...
    log::info!("[mojaloop-tb-bridge-rs] database connected, schema initialized");
    ...
    log::info!("[mojaloop-tb-bridge-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8333".to_string()).parse().unwrap_or(8333);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[mojaloop-tb-bridge-rs] ready on :{}", port);
