...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("mojaloop-tb-bridge.processed", &serde_json::json!({"status": "success"}));
...
async fn sync_transfer(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "sync_transfer"});
    db_persist(&state, "sync_transfer", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("mojaloop-tb-bridge-rs: upstream call ok"),
        Err(e) => eprintln!("mojaloop-tb-bridge-rs: upstream call failed: {}", e),
    }
...
async fn reconcile_positions(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    if !rl_allow() {
        return HttpResponse::TooManyRequests().json(json!({"error": "rate_limit_exceeded"}));
    }
    if let Err(resp) = check_jwt(&req) { return resp; }
    ...
    let _result_data = json!({"endpoint": "reconcile_positions"});
    db_persist(&state, "reconcile_positions", &_result_data).await;
...
async fn settlement_window(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    if !rl_allow() {
        return HttpResponse::TooManyRequests().json(json!({"error": "rate_limit_exceeded"}));
    }
    if let Err(resp) = check_jwt(&req) { return resp; }
    ...
    let _result_data = json!({"endpoint": "settlement_window"});
    db_persist(&state, "settlement_window", &_result_data).await;
...
async fn list_records(req: actix_web::HttpRequest, state: web::Data<AppState>, query: web::Query<std::collections::HashMap<String, String>>) -> HttpResponse {
    ...
    if let Some(ref client) = state.db_client {
        match client.query(
            ...
            &[&"mojaloop_tb_bridge_rs", &(limit as i64), &(offset as i64)]
        ).await {
            Ok(rows) => {
                ...
                let total: i64 = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"mojaloop_tb_bridge_rs"]).await.map(|r| r.get(0)).unwrap_or(0);
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    if let Some(ref client) = state.db_client {
        if let Ok(row) = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"mojaloop_tb_bridge_rs"]).await {
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "mojaloop-tb-bridge-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"mojaloop-tb-bridge-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"mojaloop-tb-bridge-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "mojaloop_tb_bridge_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("mojaloop-tb-bridge-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("mojaloop-tb-bridge-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("mojaloop-tb-bridge-rs", 10325);
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[mojaloop-tb-bridge-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("messaging.interbank", "mojaloop-tb-bridge");
    eprintln!("[mojaloop-tb-bridge] Data flow initialized: topic=messaging.interbank");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "mojaloop-tb-bridge-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "mojaloop_tb_bridge");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "mojaloop_tb_bridge");
    }
