...
async fn main() -> std::io::Result<()> {
    ...
    let db_name = "recon-engine-rs".replace("-", "_");
    ...
    log::info!("[recon-engine-rs] database connected, schema initialized");
    ...
    log::info!("[recon-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8323".to_string()).parse().unwrap_or(8323);
