...
func TestCRC16(t *testing.T) {
	crc := crc16("test")
	if crc == 0 { t.Error("CRC should not be zero for non-empty input") }
}

func TestGenerateEMVQR(t *testing.T) {
	payload := generateEMVQR("MERCH001", "TestMerchant", "5411", 100000, true)
	if payload == "" { t.Error("EMV QR payload should not be empty") }
}
