...
// --- NQR (Nigeria Quick Response) QR code payments — CBN cashless initiative ---
...
func handleHealthz(w http.ResponseWriter, r *http.Request) {
	...
	count := len(records)
	recordsMu.RUnlock()
	respondJSON(w, 200, map[string]interface{}{"status": "healthy", "service": "nqr-payments-go", "version": "1.0.0", "records": count})
}
...
func handleCreate(w http.ResponseWriter, r *http.Request) {
	...
	rec := Record{
		ID: fmt.Sprintf("NQR-%s", secureRandID()),
...
func main() {
	...
	mux.HandleFunc("/api/v1/nqr_payments", handleList)
	mux.HandleFunc("/api/v1/nqr_payments/create", handleCreate)
	log.Printf("NQR (Nigeria Quick Response) QR code payments — CBN cashless initiative listening on :%s", PORT)
