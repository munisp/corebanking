...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[liveness-detection-rs] starting");

    let db_name = "liveness-detection-rs".replace("-", "_");
    ...
    log::info!("[liveness-detection-rs] database connected, schema initialized");
    ...
    log::info!("[liveness-detection-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8345".to_string()).parse().unwrap_or(8345);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[liveness-detection-rs] ready on :{}", port);
