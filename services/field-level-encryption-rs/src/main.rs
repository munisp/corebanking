...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("field-level-encryption.processed", &serde_json::json!({"status": "success"}));
...
async fn encrypt_field(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("field-level-encryption-rs: upstream call ok"),
        Err(e) => eprintln!("field-level-encryption-rs: upstream call failed: {}", e),
    }
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "field-level-encryption-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"field-level-encryption-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"field-level-encryption-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "field_level_encryption_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("field-level-encryption-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("field-level-encryption-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("field-level-encryption-rs", 10427);
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[field-level-encryption-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("security.infra", "field-level-encryption");
    eprintln!("[field-level-encryption] Data flow initialized: topic=security.infra");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "field-level-encryption-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "field_level_encryption");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "field_level_encryption");
    }
