...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[field-level-encryption-rs] starting");

    let db_name = "field-level-encryption-rs".replace("-", "_");
    ...
    log::info!("[field-level-encryption-rs] database connected, schema initialized");
    ...
    log::info!("[field-level-encryption-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8282".to_string()).parse().unwrap_or(8282);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[field-level-encryption-rs] ready on :{}", port);
