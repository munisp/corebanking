...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[commodity-exchange-rs] starting");

    let db_name = "commodity-exchange-rs".replace("-", "_");
    ...
    log::info!("[commodity-exchange-rs] database connected, schema initialized");
    ...
    log::info!("[commodity-exchange-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8630".to_string()).parse().unwrap_or(8630);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[commodity-exchange-rs] ready on :{}", port);
