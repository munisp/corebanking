...
// Fluvio WASM transform — SmartModule transformations for stream processing
async fn transform(body: web::Json<serde_json::Value>) -> HttpResponse {
    REQUEST_COUNT.fetch_add(1, AtomicOrdering::Relaxed);
    let input = body.get("data").and_then(|v| v.as_str()).unwrap_or("");
    let transform_type = body.get("transform").and_then(|v| v.as_str()).unwrap_or("passthrough");
    
    let output = match transform_type {
        "uppercase" => input.to_uppercase(),
        "lowercase" => input.to_lowercase(),
        "reverse" => input.chars().rev().collect(),
        "hash" => { let mut h = Sha256::new(); h.update(input.as_bytes()); format!("{:x}", h.finalize()) },
        "mask_pii" => {
            // Mask Nigerian BVN/NIN patterns
            let masked = input.replace(|c: char| c.is_ascii_digit(), "*");
            masked
        },
        _ => input.to_string(),
    };
    
    HttpResponse::Ok().json(json!({"input": input, "output": output, "transform": transform_type}))
}

async fn list_transforms() -> HttpResponse {
    HttpResponse::Ok().json(json!({"transforms": [
        ...
        {"name": "mask_pii", "description": "Mask PII (BVN/NIN/phone)"},
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8305);
    ...
    println!("fluvio-wasm-transform-rs v2.0 on :{}", port);
...
fn init_data_flow() -> EventBus {
    ...
    eprintln!("[fluvio-wasm-transform] Data flow initialized: topic=infra.events");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "fluvio-wasm-transform-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "fluvio_wasm_transform");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "fluvio_wasm_transform");
    }
