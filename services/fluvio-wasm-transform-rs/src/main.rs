...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[fluvio-wasm-transform-rs] starting");

    let db_name = "fluvio-wasm-transform-rs".replace("-", "_");
    ...
    log::info!("[fluvio-wasm-transform-rs] database connected, schema initialized");
    ...
    log::info!("[fluvio-wasm-transform-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8541".to_string()).parse().unwrap_or(8541);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[fluvio-wasm-transform-rs] ready on :{}", port);
