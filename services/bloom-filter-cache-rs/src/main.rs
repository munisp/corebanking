...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[bloom-filter-cache-rs] starting");

    let db_name = "bloom-filter-cache-rs".replace("-", "_");
    ...
    log::info!("[bloom-filter-cache-rs] database connected, schema initialized");
    ...
    log::info!("[bloom-filter-cache-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8634".to_string()).parse().unwrap_or(8634);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[bloom-filter-cache-rs] ready on :{}", port);
