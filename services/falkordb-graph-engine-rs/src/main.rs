...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[falkordb-graph-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8277".to_string()).parse().unwrap_or(8277);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[falkordb-graph-engine-rs] ready on :{}", port);
