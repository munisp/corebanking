...
54Bank Security Gateway service

## Quick Start

```bash
# Build
go build -o security-gateway-go .

# Run
PORT=8101 ./security-gateway-go
```
...
```bash
docker build -t 54bank/security-gateway-go .
docker run -p 8101:8101 54bank/security-gateway-go
```
