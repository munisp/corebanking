...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[watchlist-manager-rs] starting");

    let db_name = "watchlist-manager-rs".replace("-", "_");
    ...
    log::info!("[watchlist-manager-rs] database connected, schema initialized");
    ...
    log::info!("[watchlist-manager-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8676".to_string()).parse().unwrap_or(8676);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[watchlist-manager-rs] ready on :{}", port);
