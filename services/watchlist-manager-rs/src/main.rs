...
async fn search_watchlist(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("watchlist-manager-rs: upstream call ok"),
        Err(e) => eprintln!("watchlist-manager-rs: upstream call failed: {}", e),
    }
...
async fn list_records(req: actix_web::HttpRequest, state: web::Data<AppState>, query: web::Query<std::collections::HashMap<String, String>>) -> HttpResponse {
    ...
    if let Some(ref client) = state.db_client {
        match client.query(
            ...
            &[&"watchlist_manager_rs", &(limit as i64), &(offset as i64)]
        ).await {
            Ok(rows) => {
                ...
                let total: i64 = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"watchlist_manager_rs"]).await.map(|r| r.get(0)).unwrap_or(0);
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    if let Some(ref client) = state.db_client {
        if let Ok(row) = client.query_one("SELECT COUNT(*) FROM service_records WHERE service = $1", &[&"watchlist_manager_rs"]).await {
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        ...
        let svc_name = String::from("watchlist-manager-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("watchlist-manager-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("watchlist-manager-rs", 10429);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[watchlist-manager-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    #[test]
    fn test_search_score_exists() {
        // Verify search_score compiles and is callable
        // Domain function: search_score(query: &str, entry_name: &str) -> f64
        assert!(true, "search_score should be defined");
    }

    #[test]
    fn test_list_type_priority_exists() {
        // Verify list_type_priority compiles and is callable
        // Domain function: list_type_priority(list_type: &str) -> u8
        assert!(true, "list_type_priority should be defined");
    }

    #[test]
    fn test_search_watchlist_exists() {
        // Verify search_watchlist compiles and is callable
        // Domain function: search_watchlist(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "search_watchlist should be defined");
    }

    #[test]
    fn test_add_entry_exists() {
        // Verify add_entry compiles and is callable
        // Domain function: add_entry(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "add_entry should be defined");
    }
    #[test]
