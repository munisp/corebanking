...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[kpi-threshold-monitor-rs] starting");

    let db_name = "kpi-threshold-monitor-rs".replace("-", "_");
    ...
    log::info!("[kpi-threshold-monitor-rs] database connected, schema initialized");
    ...
    log::info!("[kpi-threshold-monitor-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8002".to_string()).parse().unwrap_or(8002);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[kpi-threshold-monitor-rs] ready on :{}", port);
