...
// egress-controller-rs — Network egress control
...
async fn check_egress(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("egress-controller-rs: upstream call ok"),
        Err(e) => eprintln!("egress-controller-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "egress-controller-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "egress-controller-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"egress-controller-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"egress-controller-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "egress_controller_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("egress-controller-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8212);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("egress-controller-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("egress-controller-rs on port {}", port);
    start_grpc_server("egress-controller-rs", 10468);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[egress-controller-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/check_egress", web::post().to(check_egress))
...
mod tests {
    ...
    #[test]
    fn test_allowed_destination_exists() {
        // Verify allowed_destination compiles and is callable
        // Domain function: allowed_destination(host: &str, whitelist: &[&str]) -> bool
        assert!(true, "allowed_destination should be defined");
    }

    #[test]
    fn test_check_egress_exists() {
        // Verify check_egress compiles and is callable
        // Domain function: check_egress(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "check_egress should be defined");
    }
    #[test]
