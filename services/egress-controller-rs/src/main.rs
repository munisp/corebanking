...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[egress-controller-rs] starting");

    let db_name = "egress-controller-rs".replace("-", "_");
    ...
    log::info!("[egress-controller-rs] database connected, schema initialized");
    ...
    log::info!("[egress-controller-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8517".to_string()).parse().unwrap_or(8517);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[egress-controller-rs] ready on :{}", port);
