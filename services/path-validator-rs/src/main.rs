...
// path-validator-rs — URL path traversal prevention
...
async fn validate_path(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "validate_path"});
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "path-validator-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "path-validator-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"path-validator-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"path-validator-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "path_validator_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("path-validator-rs");
...
fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8233);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("path-validator-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("path-validator-rs on port {}", port);
    start_grpc_server("path-validator-rs", 10382);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[path-validator-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/validate_path", web::post().to(validate_path))
...
mod tests {
    ...
    #[test]
    fn test_is_path_traversal_exists() {
        // Verify is_path_traversal compiles and is callable
        // Domain function: is_path_traversal(path: &str) -> bool
        assert!(true, "is_path_traversal should be defined");
    }

    #[test]
    fn test_validate_path_exists() {
        // Verify validate_path compiles and is callable
        // Domain function: validate_path(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "validate_path should be defined");
    }
    #[test]
