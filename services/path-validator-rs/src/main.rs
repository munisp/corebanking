...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[path-validator-rs] starting");

    let db_name = "path-validator-rs".replace("-", "_");
    ...
    log::info!("[path-validator-rs] database connected, schema initialized");
    ...
    log::info!("[path-validator-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8427".to_string()).parse().unwrap_or(8427);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[path-validator-rs] ready on :{}", port);
