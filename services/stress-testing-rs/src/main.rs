...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[stress-testing-rs] starting");

    let db_name = "stress-testing-rs".replace("-", "_");
    ...
    log::info!("[stress-testing-rs] database connected, schema initialized");
    ...
    log::info!("[stress-testing-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8658".to_string()).parse().unwrap_or(8658);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[stress-testing-rs] ready on :{}", port);
