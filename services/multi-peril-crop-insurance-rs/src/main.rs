...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[multi-peril-crop-insurance-rs] starting");

    let db_name = "multi-peril-crop-insurance-rs".replace("-", "_");
    ...
    log::info!("[multi-peril-crop-insurance-rs] database connected, schema initialized");
    ...
    log::info!("[multi-peril-crop-insurance-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8887".to_string()).parse().unwrap_or(8887);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[multi-peril-crop-insurance-rs] ready on :{}", port);
