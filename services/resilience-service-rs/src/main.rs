...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[resilience-service-rs] starting");

    let db_name = "resilience-service-rs".replace("-", "_");
    ...
    log::info!("[resilience-service-rs] database connected, schema initialized");
    ...
    log::info!("[resilience-service-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8775".to_string()).parse().unwrap_or(8775);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[resilience-service-rs] ready on :{}", port);
