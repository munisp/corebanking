...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[circuit-breaker-rs] starting");

    let db_name = "circuit-breaker-rs".replace("-", "_");
    ...
    log::info!("[circuit-breaker-rs] database connected, schema initialized");
    ...
    log::info!("[circuit-breaker-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8054".to_string()).parse().unwrap_or(8054);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[circuit-breaker-rs] ready on :{}", port);
