...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[etd-trading-rs] starting");

    let db_name = "etd-trading-rs".replace("-", "_");
    ...
    log::info!("[etd-trading-rs] database connected, schema initialized");
    ...
    log::info!("[etd-trading-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8441".to_string()).parse().unwrap_or(8441);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[etd-trading-rs] ready on :{}", port);
