...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[immutable-audit-rs] starting");

    let db_name = "immutable-audit-rs".replace("-", "_");
    ...
    log::info!("[immutable-audit-rs] database connected, schema initialized");
    ...
    log::info!("[immutable-audit-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8045".to_string()).parse().unwrap_or(8045);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[immutable-audit-rs] ready on :{}", port);
