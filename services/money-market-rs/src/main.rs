...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[money-market-rs] starting");

    let db_name = "money-market-rs".replace("-", "_");
    ...
    log::info!("[money-market-rs] database connected, schema initialized");
    ...
    log::info!("[money-market-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8048".to_string()).parse().unwrap_or(8048);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[money-market-rs] ready on :{}", port);
