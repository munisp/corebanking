...
```bash
# Build
go build -o browser-fingerprint-go .

# Run
PORT=6379 ./browser-fingerprint-go
```
...
```bash
docker build -t 54bank/browser-fingerprint-go .
docker run -p 6379:6379 54bank/browser-fingerprint-go
```
