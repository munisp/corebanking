...
"""
postgres-vacuum-py — Production-hardened service
"""
import os
import sys
import json
...
PORT = int(os.environ.get("PORT", "9628"))
...
def db_query(table, page=1, limit=50):
    ...
    try:
        ...
        cur.execute("SELECT id, data, created_at FROM records WHERE service = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    ("postgres-vacuum-py", limit, offset))
        rows = cur.fetchall()
        items = []
        for row in rows:
            ...
            items.append(item)
        cur.execute("SELECT COUNT(*) FROM records WHERE service = %s", ("postgres-vacuum-py",))
...
# --- Domain Logic ---
def gen_id():
    return "POS-" + "".join(random.choices(string.hexdigits[:16].upper(), k=8))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        _cache_key = f"postgres_vacuum_{self.path}"
        ...
        logger.info(f"[postgres-vacuum-py] {self.command} {self.path} trace={trace_id}")
        inc_requests()
        if not _rl_allow():
        ...
        if path == "/v1/cache-metrics":
            ...
            self._json(200, {"service": "postgres-vacuum-py", **_degrade.status()})
        elif path == "/v1/alerts":
            ...
            body = (
                ...
                f'requests_total{{service=\"postgres-vacuum-py\"}} {request_count}\n'
                f'# HELP errors_total Total errors\n'
                f'# TYPE errors_total counter\n'
                f'errors_total{{service=\"postgres-vacuum-py\"}} {error_count}\n'
            )
            self.send_response(200)
            ...
            if err:
                ...
                return
            items, total = db_query("postgres_vacuum_py")
    ...
    def do_POST(self):
        ...
        if path == "/v1/create":
            try:
                result = db_insert("postgres_vacuum_py", body)
                ...
                return
        elif path == "/v1/postgres-vacuum/update":
            rid = body.get("id", "")
            for rec in records:
                if rec["id"] == rid:
                    if "status" in body:
        ...
        elif path == "/v1/postgres-vacuum/process":
            rid = body.get("id", "")
            for rec in records:
        ...
        elif path == "/v1/postgres-vacuum/validate-config":
            result = validate_config(body.get("config", body))
            self.respond(200, result)
        elif path == "/v1/postgres-vacuum/resource-usage":
...
_event_bus = EventBus("platform.events", "postgres-vacuum")


# --- Data Flow Emit Point ---
def emit_processing_event(action: str, data: dict) -> None:
    """Called by handlers after successful processing."""
    _event_bus.emit("postgres-vacuum." + action, data)

if __name__ == "__main__":
    get_db()
    server = HTTPServer(("0.0.0.0", PORT), Handler)
    logger.info(json.dumps({"service": "postgres-vacuum-py", "port": PORT, "message": "starting"}))
    try:
