...
```bash
# Build
go build -o regulatory-sandbox-go .

# Run
PORT=6379 ./regulatory-sandbox-go
```
...
```bash
docker build -t 54bank/regulatory-sandbox-go .
docker run -p 6379:6379 54bank/regulatory-sandbox-go
```
