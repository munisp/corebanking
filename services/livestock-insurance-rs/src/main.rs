...
// livestock-insurance-rs — Livestock insurance claims processing
...
async fn degradation_status() -> HttpResponse {
    ...
    _bus.emit("livestock-insurance.processed", &serde_json::json!({"status": "success"}));
...
async fn process_claim(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("livestock-insurance-rs: upstream call ok"),
        Err(e) => eprintln!("livestock-insurance-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "livestock-insurance-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "livestock-insurance-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"livestock-insurance-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"livestock-insurance-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "livestock_insurance_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("livestock-insurance-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8176);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("livestock-insurance-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("livestock-insurance-rs on port {}", port);
    start_grpc_server("livestock-insurance-rs", 10400);
        ...
        App::new()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[livestock-insurance-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/process_claim", web::post().to(process_claim))
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("agriculture.operations", "livestock-insurance");
    eprintln!("[livestock-insurance] Data flow initialized: topic=agriculture.operations");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "livestock-insurance-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "livestock_insurance");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "livestock_insurance");
    }
