...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[livestock-insurance-rs] starting");

    let db_name = "livestock-insurance-rs".replace("-", "_");
    ...
    log::info!("[livestock-insurance-rs] database connected, schema initialized");
    ...
    log::info!("[livestock-insurance-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8218".to_string()).parse().unwrap_or(8218);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[livestock-insurance-rs] ready on :{}", port);
