...
async fn health() -> HttpResponse {
    let _claim_status = claim_status(true, false);
...
async fn process_claim(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("livestock-insurance-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("livestock-insurance-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("livestock-insurance-rs: upstream call join failed: {}", e),
    }
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .service(web::resource("/api/v1/products").wrap(actix_web::middleware::from_fn(jwt_route_guard)).route(web::post().to(create_record)))
            .service(web::resource("/api/v1/products/{id}").wrap(actix_web::middleware::from_fn(jwt_route_guard)).route(web::get().to(get_record)))
