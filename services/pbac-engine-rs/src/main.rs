...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[pbac-engine-rs] starting");

    let db_name = "pbac-engine-rs".replace("-", "_");
    ...
    log::info!("[pbac-engine-rs] database connected, schema initialized");
    ...
    log::info!("[pbac-engine-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8703".to_string()).parse().unwrap_or(8703);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[pbac-engine-rs] ready on :{}", port);
