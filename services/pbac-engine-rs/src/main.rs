...
// pbac-engine-rs — Policy-Based Access Control engine
...
async fn evaluate_policy_handler(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("pbac-engine-rs: upstream call ok"),
        Err(e) => eprintln!("pbac-engine-rs: upstream call failed: {}", e),
    }
