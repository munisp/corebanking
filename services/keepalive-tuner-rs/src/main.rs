...
// keepalive-tuner-rs — TCP keepalive tuning
...
async fn tune_keepalive(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "tune_keepalive"});
    db_persist(&state, "tune_keepalive", &_result_data).await;
    // Inter-service call
    ...
    match call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}") {
        Ok(_resp) => eprintln!("keepalive-tuner-rs: upstream call ok"),
        Err(e) => eprintln!("keepalive-tuner-rs: upstream call failed: {}", e),
    }
...
async fn stats(state: web::Data<AppState>) -> HttpResponse {
    let records = state.records.lock().unwrap_or_else(|e| { eprintln!("Mutex poisoned, recovering: {}", e); e.into_inner() });
    HttpResponse::Ok().json(json!({"total": records.len(), "service": "keepalive-tuner-rs"}))
}
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "keepalive-tuner-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"keepalive-tuner-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"keepalive-tuner-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "keepalive_tuner_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("keepalive-tuner-rs");
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8226);
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("keepalive-tuner-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    println!("keepalive-tuner-rs on port {}", port);
    start_grpc_server("keepalive-tuner-rs", 10493);
    HttpServer::new(move || {
        App::new()
            .wrap(
                Cors::default()
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[keepalive-tuner-rs] {} {} trace={}", req.method(), req.path(), trace_id);
            ...
            .route("/v1/tune_keepalive", web::post().to(tune_keepalive))
...
mod tests {
    ...
    #[test]
    fn test_optimal_interval_exists() {
        // Verify optimal_interval compiles and is callable
        // Domain function: optimal_interval(latency_ms: u64) -> u64
        assert!(true, "optimal_interval should be defined");
    }

    #[test]
    fn test_tune_keepalive_exists() {
        // Verify tune_keepalive compiles and is callable
        // Domain function: tune_keepalive(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "tune_keepalive should be defined");
    }
    #[test]
