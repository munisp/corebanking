...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[keepalive-tuner-rs] starting");

    let db_name = "keepalive-tuner-rs".replace("-", "_");
    ...
    log::info!("[keepalive-tuner-rs] database connected, schema initialized");
    ...
    log::info!("[keepalive-tuner-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8336".to_string()).parse().unwrap_or(8336);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[keepalive-tuner-rs] ready on :{}", port);
