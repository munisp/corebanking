...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[tigerbeetle-multicurrency-rs] starting");

    let db_name = "tigerbeetle-multicurrency-rs".replace("-", "_");
    ...
    log::info!("[tigerbeetle-multicurrency-rs] database connected, schema initialized");
    ...
    log::info!("[tigerbeetle-multicurrency-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8823".to_string()).parse().unwrap_or(8823);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[tigerbeetle-multicurrency-rs] ready on :{}", port);
