...
async fn health(state: web::Data<AppState>) -> HttpResponse {
    ...
    _bus.emit("tigerbeetle-multicurrency.processed", &serde_json::json!({"status": "success"}));
    HttpResponse::Ok().json(json!({
        "status": overall, "service": "tigerbeetle-multicurrency-rs",
...
// TigerBeetle multicurrency — FX operations with multiple ledgers
async fn create_fx_transfer(req: actix_web::HttpRequest, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let from_currency = body.get("from_currency").and_then(|v| v.as_str()).unwrap_or("NGN");
    let to_currency = body.get("to_currency").and_then(|v| v.as_str()).unwrap_or("USD");
    let amount_kobo = body.get("amount_kobo").and_then(|v| v.as_u64()).unwrap_or(0);
    let rate_bps = body.get("rate_bps").and_then(|v| v.as_u64()).unwrap_or(0);
    
    if amount_kobo == 0 { return HttpResponse::BadRequest().json(json!({"error": "amount required"})); }
    
    let converted = (amount_kobo * rate_bps) / 10000;
    HttpResponse::Ok().json(json!({
        "transfer_id": format!("FX-{}", REQUEST_COUNT.load(AtomicOrdering::Relaxed)),
        "from": {"currency": from_currency, "amount_kobo": amount_kobo, "ledger": 1},
        "to": {"currency": to_currency, "amount_kobo": converted, "ledger": 2},
        "rate_bps": rate_bps,
        "status": "posted",
    }))
}

async fn list_ledgers() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ledgers": [
        {"id": 1, "currency": "NGN", "name": "Nigerian Naira"},
        {"id": 2, "currency": "USD", "name": "US Dollar"},
        {"id": 3, "currency": "GBP", "name": "British Pound"},
        {"id": 4, "currency": "EUR", "name": "Euro"},
        {"id": 5, "currency": "GHS", "name": "Ghanaian Cedi"},
    ]}))
}
...
async fn main() -> std::io::Result<()> {
    let port: u16 = env::var("PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8303);
    ...
    println!("tigerbeetle-multicurrency-rs v2.0 on :{}", port);
...
fn init_data_flow() -> EventBus {
    let bus = EventBus::new("treasury.fx", "tigerbeetle-multicurrency");
    eprintln!("[tigerbeetle-multicurrency] Data flow initialized: topic=treasury.fx");
    bus
}
...
mod tests {
    ...
    fn test_service_config() {
        ...
        assert!(true, "tigerbeetle-multicurrency-rs service module loads");
    }
    ...
    fn test_eventbus_creation() {
        let bus = EventBus::new("test.topic", "tigerbeetle_multicurrency");
        assert_eq!(bus.topic, "test.topic");
        assert_eq!(bus.service_name, "tigerbeetle_multicurrency");
    }
