...
async fn convert_currency(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    let _result_data = json!({"endpoint": "convert_currency"});
    db_persist(&state, "convert_currency", &_result_data).await;
    // Inter-service call
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("tigerbeetle-multicurrency-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("tigerbeetle-multicurrency-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("tigerbeetle-multicurrency-rs: upstream call join failed: {}", e),
    }
...
async fn readyz() -> HttpResponse {
    HttpResponse::Ok().json(json!({"ready": true, "service": "tigerbeetle-multicurrency-rs"}))
}
...
async fn prom_metrics() -> HttpResponse {
    ...
    let body = format!(
        "# TYPE requests_total counter\nrequests_total{{service=\"tigerbeetle-multicurrency-rs\"}} {}\n         # TYPE errors_total counter\nerrors_total{{service=\"tigerbeetle-multicurrency-rs\"}} {}\n", r, e);
...
async fn db_persist(state: &web::Data<AppState>, endpoint: &str, data: &serde_json::Value) {
    if let Some(ref client) = state.db_client {
        let id = format!("{}_{}_{}", "tigerbeetle_multicurrency_rs", endpoint, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos()).unwrap_or(0));
        let svc_name = String::from("tigerbeetle-multicurrency-rs");
...
async fn main() -> std::io::Result<()> {
    ...
    let db_client = if let Ok(url) = std::env::var("DATABASE_URL") {
        match init_db(&url).await {
            Some(c) => { println!("tigerbeetle-multicurrency-rs: connected to Postgres"); Some(std::sync::Arc::new(c)) }
            None => None,
        }
    } else { None };
    ...
    start_grpc_server("tigerbeetle-multicurrency-rs", 10402);
    HttpServer::new(move || {
        App::new()
                .wrap(
            ...
            .wrap_fn(|req, srv| {
                ...
                let trace_id = req.headers().get("X-Trace-Id")
                    ...
                    .unwrap_or("none")
                    .to_string();
                eprintln!("[tigerbeetle-multicurrency-rs] {} {} trace={}", req.method(), req.path(), trace_id);
...
mod tests {
    ...
    #[test]
    fn test_currency_code_to_ledger_exists() {
        // Verify currency_code_to_ledger compiles and is callable
        // Domain function: currency_code_to_ledger(code: &str) -> u32
        assert!(true, "currency_code_to_ledger should be defined");
    }

    #[test]
    fn test_convert_currency_exists() {
        // Verify convert_currency compiles and is callable
        // Domain function: convert_currency(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "convert_currency should be defined");
    }
    #[test]
