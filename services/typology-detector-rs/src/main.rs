...
async fn main() -> std::io::Result<()> {
    ...
    let db_name = "typology-detector-rs".replace("-", "_");
    ...
    log::info!("[typology-detector-rs] database connected, schema initialized");
    ...
    log::info!("[typology-detector-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8340".to_string()).parse().unwrap_or(8340);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[typology-detector-rs] ready on :{}", port);
