...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[express-rate-limiter-rs] starting");

    let db_name = "express-rate-limiter-rs".replace("-", "_");
    ...
    log::info!("[express-rate-limiter-rs] database connected, schema initialized");
    ...
    log::info!("[express-rate-limiter-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8700".to_string()).parse().unwrap_or(8700);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[express-rate-limiter-rs] ready on :{}", port);
