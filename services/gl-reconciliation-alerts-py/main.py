"""
gl-reconciliation-alerts-py - Production-ready service with PostgreSQL persistence.
...
logger = logging.getLogger("gl-reconciliation-alerts-py")

# Configuration
DATABASE_URL = os.getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/gl_reconciliation_alerts_py")
...
def init_schema():
    ...
    settled_at TIMESTAMPTZ
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS idx_settlements_tenant ON settlements(tenant_id)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_settlements_status ON settlements(status)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_settlements_created ON settlements(created_at DESC)")
...
async def lifespan(app: FastAPI):
    init_schema()
    logger.info(f"[gl-reconciliation-alerts-py] ready on :%d", PORT)
    logger.info(f"[gl-reconciliation-alerts-py] middleware: keycloak=%s kafka=%s redis=%s opensearch=%s permify=%s",
...
app = FastAPI(title="gl-reconciliation-alerts-py", version="1.0.0", lifespan=lifespan)
...
@app.get("/healthz")
def health():
    return {"status": "healthy", "service": "gl-reconciliation-alerts-py", "version": "1.0.0"}
...
def metrics():
    try:
        ...
        return {"service": "gl-reconciliation-alerts-py", "total_records": count}
    except Exception:
        return {"service": "gl-reconciliation-alerts-py", "total_records": 0}
...
def list_records(x_tenant_id: Optional[str] = Header(None)):
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        if x_tenant_id:
            cur.execute(
                "SELECT id, status, created_at FROM settlements WHERE tenant_id = %s::uuid ORDER BY created_at DESC LIMIT 50",
                (x_tenant_id,)
            )
        else:
            cur.execute("SELECT id, status, created_at FROM settlements ORDER BY created_at DESC LIMIT 50")
        rows = cur.fetchall()
...
@app.post("/api/v1/settlements", status_code=201)
def create_record(body: CreateRequest, x_tenant_id: Optional[str] = Header(None)):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO settlements (id, tenant_id, status) VALUES (%s::uuid, %s::uuid, %s)",
        ...
        cur.execute(
            ...
            ("settlements.created", record_id, payload)
        )
    conn.commit()
    return {"id": record_id, "status": "created"}


@app.get("/api/v1/settlements/{record_id}")
def get_record(record_id: str):
    conn = get_db()
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute("SELECT id, status, created_at FROM settlements WHERE id = %s::uuid", (record_id,))
        row = cur.fetchone()
    if not row:
...
@app.put("/api/v1/settlements/{record_id}")
def update_record(record_id: str, body: UpdateRequest):
    status = body.status or "updated"
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE settlements SET status = %s, updated_at = NOW() WHERE id = %s::uuid",
            (status, record_id)
        )
        payload = json.dumps({"id": record_id, "status": status})
        cur.execute(
            "INSERT INTO outbox (event_type, aggregate_id, payload) VALUES (%s, %s, %s::jsonb)",
            ("settlements.updated", record_id, payload)
        )
    conn.commit()
    return {"id": record_id, "status": status}


@app.delete("/api/v1/settlements/{record_id}", status_code=204)
def delete_record(record_id: str):
    conn = get_db()
    with conn.cursor() as cur:
        cur.execute("UPDATE settlements SET status = 'deleted', updated_at = NOW() WHERE id = %s::uuid", (record_id,))
        payload = json.dumps({"id": record_id})
        cur.execute(
            "INSERT INTO outbox (event_type, aggregate_id, payload) VALUES (%s, %s, %s::jsonb)",
            ("settlements.deleted", record_id, payload)
        )
    conn.commit()
