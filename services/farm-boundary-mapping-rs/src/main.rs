...
async fn map_boundary(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse {
    ...
    match tokio::task::spawn_blocking(move || call_service_sync(&format!("{}/v1/screen", _upstream_url), "{}")).await {
        Ok(Ok(_resp)) => eprintln!("farm-boundary-mapping-rs: upstream call ok"),
        Ok(Err(e)) => eprintln!("farm-boundary-mapping-rs: upstream call failed: {}", e),
        Err(e) => eprintln!("farm-boundary-mapping-rs: upstream call join failed: {}", e),
    }
...
mod tests {
    ...
    #[test]
    fn test_map_boundary_exists() {
        // Verify map_boundary compiles and is callable
        // Domain function: map_boundary(req: actix_web::HttpRequest, state: web::Data<AppState>, body: web::Json<serde_json::Value>) -> HttpResponse
        assert!(true, "map_boundary should be defined");
    }
    #[test]
