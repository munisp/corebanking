...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("[farm-boundary-mapping-rs] starting");

    let db_name = "farm-boundary-mapping-rs".replace("-", "_");
    ...
    log::info!("[farm-boundary-mapping-rs] database connected, schema initialized");
    ...
    log::info!("[farm-boundary-mapping-rs] middleware: keycloak={} kafka={} redis={} opensearch={} permify={}",
        keycloak_url, kafka_brokers, redis_url, opensearch_url, permify_url);

    let port: u16 = env::var("PORT").unwrap_or_else(|_| "8793".to_string()).parse().unwrap_or(8793);
    let data = web::Data::new(AppState { db: pool });

    log::info!("[farm-boundary-mapping-rs] ready on :{}", port);
